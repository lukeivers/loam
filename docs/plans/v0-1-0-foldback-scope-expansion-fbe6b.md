...
**Parent plan:** `docs/plans/v0-1-0-foldback-scope-expansion.md` (FBE.6b row to be backfilled in §8 register; FBE.6 row at lines 614-652 is the FOLDBACK record; FBE.8 row at lines 654-678 records the source-side fixes that closed FBE.6's blockers; FBE.6b is the re-verification).
...
| **AC.FBE.6b.7** | Negative AC: zero source/doc edits during FBE.6b. | `git diff f0f9253..SEAL_COMMIT --name-only` produces only paths under: (a) `framework/hands-off-lifecycle/seals/SEAL_COMMIT.v0-1-0-foldback-fbe6b` (NEW narrative file), (b) `docs/plans/` (sub-plan + manifest + parent backfill via universal admission), (c) `framework/hands-off-lifecycle/tests/SEAL_COMMIT` (apply step's frozen-baseline-aware sidecar bump). No source or doc edits outside the plans/seals scope. |
...
- `docs/plans/v0-1-0-foldback-scope-expansion-fbe6b.md` — this sub-plan (NEW).
- `docs/plans/v0-1-0-foldback-scope-expansion-fbe6b.manifest.yaml` — amendment manifest (NEW).
- `docs/plans/v0-1-0-foldback-scope-expansion.md` — parent §8 FBE.6b register backfill at completion.
...
- Edit `docs/plans/v0-1-0-foldback-scope-expansion.md` §8 — add new FBE.6b row (after FBE.8 row at lines 654-678), record sub-plan/manifest/seal SHAs.
