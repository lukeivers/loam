...
**Programme master:** `docs/rebuild/plans/v0-1-x-roadmap.md` §8 v0.1.x sealed-amendment register (this amendment to be backfilled at completion).
...
**Binds to:**
- **AC.PO.1 + AC.PO.2** (prime objective per `docs/rebuild/VALUE_PROPOSITION.md`) — M-FBM is the v0.1.0 memory-substrate floor; if the worker dies silently and tests pass, the primary-persona's working memory is broken without observable signal. Operational-health ACs ensure tests fail when the substrate is operationally broken.
...
3. Author manifest YAML at `docs/rebuild/plans/m-fbm-operational-health.manifest.yaml`.
...
7. Backfill parent §8 v0.1.x register row in `docs/rebuild/plans/v0-1-x-roadmap.md`.
8. `loam amend apply docs/rebuild/plans/m-fbm-operational-health.manifest.yaml` (auto-commit).
9. `loam amend seal docs/rebuild/plans/m-fbm-operational-health.manifest.yaml` (seal commit).
...
Manifest at `docs/rebuild/plans/m-fbm-operational-health.manifest.yaml` carries:
...
narrative:
  ...
  body: |
    ...
    docs/rebuild/plans/m-fbm-operational-health.md and status file at
...
- `docs/rebuild/plans/v0-1-0-foldback-scope-expansion-fbe7.md` — FBE.7 narrative seal anchor for M-FBM substrate
- `docs/rebuild/plans/v0-1-x-roadmap.md` — parent register §8 to be backfilled at completion
