"""Tests for AC.D-np.1 — `pos-amend new-plan <slug>` scaffolds a vars-file
at the predictable path.

Per `docs/rebuild/plans/pos-amend-new-plan-orchestration.md`:

    Invoking ``pos-amend new-plan <slug>`` (with no other flags) writes a
...
must successfully feed into ``pos-amend template render plan/dev-discipline``
...
from pos_amend.template_engine import parse_template
...
def test_AC_D_np_1_invocation_via_cli_main_succeeds(
    ...
    """Drive via the public ``pos-amend new-plan <slug>`` CLI surface
