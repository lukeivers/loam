...
- **133K LOC of plan-docs** in `docs/rebuild/plans/` (244 plan-doc files, 106 manifest YAMLs, 41 builder-plans). For comparison: **166K LOC of Python source** under `framework/`. The plan-doc corpus is ~80% the size of the source code.
...
Now archived at `docs/rebuild/archive/synthesis-tool-2026-05-04/`:
...
Sampling distinct shapes in `docs/rebuild/plans/`:
...
1. `docs/rebuild/plans/<name>.md` — plan-docs
2. `docs/rebuild/plans/<name>.manifest.yaml` — apply-time contract + redundant narrative
3. `docs/rebuild/plans/<name>.builder-plan.md` — builder hand-off (when present)
4. `framework/<comp>/seals/SEAL_COMMIT.<slug>` — seal narrative (per amendment)
5. `docs/rebuild/STATE.md` — top-level program state (127 lines)
6. `docs/rebuild/FUTURE_IDEAS.md` — graduated ideas (665 lines)
7. `docs/rebuild/FUTURE_IDEAS_DRAFT.md` — point-of-occurrence ideas (201 lines)
8. `docs/rebuild/BACKLOG.md` — work backlog (69 lines)
9. `docs/rebuild/VALUE_PROPOSITION.md` — prime objective spec (114 lines)
10. `docs/rebuild/plans/v0-1-x-roadmap.md` — release sequence + method-decision register (465 lines)
...
| 244 plan-docs | `ls docs/rebuild/plans/` filtered `*.md` non-builder |
...
| 4,350 LOC synthesis tool archive | `find docs/rebuild/archive/synthesis-tool-2026-05-04 \( -name "*.py" -o -name "*.yaml" -o -name "*.md" \) -exec wc -l {} +` |
...
| Highest amendment number 131 | `grep -h "^  number:" docs/rebuild/plans/*.manifest.yaml \| sort -nu \| tail` |
