...
| AC | Verdict | Evidence |
|---|---|---|
| AC.NFCLEAN.1 — Historical SHIPPED-LOCAL title sweep: v0.7.1 / v0.7.2 / v0.7.3 | GREEN | Direct `_backfill_state_md_leading_title(body, version)` invocation per version (HARD HALT #7 strict-scope ruling — top-level `apply_backfill(...)` produces 6 additional side-effect edits, deferred to F-NFCLEAN-FOLLOWON FIDRAFT). Post-fix `grep -c "PATCH SHIPPED LOCAL" docs/STATE.md` returns 2: (1) line 134 (v0.7.4 row's prose narrative describing what v0.7.4 was fixing about v0.7.3 — `**v0.7.3 PATCH SHIPPED LOCAL**` appears inside v0.7.4's defect-description prose, NOT as a leading title), (2) line 139 (in-flight v0.8.1 row's leading title — auto-flips on dispatcher publish). The 3 historical row leading titles (v0.7.1 / v0.7.2 / v0.7.3) are confirmed flipped to `**v0.7.X PATCH SHIPPED PUBLIC**` matching their bodies' SHIPPED-PUBLIC markers. Probe writeup §1 Stage 1 documents per-line verification. |
