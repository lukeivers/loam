...
**Parent plan:** `docs/rebuild/plans/v0-1-0-foldback-scope-expansion.md` §4 FBE.3 + §3 Decision B.2 (B.alt = SHIP).
...
### Surface #6 — Pre-existing dirty `docs/rebuild/FUTURE_IDEAS_DRAFT.md` may block `loam amend seal` (RECORDED)

Per dispatch and per FBE.2/FBE.7 status reports: at dispatch start the
working tree shows `docs/rebuild/FUTURE_IDEAS_DRAFT.md` as modified
...
- **AC.PO.1 + AC.PO.2** (prime objective per
  `docs/rebuild/VALUE_PROPOSITION.md`) — the dev-sdlc plugin is the
...
| **AC.FBE.3.S** | Sealed-component fence: `git diff BASELINE..SEAL_COMMIT --name-only` produces only paths under `plugins/dev-sdlc/` (the sealed component — sidecar bump + fence-test BASELINE bump) + `framework/tools/pos-publish-framework-only/` (manifest owner: 2-section partition-manifest edit + 2 test fixture spot-check edits) + `docs/rebuild/plans/` (universal_paths.prefixes; sub-plan + manifest YAML + parent plan-doc backfill). | `plugins/dev-sdlc/tests/test_no_sealed_amendments.py` invariant + manual `git diff --name-only` check at seal time. |
...
- `docs/rebuild/plans/v0-1-0-foldback-scope-expansion-fbe3.md` (this file).
- `docs/rebuild/plans/v0-1-0-foldback-scope-expansion-fbe3.manifest.yaml`.
...
- **HT-7:** `loam amend seal` complains about dirty
  `docs/rebuild/FUTURE_IDEAS_DRAFT.md` (per Surface #6). Mitigation:
...
6. **Parent plan-doc backfill** —
   `docs/rebuild/plans/v0-1-0-foldback-scope-expansion.md` §8 FBE.3
   entries get the apply + seal SHAs (separate NEW commit; admitted
   via `docs/rebuild/plans/` universal prefix).
7. **Status file** — write
...
- **Parent plan:** `docs/rebuild/plans/v0-1-0-foldback-scope-expansion.md`
...
- **FBE.1 sub-plan:**
  `docs/rebuild/plans/v0-1-0-foldback-scope-expansion-fbe1.md`.
- **FBE.2 sub-plan (closest shape):**
  `docs/rebuild/plans/v0-1-0-foldback-scope-expansion-fbe2.md`.
