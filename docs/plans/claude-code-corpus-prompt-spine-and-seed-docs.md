...
2. A **two-class corpus authoring guide** lives under
   `docs/capability-corpus/AUTHORING.md`. It defines
...
`docs/capability-corpus/` (not under
...
only) + `docs/capability-corpus/` (new docs tree —
...
- *Spine fires:* the persona checks the leverage rule;
  ...
  `docs/capability-corpus/claude-code/schedule.md`
...
- Negative-shape (no capability content under any path
  outside `docs/capability-corpus/` and the
...
`docs/capability-corpus/`; ≥ 8 entries; ≤ 1500 chars
...
matching the regex `docs/capability-corpus/.+\.md`;
...
The file `docs/capability-corpus/AUTHORING.md`
exists and carries:
...
- A *Class A-prime — pos-v2 harness primitives* section
  ...
  (sourced from `docs/archive/component-research/<name>/` rather
  than Anthropic).
...
`docs/capability-corpus/claude-code/` (and/or under
`docs/capability-corpus/harness/` for Class A-prime
...
**Test shape:** glob
`docs/capability-corpus/claude-code/*.md` +
`docs/capability-corpus/harness/*.md`; assert
...
`docs/capability-corpus/best-practice/`, each
...
**Test shape:** glob
`docs/capability-corpus/best-practice/*.md`;
...
- `docs/capability-corpus/AUTHORING.md` (the
  authoring guide),
- `docs/capability-corpus/<class-dir>/*.md` (the
  seed docs).
...
- `primary-persona/tests/` (new test files for AC.α.1,
  AC.α.6, AC.α.7),
- `docs/capability-corpus/` (new tree — authoring
  guide + seed docs),
- `docs/plans/claude-code-corpus-prompt-spine-and-seed-docs*`
...
- New operational rule *Lean on the corpus* in
  `primary-persona/templates/persona-template/prompt.md` →
  AC.α.1, AC.α.6.
- `docs/capability-corpus/AUTHORING.md` content →
  AC.α.2.
- `docs/capability-corpus/claude-code/*.md` +
  `docs/capability-corpus/harness/*.md` content →
  AC.α.3, AC.α.5.
- `docs/capability-corpus/best-practice/*.md`
...
sub-directories under `docs/capability-corpus/`:
...
- **(a) Directory layout mirrors the URI partition
  ...
  `docs/capability-corpus/<class-dir>/<name>.md`
...
2. **Scope fence — `primary-persona/` template-content +
   `docs/capability-corpus/` only.** Source under
   ...
   New tree under `docs/capability-corpus/`. No
...
8. **Authority bound.** Builder may refine: exact prose
   ...
   `docs/capability-corpus/` (provided D-OWNER.4's
...
- **MCP tool calls in α's spine.** Pre-β, the spine
  names corpus paths (`docs/capability-corpus/<class>/<name>.md`)
...
1. **Cross-component scope expansion beyond
   ...
   `docs/capability-corpus/`.** Any required
...
4. **The corpus-tree paths conflict with an existing
   ...
   `docs/capability-corpus/`; if a pre-existing
...
plan: docs/plans/claude-code-corpus-prompt-spine-and-seed-docs.md
...
narrative:
  ...
  body: |
    ...
    #    docs/capability-corpus/AUTHORING.md
...
4. Write builder-plan to
   `docs/plans/claude-code-corpus-prompt-spine-and-seed-docs.builder-plan.md`
...
7. For each selected seed Class B pattern: synthesise
   ...
   `docs/capability-corpus/best-practice/<topic>.md`
   (AC.α.4, AC.α.5).
...
- Sibling amendment #47 (`.mcp.json` writer — surface β
  later extends):
  `docs/plans/amendment-47-workspace-local-mcp-json-writer.md`
...
- **D-build.2 — AC.M.S widening for capability-corpus
  ...
  `docs/capability-corpus/` to the same fence.
