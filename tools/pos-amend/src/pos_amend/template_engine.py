"""Markdown template engine for ``pos-amend template`` (v1).
...
This module is purely additive to pos-amend (AC.D-tpl.6): no existing
...
def discover_templates(root: Path) -> list[tuple[str, str, Path]]:
    ...
    Used by ``pos-amend template list`` (AC.D-tpl.4). Sort order is
