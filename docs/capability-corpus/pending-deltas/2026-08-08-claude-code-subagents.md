...
For Bash commands, Claude Code also checks the command itself and blocks one that redirects git into the main checkout; the redirect vectors and the too-complex-to-check rule are listed under How Claude Code enforces isolation .
...
When a blocked value is a family alias such as opus , Claude Code runs the subagent on the newest version of that family the allowlist permits, following the same substitution rules and provider scope as /model .
...
Beyond subagents and teammates, in sessions where cross-session messaging is enabled, the same tool can message your other Claude Code sessions on this machine, or reply to your sessions beyond it .
