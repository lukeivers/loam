...
amendment dispatch).
**Companion plan:** `docs/rebuild/plans/workspace-sync-resolver-cost-overhaul.md`.
...
4. **Phase D — Apply + Seal.**
   4.1 Author manifest at `docs/rebuild/plans/workspace-sync-resolver-cost-overhaul.manifest.yaml` (next free amendment = 57; baseline = current HEAD pre-amendment-commit).
