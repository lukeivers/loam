...
**Parent plan:** `docs/rebuild/plans/v0-1-0-foldback-scope-expansion.md` (FBE.11 row to be backfilled in §8 register at completion; FBE.6c row at lines 820-855 is the FOLDBACK record that named BLOCKER-FBE6c.1).
...
1. `docs/rebuild/plans/oss-v0-1-0-publish-dry-run.md` AC.M11a.7 (the canonical operator instruction for the staging push).
2. `docs/rebuild/plans/v0-1-0-foldback-scope-expansion.md` §4 AC.FBE.6.4 row (the forward-AC text that future FBE.6d will follow).
...
|---|---|---|
| **AC.FBE.11.1** | `docs/rebuild/plans/oss-v0-1-0-publish-dry-run.md` AC.M11a.7 (lines 170-178) updated to instruct the dual-ref push: `git push <staging-url> framework-only:main framework-only:framework-only --force`. Verification text updated to assert BOTH refs land at the same SHA. | `grep -n "framework-only:framework-only" docs/rebuild/plans/oss-v0-1-0-publish-dry-run.md` returns ≥1 hit; the AC.M11a.7 §body names both refs in the push command + verifies both via `git ls-remote`. |
| **AC.FBE.11.2** | `docs/rebuild/plans/oss-v0-1-0-publish-dry-run.md` line 32 (M11a summary) + line 430 (D-build.M11a.10 mechanism note) updated to dual-ref push. | `git grep -n "framework-only:framework-only" docs/rebuild/plans/oss-v0-1-0-publish-dry-run.md` shows the summary + mechanism-note lines among the hits. |
| **AC.FBE.11.3** | `docs/rebuild/plans/v0-1-0-foldback-scope-expansion.md` §4 AC.FBE.6.4 row (line 366) updated to the dual-ref push command, so future FBE.6d (which mirrors AC.FBE.6.4 verbatim per FBE.6/FBE.6b/FBE.6c precedent) inherits the fix. | `grep -n "framework-only:framework-only" docs/rebuild/plans/v0-1-0-foldback-scope-expansion.md` returns ≥1 hit at line 366 (AC.FBE.6.4 row). |
...
| **AC.FBE.11.6** | Negative AC: zero edits to source under `framework/<comp>/src/` or `framework/<comp>/tests/`. | `git diff 03ef8da..SEAL_COMMIT --name-only` produces only paths under: (a) `docs/rebuild/plans/` (sub-plan + manifest + parent §8 backfill + AC.FBE.6.4 row update + dry-run plan-doc updates), (b) `framework/hands-off-lifecycle/seals/SEAL_COMMIT.v0-1-0-foldback-fbe11` (NEW narrative file), (c) `framework/hands-off-lifecycle/tests/SEAL_COMMIT` (apply step's frozen-baseline-aware sidecar bump). No edits under `framework/<comp>/src/` or `framework/<comp>/tests/`. |
...
- `docs/rebuild/plans/oss-v0-1-0-publish-dry-run.md` — update AC.M11a.7 §body (lines ~170-178), summary (line 32), D-build.M11a.10 mechanism note (line 430) for dual-ref push. **Doc-only edit; no source delta.**
- `docs/rebuild/plans/v0-1-0-foldback-scope-expansion.md` — update §4 AC.FBE.6.4 row (line 366) for dual-ref push command (the forward-AC text future FBE.6d inherits). **Doc-only edit; no source delta.**

### Plan-doc paper trail (universal-paths admission):
- `docs/rebuild/plans/v0-1-0-foldback-scope-expansion-fbe11.md` — this sub-plan (NEW).
- `docs/rebuild/plans/v0-1-0-foldback-scope-expansion-fbe11.manifest.yaml` — amendment manifest (NEW).
- `docs/rebuild/plans/v0-1-0-foldback-scope-expansion.md` §8 FBE.11 register backfill at completion.
...
- `docs/rebuild/plans/v0-1-0-foldback-scope-expansion-fbe6.md` / `-fbe6b.md` / `-fbe6c.md` — sealed-amendment historical records, not retroactively edited.
...
- `loam amend apply` — admits the universal-paths plan/seal/dry-run scope.
  - `docs/rebuild/plans/oss-v0-1-0-publish-dry-run.md` and `docs/rebuild/plans/v0-1-0-foldback-scope-expansion.md` both ride the `docs/rebuild/plans/` universal-paths prefix admission.
...
- Parent plan §8 backfill: edit `docs/rebuild/plans/v0-1-0-foldback-scope-expansion.md` §8 — add new FBE.11 row (after FBE.6c row at lines 820-855), record sub-plan/manifest/seal SHAs.
