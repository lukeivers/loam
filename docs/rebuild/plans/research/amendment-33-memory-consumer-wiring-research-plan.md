...
**Session-start corpus:** research agent reads all five mandatory paths in `CLAUDE.md`'s session-start-discipline section. Component-scoped reads: `docs/rebuild/components/memory-system/`, `docs/rebuild/components/primary-persona-layer/` (or whatever that component's current directory is), `docs/rebuild/components/scope-of-work/`, `docs/rebuild/components/orchestrator/`, `docs/rebuild/components/objective-tracker/`, `docs/rebuild/components/self-correction/`. And the three ODD docs for shape.
