...
**Companion research:** none authored separately. Master plan (`docs/plans/first-run-primary-persona-default-agent-wiring.md`) carries the investigative narrative.
...
- **v1.0 line 153 — Persona present in every interactive session** (`docs/spec/pos-v2-objectives-spec.md` §152–153): *"every interactive session (terminal or desktop) starts with the primary persona present by default — asserted by a session-start test."* A persona cannot be loaded if its directory does not exist. This amendment makes the directory exist on a fresh clone, which is the necessary precondition for v1.0 line 153 to hold without requiring the user to author files.
...
- **v1.2 addendum (proposal §1.0 mapping) — workspace without persona cannot start session** (`docs/archive/component-research/primary-persona-loader/proposal.md` §158): the loader's fail-closed today produces a hard failure on a fresh clone instead of a guided onboarding to the persona-present state. Scaffolding the starter directory closes that hazard at the workspace-bootstrap layer.
- **workspace-bootstrap proposal B16 / B25** (`docs/archive/component-research/workspace-bootstrap/proposal.md` §117 + §134): the framework-internal phase surface (`first_run_scaffold` adapter introduced by amendment #4) is the correct layer for new bootstrap-time contributions; this amendment extends that adapter without altering the phase model itself.
...
**Maps to:** v1.2 R16 framework-not-content (`docs/spec/pos-v2-objectives-spec.md` §348–356) → AC.PO.2 (toolkit purity).
...
- `docs/plans/amendment-36-workspace-bootstrap-persona-scaffold*` (this plan + manifest),
...
4. Write builder-plan to `docs/plans/amendment-36-workspace-bootstrap-persona-scaffold.builder-plan.md` naming specific files + symbols expected to be touched.
...
plan: docs/plans/amendment-36-workspace-bootstrap-persona-scaffold.md
