...
| Intake LLM intent-extraction seam #172 | SEALED-LOCAL 2026-06-01 | — | The 1.0-acceptance-smoke "loop until READY" close (sub-plan `docs/plans/intake-llm-intent-seam.md`). Owner-ratified (Telegram 13398) build of the design fork #166-#171 left open (`four-step-loop-ran` PARTIAL on all three variants). Single-component fence on `framework/workspace-bootstrap/`: NEW `intent_extract.py` (an `IntentExtractor` Protocol + a DISABLED-by-default extractor keeping the baseline distillation pure-regex + a real spawn-isolated `ClaudeIntentExtractor` — ONE bounded `claude -p` via `loam_spawn_isolation.spawn_isolated_claude`, hard timeout, JSON envelope, fail-soft to regex on ANY failure) replaces the brittle regex distillation as the PRIMARY path; leg 4 (adjust-from-the-answer) made real — a visible close adjustment turn reflecting the confirmation's detail or answering a capability doubt honestly (protection-floor). No Anthropic SDK / API key. AC.INTENT.1/.2/.3/.4 + AC.INTENT.S (outcome-altitude). Default path byte-identical regex; full wb suite 619 passed / 15 skipped (zero regressions). BASELINE `b8817c71`; source-edit `377df45c`; manifest baseline `b994540f`; apply `65f420e1`; seal `560bf5c7`. Smoke re-run (real extractor via `register_intent_extractor`, no second fence) at `docs/experiments/loam-1.0-acceptance-smoke-rerun8.md`. NOT pushed / NOT merged — owner-gated. |
| WMS Increment 3 — INTAKE (conversational work-capture) #84 | SEALED-LOCAL 2026-06-03 (stacked on `build/wms-increment-2`, unpushed) | — | Roadmap item 3 of the Work Management System (sub-plan `docs/plans/wms-increment-3-intake-conversational-work-capture.md`; architecture §4a/§7-row-3/WMS-D4). Owner-greenlit "build it all the way through" (Telegram 13704). SINGLE-component fence on `framework/primary-persona/` — a NEW keep-pace `intake.py` `TriggerKind.turn` contributor: detect a piece of work the user mentions in an ordinary turn → propose ONE plain-language work-item backed by a `proposed`-state item in the increment-2 store, at a LIGHT default (propose-and-confirm; create in `proposed`, promote to `active` on a plain-language confirm, abandon on dismiss; never silent auto-create, never re-nag). Per-user via the #34 `work-tracking`/`intake-aggressiveness` cell (`off`/`light`/`eager`, default light — WMS-D4 confirmed). Composes the #56 spawn-isolation MECHANISM with a WORK-SHAPED extractor (NOT the onboarding stop/start prompt — D-INTK.4 / RF#1; the sealed `intent_extract.py` is NOT modified) + fail-soft to NO-proposal on extractor decline; conservative high-confidence dedup (D-INTK.3, propose-when-unsure to avoid a silent false-merge); `origin: conversation` `LiftedFrom` provenance. The objective-tracker store is CONSUMED via its existing `create`/`start`/`mark_abandoned` API, NOT modified (D-INTK.1). AC.INTK.DETECT.1/.2 + PROPOSE.1/.2 + CONFIRM.1 + LIGHT.1/.2 + DEDUP.1 + outcome-altitude AC.INTK.LIVE.1 (one real work turn through the live `build_intake_contributor` turn-path + #56 seam + store `create` yields exactly ONE correctly-placed `proposed` item; a chatter turn yields ZERO). 23 new intake AC tests green; full primary-persona suite green (the pre-existing `test_AC_MSC_3` #52 FS-coupling defect resolved in-tree by provisioning the gitignored `workspace/personas/primary/contract.yaml` runtime state to match main — NOT a code edit; loam-mode untouched). BASELINE `e0afa9c1`; plan-doc anchor `3a0596b8`; source-edit `1c1de6a6`; apply `1c0108cb`; seal `d8d10c7f`. Post-seal `apply --dry-run` clean; fence diff `BASELINE..seal` touches only `framework/primary-persona/` + universal-admitted paths. NOT pushed / NOT merged — stacked on the unpushed increment-2 branch. |
| WMS Increment 4 — PRIORITIZATION (derived, transparent) + RELATIONAL WEB (surfaced) #84 | SEALED-LOCAL 2026-06-03 (stacked on `build/wms-increment-3`, unpushed) | — | Roadmap item 4 of the Work Management System (sub-plan `docs/plans/wms-increment-4-prioritization-and-relational-web.md`; architecture §4b/§4c/§7-row-4/WMS-D5). Owner-greenlit "build it all the way through" (Telegram 13704). SINGLE-component fence on `framework/primary-persona/` — two NEW keep-pace modules. (a) `prioritize.py` — a derived, TRANSPARENT, calibrate-on-use multi-signal ordering: blends FIVE signals (the existing `tracker_context` open-loop priority-key; blocking-impact off the existing edge graph; goal-alignment off `OBJECTIVES.md` subgoals; recency/staleness off the existing `last_transition_at`; explicit owner pin/defer as a HARD override above the blend) with CALIBRATE-ON-USE weights (no imported magic number, Lens-4) and a PLAIN-LANGUAGE per-item reason naming the dominant signal — never a surfaced numeric score (Lens-2 / D-PRI.4). (b) `relational.py` — a `TriggerKind.turn` lens surfacing the relational web from the EXISTING `unblocked_next`/`waiting_on_other`/`trace_to_root` queries: the PRIORITIZED unblocked-next (single-top + short tail — D-PRI.3) + reason, blocked-on-what, waiting-on-me-vs-others, and the decomposition tree — ONE concise capped fail-soft block (Slice-D discipline), registered live in the production composer beside projects/work-streams. The objective-tracker store is CONSUMED via its existing READ API (`unblocked_next`/`waiting_on_other`/`query_projection_view`/`trace_to_root` + the `priority` projection field), NOT modified (D-PRI.1) — NO edge/priority STORAGE added. The #71 edge-mutation graph self-heal is explicitly DEFERRED (D-PRI.5 / §10 RF#2): inc-4 READS FBM `derive_project_state` as a priority signal only. AC.PRI.1–5 (multi-signal ordering / responsive-to-signal-change / pin-defer-hard-override / transparent-reason-no-score / calibrate-on-use weights) + AC.REL.1–4 (unblocked-and-blocked-on-what / waiting-on-me-vs-others / decomposition / no-fabricated-relationship) + AC.SURF.1–2 (one capped fail-soft turn block / bounded per-turn TTL cost) + outcome-altitude AC.WMS4.LIVE.1 (a REAL store with a REAL dependency chain — C waits_on B, B blocks C, A independent — through the live production entry points yields the RIGHT unblocked-next item B + a transparent plain-language reason, no pre-arranged state). 31 new WMS-inc4 AC tests green; full primary-persona suite green (the pre-existing `test_AC_MSC_3` #52 FS-coupling defect resolved in-tree by provisioning the gitignored `workspace/personas/primary/contract.yaml` runtime state to match main — NOT a code edit; loam-mode untouched). BASELINE `8e29e4c7`; plan-doc anchor `3297d5f5`; source-edit `7d9c1b99`; apply `a80dd06a`; seal `1fb84d9c`. Post-seal `apply --dry-run` clean; fence diff `BASELINE..seal` touches only `framework/primary-persona/` + universal-admitted paths. NOT pushed / NOT merged — stacked on the unpushed increment-3 branch. |
| WMS Increment 6 — PER-USER LENS CHOICE (the L4 wiring) #84 | SEALED-LOCAL 2026-06-04 (stacked on `build/wms-increment-5`, unpushed) | — | Roadmap item 6 of the Work Management System (sub-plan `docs/plans/sealed/wms-increment-6-per-user-lens-choice.md`; architecture §1/§3/§6/§7-row-6/WMS-D3). The L4 chooser over the inc-5 lens SET — builds NO new lens. SINGLE-component fence on `framework/primary-persona/` — one NEW keep-pace module `lens_choice.py` (a deterministic fail-open RESOLVER reading the #34 `work-tracking`/`preferred-lens` cell via the SAME `cell_or_prior` path `intake.py` uses, mapping a recognised value to a lens-SET and fail-opening to a NON-EMPTY default derived from the EXISTING `technical-exposure` axis: plain → on-my-plate, open → work-streams; NO LLM on the hot path, never raises, never empty — D-WMS6.2/.5 + RF#3) + a WMS-area-scoped SWITCH WRITER (re-emits the #34 matrix line-shape via `render_matrix` + a locked `Cell` WITHOUT routing through `apply_override` — which rejects the `work-tracking` area, verified — and WITHOUT widening `AIM_AREAS`/the seed-writer; D-WMS6.4) + a plain-language switch handler with a zero-internal-vocab confirmation (D-WMS6.6) — plus a CHOICE-AWARE edit to the per-turn registration (`session_start_emitter.py`): it registers exactly the CHOSEN lens(es) at `TriggerKind.turn` instead of the unconditional inc-4 trio (the FBM-don't-bloat composition; D-WMS6.3 REPLACE-with-a-multi-select-set), keeping the un-chosen lenses as the inc-5 on-demand `render_*_block` entry points, and fails OPEN to the inc-4 always-on trio (never zero blocks; AC.SURFACE.4 / §8 #3). `interaction_model.py` (the #34 engine) + the objective-tracker store are CONSUMED read-only — NO #34-taxonomy mutation, NO store mutation, NO new lens. AC.CHOICE.1–4 (profile→lens-set / fail-open-to-non-empty-exposure-default / fail-open-on-matrix-error / deterministic-no-LLM-no-mutation) + AC.SURFACE.1–4 (turn-surface = chosen set / un-chosen lens still on-demand / block-count = set-size / fail-open-to-the-old-trio-not-empty) + AC.SWITCH.1–4 (plain switch changes next-turn surface / no #34-taxonomy mutation round-trips through the live reader / zero-internal-vocab confirm / explicit-pick-sticky-over-derived-default) + outcome-altitude AC.WMS6.LIVE.1 (a REAL #34 profile + REAL store, NO pre-arranged state, through the production entry points: a plain/plate profile surfaces on-my-plate and NOT the trio; a plain switch writes the cell so a fresh resolve returns the switched-to set; un-chosen lenses still render on demand). 30 new WMS-inc6 AC tests green; the inc-4 `test_AC_SURF_1` production-registration assertion updated to the choice-aware contract (D-WMS6.3 REPLACE supersedes the unconditional trio); full primary-persona suite green (the pre-existing `test_AC_MSC_3` #52 FS-coupling defect resolved in-tree by provisioning the gitignored `workspace/personas/primary-persona/contract.yaml` runtime state — NOT a code edit; loam-mode untouched). BASELINE `a236e490`; plan-doc anchor `b6f47776`; source-edit `da01078b`; apply `2ec0ecd2`; seal `91d9a6bf`; §14-backfill `eb49e3fe`. Post-seal `apply --dry-run` clean; fence diff `BASELINE..seal` touches only `framework/primary-persona/` + universal-admitted paths. NOT pushed / NOT merged — stacked on the unpushed increment-5 branch. |
...
| deliberate-reasoning slice-1 — metacognitive gate + evidence-bound re-entrant loop + pre-registered experiment | SEALED-LOCAL 2026-06-22 | — | NEW component `framework/deliberate-reasoning/` (sub-plan `docs/plans/sealed/metacognitive-gate-reentrant-loop-slice1.md`; ratified `c0c584f4`). A turn-level GATE decides per-turn whether the fast inference path answers or a DELIBERATE evidence-bound re-entrant LOOP engages — default OFF, escalating only on the three observable triggers (low_confidence/novelty/stakes per D-MGRL.4); the loop runs draft→adversarial evidence-bound critique→revise→re-check with a NO-DEGRADATION guard; a PRE-REGISTERED experiment fixes metric/task-set/blind-judge/theory-vs-generic discriminator BEFORE any scored run (AC.MGRL.4 git-ancestry). AC.MGRL.1–7 + AC.MGRL.OA (outcome-altitude, escalation through the live `process_turn` entry point with no pre-arranged state). 35 component tests green. Instrument sealed as built — the first real LLM-critic scored run is a separate follow-on (the pre-reg→scored-run SHA pair gets recorded by that run, not here). BASELINE applied at plan-doc `c0c584f4`; source-edit `01acd52d`; apply `d8b71827`; seal `a6156590`; §14-backfill `7c0f2bbb`. Two post-seal correctives for the JOB-A interleaving (below): seal-fence BASELINE bump `052e23ff` + PB-stem reword `80cd7705` + sealed-manifest baseline bump `1fdc2e02`. Post-seal `apply --dry-run` clean (manifest baseline advanced to the pre-seal tip `708a66ec` so the in-fence window excludes the interleaved dev-sdlc corrective). NOT pushed / NOT merged — owner-gated. |
| memory volatility classifier + read-side disposition #memory | SEALED-LOCAL 2026-06-28 | — | EXTENDS the sealed `primary-persona` on the already-sealed bitemporal interval machinery (sub-plan `docs/plans/memory-volatility-classifier-read-disposition.md`; research `pos3/workspace/strategy/research/memory-system-issues-2026-06-28/memory-system-research.md` §4, owner-ratified "go with your recommendations"). Write side: `classify_volatility` (deterministic, stdlib `re`) → `durable`/`volatile-hard`/`volatile-soft` on the named operational-status tells, with a D2 safe bias (a hard tell co-occurring with a durable-decision signal de-escalates to SOFT so a ruling is never hard-excluded; classifier error → `durable`, never drops); `write_episode` stamps `volatility` + (HARD) a `volatile_until = reference_time + VOLATILE_WINDOW` close, durable/soft born open. Read side: `_supersession_interval` reads `volatile_until` as an ADDITIVE interval close (supersession marker precedence; volatility close applies only to an UNMARKED record so AC.SUP.2 `as_of` windows stay byte-identical), so the sealed `_filter_by_interval` hard-excludes HARD from the default current view (live keep_pace path `_episode_hits`→`search(as_of=None)`→`_compose_score`→`_filter_by_interval`) while `as_of`-in-window still reaches it; SOFT survives but its keep_pace pointer is annotated `[VOLATILE — re-verify before serving]`. AC.VOL.1–5 incl. AC.VOL.5 outcome-altitude (real `write_episode` + `search`, no pre-arranged state). Full `primary-persona` suite green. **F2: research item #4 (recency-blend exemption) DROPPED as INERT in canonical** — `_blend_recency`/`RECENCY_BLEND_WEIGHT` are dead code (zero callers); the live `_compose_score` has no recency term; the recency adversity is a pos3-old-tree artifact the owner-gated canonical→pos3 sync already removes. SINGLE-component fence on `framework/primary-persona/`. BASELINE `a7c9f1b2`; plan `25610cbf`; source `779d306f`; manifest `f6bfb490`; apply `f9fb305c`; seal `fe7e2de2`. Post-seal `apply --dry-run` clean. **Live activation = the owner-gated canonical→pos3 sync (plan §8).** NOT pushed / NOT synced. |
| dev-sdlc corrective — pbret-register-v160-retirement-record-keeps (JOB A, cleared the seal-blocking sweep) | SEALED-LOCAL 2026-06-22 | — | Registered two PRE-EXISTING v1.6.0 completed-work records (`docs/plans/release-integration-v1-6-0.md` + `docs/state-migrations/v1-6-0-claude-leverage-and-principle-foundation.migration.yaml`) as AC.PBRET.5 retirement-sweep justified keeps (new D-K11 sub-block, the D-K9 historical-record class). Re-verified Tier-0: every retired-benchmark mention in both files DOCUMENTS the retirement (the release record lists work-slice (d) — the full retirement of the retired benchmark — as a bundled slice; the migration yaml records the deletions as a declared no-op) — historical records, NOT live usages. Per the D-K10 precedent, the plan §10 D-PBRET.6 register sealed into `docs/plans/sealed/` with the retirement plan, so the keep lives in the test's REGISTERED_KEEPS + is surfaced here, not in sealed audit history. Guard stays ENFORCING (the mutation-detection sibling still goes RED on injected strays; the assertion is not weakened). SINGLE-component fence on `plugins/dev-sdlc/`. Plan-doc `6542865f`; source-edit `161816a4`; apply `fea022da`; seal `dfda5bbf`; §14-backfill `708a66ec`. Post-seal dry-run clean; fence diff touches only `plugins/dev-sdlc/` + `docs/plans/`. NOT pushed / NOT merged. |
