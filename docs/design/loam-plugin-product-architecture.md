...
| 0g | **Deploy-safety floor** | can't-disable | destructive-action gate keyed off the resolved-target production-ness (`max(declared, resolved)`) + attestation / refuse-all-destructive default + per-gate fail-CLOSED (closes G1/G2/G15) | per-environment `deploy.yaml` read by a PreToolUse `permissionDecision: deny` (fail-CLOSED), composing on the `safety-layer` fail-policy primitive | a non-technical owner can't safely opt into dropping prod — BUILT `framework/deploy-safety-floor/` + `safety-layer` fail-policy (SEALED LOCAL 2026-06-27, seals `cf1ed11e` / `828de228`; plan `docs/plans/deploy-safety-floor-gate-primitives.md`) |
| 0h | **Secure-build baseline** | can't-disable | secrets-never-committed at the commit/push boundary + dependency-audit gate + artifact-cleanliness, for the artifact loam BUILDS (Tilth `F-SECURE-BUILD-BASELINE`) | additive `secret_pattern_guard` staged-diff branch + shell-out `npm audit` / `pip-audit` + generated `.gitignore` + pre-commit sweep | a user can't safely opt into shipping a known-CVE artifact or committing a secret — BUILT `framework/secure-build-baseline/` (SEALED LOCAL 2026-06-27, seal `3225eeee`) |
