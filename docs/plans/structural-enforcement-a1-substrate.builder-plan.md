...
**Status:** authored 2026-04-26 by build agent. **Working directory:** `/Users/lukeivers/ivers-corp-pos-v2/`. **Plan-doc that governs:** `docs/plans/structural-enforcement-a1-substrate.md` (LOCKED). **Research:** `docs/plans/research/structural-enforcement-of-critical-requirements-research.md` (LOCKED).
...
- `objective-tracker/tests/test_no_sealed_amendments.py` — existing test; BASELINE advances via `pos-amend apply` to the pre-amendment-tip SHA; seal-diff stays inside `objective-tracker/` + `docs/plans/` + universal admissions.
...
- `docs/plans/structural-enforcement-a1-substrate.builder-plan.md` — this file. (paper trail per FUTURE_IDEAS plan-before-code CDC)
- `docs/plans/structural-enforcement-a1-substrate.manifest.yaml` — pos-amend manifest. (bookkeeping)
...
`docs/plans/structural-enforcement-a1-substrate.manifest.yaml`:
...
- `universal_paths.files:` `CLAUDE.md`, `docs/odd-in-pos.md`, `docs/odd-methodology.md`, `docs/FUTURE_IDEAS.md`, `.gitignore`.
...
1. Author manifest at `docs/plans/structural-enforcement-a1-substrate.manifest.yaml`.
...
7. `pos-amend seal --plan-doc /Users/lukeivers/ivers-corp-pos-v2/docs/plans/structural-enforcement-a1-substrate.md <manifest>` — runs scoped sweep + creates seal commit + advances SEAL_COMMIT sidecars to the seal commit SHA + appends builder-plan §SHA backfill via the new-plan §14 mechanism.
