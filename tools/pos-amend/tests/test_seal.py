"""AC.D-sa.1 – AC.D-sa.7 — `pos-amend seal` finalisation extension.
...
the post-amendment-commit state. ``pos-amend seal`` is then
...
from pos_amend.commands.seal import _discover_sealed_components
...
def _make_fake_component(
    ...
    # tuple literals so `pos-amend apply --dry-run` finds the
...
def _write_manifest(
    ...
    *,
    ...
    for c in components:
        ...
        lines.append(
            f"    seal_test: {c}/tests/test_no_sealed_amendments.py"
        )
        lines.append(f"    sidecar: {c}/tests/SEAL_COMMIT")
...
def test_AC_D_sa_1_single_invocation_finalises(sealed_repo, monkeypatch) -> None:
    """`pos-amend seal <manifest>` finalises in one process: sidecar
    ...
    sidecar = (repo / "alpha" / "tests" / "SEAL_COMMIT").read_text().strip()
    assert sidecar == amendment_sha

    # (b) narrative file written
    narrative = repo / "alpha" / "seals" / "SEAL_COMMIT.fixture"
    ...
    # (e) post-seal `pos-amend apply --dry-run` exits 0
...
def test_AC_D_sa_2_commit_message_deterministic_template(sealed_repo) -> None:
    ...
    assert "alpha/tests/SEAL_COMMIT" in full_body
    # Narrative target
    assert "alpha/seals/SEAL_COMMIT.fixture" in full_body
...
def test_AC_D_sa_4_no_finalize_preserves_pre_extension_behaviour(
    ...
    status = _git(repo, "status", "--porcelain").stdout
    assert "alpha/tests/SEAL_COMMIT" in status
    narrative_file = repo / "alpha" / "seals" / "SEAL_COMMIT.fixture"
...
def test_AC_D_sa_5_failing_component_test_halts_before_commit(
    ...
    (repo / "alpha" / "tests" / "test_basic.py").write_text(
        textwrap.dedent(
            """
            def test_will_fail():
...
# ----------------------------------------------------------------------
# AC.D-sa.6 — pre-existing pos-amend tests + invocations remain green
# ----------------------------------------------------------------------


def test_AC_D_sa_6_existing_test_suite_still_green(tmp_path: Path) -> None:
    """The pre-existing pos-amend test suite — every test except this
    ...
    """
    pos_amend_root = Path(__file__).parent.parent
    tests_dir = pos_amend_root / "tests"
    proc = subprocess.run(
        [
            sys.executable,
            "-m",
    ...
    assert proc.returncode == 0, (
        "existing pos-amend tests regressed:\n"
        + proc.stdout
        + proc.stderr
    )
...
def test_AC_D_sa_6_legacy_seal_signature_idempotent(sealed_repo) -> None:
    ...
    run). Mirrors the prior `pos-amend seal` idempotency contract."""
    repo = sealed_repo
    manifest_path = _write_manifest(
        repo,
        components=["alpha"],
