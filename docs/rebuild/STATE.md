...
**Created:** 2026-04-17 16:30 CDT. **Last refresh:** 2026-05-01. **Status:** All thirteen sealed components built (Phase 1–4); amendment cycle active (currently #96). D-architecture commitment 2026-04-26; workspace-sync migration to git-shaped sync landed via #58–#75 series. **OSS publish (loam v0.1.0) work-plan activated 2026-04-29** at `docs/rebuild/plans/oss-v0-1-0-publish.md` after Idea 12's three open rulings closed: dormancy-rename pre-launch, Dev/SDLC plugin only at v1, public repo at `lukeivers/loam`. **All pre-publish blockers CLOSED 2026-05-01** (M9 substitution pass + memory-sidecar-recovery + M1c-corrective + post-M6 partition realignment + memory-pipeline-fix + fastmcp-group-ids-filter-fix); awaiting cross-session memory probe before M11 dry-run per first-principles reframe (cross-session continuity IS the ship-test, not within-session retrieval).
