...
- ``register_objectives(manifest, repo_root)`` — called from
  ``loam amend apply`` (non-dry-run); creates one tracker record per
  manifest entry, idempotently. Returns a ``RegistrationResult``.

- ``update_source_commits(manifest, repo_root, amendment_sha)`` —
  called from ``loam amend seal`` after the amendment SHA is known;
...
from loam_cli.amend.manifest import LiftedFromEntry, Manifest, ObjectiveEntry
...
loam amend has no runtime dep on workspace-bootstrap or primary-persona.
"""
    ...
    if not db_path.parent.exists():
        raise TrackerUnavailableError(
            ...
            detail=(
                ...
                "invoking `loam amend apply` against a manifest that "
