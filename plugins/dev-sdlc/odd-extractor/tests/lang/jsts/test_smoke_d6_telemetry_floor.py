...
- Run a full extraction against the JsTs fixture.
- Assert audit log has expected event_kind sequence:
  ``extraction_start`` (1) + ``stage_complete`` (4) +
  ``extraction_end`` (1) at minimum.
- Schema version preserved at 1; filenames monotonic NNNN.yaml.

Cycle 4a inherits Cycle 1's audit-log primitives + Cycle 3's
``slice_complete`` / ``recognizer_finding`` event_kinds. No new
event_kinds in Cycle 4a.
"""
...
def test_audit_log_has_expected_event_kinds(
    ...
    try:
        ...
        workspace.mkdir()
        ext_dir, _ = _run_full_extraction(
            jsts_playwright_app_repo, workspace
        )
