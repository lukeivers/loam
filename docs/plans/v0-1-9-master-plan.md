...
**Parent plan:** `docs/plans/eric-final-delivery-plan-2026-05-04.md` (§2 v0.1.9 row).
**Companion research (load-bearing):**

- `docs/plans/eric-saas-app-use-case-version-sequence-2026-05-04.md` (Eric path; §3 G2 PR safety gate + G6 confidence-banded contract + G8 provenance-traceable PR template).
- `docs/plans/layered-skill-story-research-2026-05-04.md` (skills path; §5 — 12 candidate SKILLs; v0.1.9 second-pass list named at §5 first-paragraph list at lines 506–511).
- `docs/plans/v0-1-8-master-plan.md` (precedent for cycle-decomposition pattern + per-cycle dispatch-brief shape).
...
- `docs/FUTURE_IDEAS_DRAFT.md` (line 143 — `KNOWN_CROSS_MODE_DEBT` allowlist drift FIDRAFT entry; graduates here).
...
| Item | Source | Placement |
|---|---|---|
| `KNOWN_CROSS_MODE_DEBT` 4-entry shrink (remove primary-persona/templates/persona-template/prompt.md → FUTURE_IDEAS_DRAFT.md entry; remove 3 workspace-sync/README.md → docs/plans/workspace-sync.* entries) | FIDRAFT line 143 (graduated) | `plugins/dev-sdlc/tools/loam-mode/tests/test_partition_references.py` |
...
- `framework/primary-persona/templates/persona-template/prompt.md` ↛ `docs/FUTURE_IDEAS_DRAFT.md` — **0 matches** (stale; the source file no longer references the target).
- `framework/workspace-sync/README.md` ↛ `docs/plans/workspace-sync.builder-plan.md` — **0 matches** (stale).
- `framework/workspace-sync/README.md` ↛ `docs/plans/workspace-sync.manifest.yaml` — **0 matches** (stale).
- `framework/workspace-sync/README.md` ↛ `docs/plans/workspace-sync.md` — **0 matches** (stale; the README references `docs/components/workspace-sync.md` — a different path that's not in the dev-only set).

The fifth entry (`framework/memory-system/launchd/README.md` → `docs/archive/component-research/true-first-run/research.md`) IS still present — confirmed at `framework/memory-system/launchd/README.md:52`. This entry stays in the allowlist; resolution is a future memory-system amendment per the original FIDRAFT entry at line 64 of FUTURE_IDEAS_DRAFT.md. Cycle 3's edit is strictly the four-stale shrink — no fence-opening, no memory-system touch.
...
- Master plan: `docs/plans/v0-1-9-master-plan.md` — §3 Cycle 1 scope.
- Eric synthesis: `docs/plans/eric-final-delivery-plan-2026-05-04.md` — §2 v0.1.9 row + Decisions I/P/Q/R.
...
Author at: `docs/plans/v0-1-9-cycle-1-pr-safety-gate-engine.md`
Manifest at: `docs/plans/v0-1-9-cycle-1-pr-safety-gate-engine.manifest.yaml`
...
- Backfill `docs/plans/v0-1-x-roadmap.md` §8 method-decision register row for v0.1.9 Cycle 1.
- Backfill `docs/plans/eric-final-delivery-plan-2026-05-04.md` §2 v0.1.9 progress notes after seal.
- Backfill `docs/plans/v0-1-9-master-plan.md` §9 method-decision register row.
...
- Master plan: `docs/plans/v0-1-9-master-plan.md` — §3 Cycle 2 scope.
...
Author at: `docs/plans/v0-1-9-cycle-2-installers-templates.md`
Manifest at: `docs/plans/v0-1-9-cycle-2-installers-templates.manifest.yaml`
...
- Master plan: `docs/plans/v0-1-9-master-plan.md` — §3 Cycle 3 scope.
...
Author at: `docs/plans/v0-1-9-cycle-3-dev-sdlc-skills-pass-2-and-audit-cleanup.md`
Manifest at: `docs/plans/v0-1-9-cycle-3-dev-sdlc-skills-pass-2-and-audit-cleanup.manifest.yaml`
...
- **Master plan source authority:** `docs/plans/eric-final-delivery-plan-2026-05-04.md` §2 v0.1.9 row + Decisions I (PLAUSIBLE→VERIFIED default-no) + P (SOC-2 floor) + Q (one-question-at-a-time) + R (HARD/SOFT smoke gate cadence).
- **Layered-skills second-pass list:** `docs/plans/layered-skill-story-research-2026-05-04.md` §5 (12 candidates; second-pass list at lines 506–511).
- **v0.1.8 master plan precedent (cycle-decomposition pattern + per-cycle dispatch-brief shape):** `docs/plans/v0-1-8-master-plan.md` (sealed at `1c2c478`).
...
- **FIDRAFT graduate (audit-allowlist drift):** `docs/FUTURE_IDEAS_DRAFT.md` line 143 (entry captured 2026-05-04 during v0.1.8 Cycle 5 release-level smoke).
...
| Cycle 3 — 6 dev-sdlc SKILLs second pass + audit-allowlist cleanup | SHIPPED | `6378cc5` | `3284087` | Single-component fence on `plugins/dev-sdlc/`. Six SKILL.md packages (`seal-narrative-writer`, `plan-docs-author`, `hook-violation-recovery`, `component-scaffold-author`, `graceful-fallthrough-with-detection`, `loam-amend-status-quick`); each body covers the FULL ritual (6-section shape: What captures / When to use / How persona applies it / Graceful degradation / Composition / Out of scope). 4-line `KNOWN_CROSS_MODE_DEBT` shrink (5 → 1 entry — 1× primary-persona prompt + 3× workspace-sync README graduated; memory-system/launchd entry stays as still-valid pending-debt); FIDRAFT line 143 closes. Tightened `test_AC_SKILLS_DSDLC1_7`'s exact-equality check to subset check per `feedback_loose_AC_text_fix_AC_not_implementation`; canonical orphan/misnamed check moves to AC.SKILLS-DSDLC2.7. Plan-doc `98468ca`; source-edit `d8e3f01`; apply `6378cc5`; seal `3284087`; §14 backfill `642097b`. AC.SKILLS-DSDLC2.1..8 + AC.AUDIT-CLEANUP.1..3 satisfied (11 ACs); 1013 dev-sdlc tests green (501 inherited + 7 new + recovered manifest-validation + cascade tests). |
| **v0.1.9 release** | SHIPPED (local) | — | tag SHA TBD | SOFT smoke gate per Decision R; quality-bar-non-negotiable applies. All 12 dev-sdlc SKILLs + 8 base loam-skills = 20 SKILLs auto-discoverable; PR-safety gate engine + override workflow + hook installers + 3 CI templates + provenance-traceable PR description + audit-allowlist cleanup all sealed. v0.1.9 sits as a local release; tag NOT pushed until Luke gates. |
