...
**Parent plan:** `docs/plans/v0-1-0-foldback-scope-expansion.md` (FBE.4 row in §1 + AC ladder in §4 + §8 register).
...
**Binds to:**
- **AC.PO.1 + AC.PO.2** (prime objective per `docs/VALUE_PROPOSITION.md`) — closing the "stranger clones the repo and `pip install -e workspace-bootstrap` hits PyPI 404 on `loam-orchestrator`" failure mode that the M11a-3 reviewer flagged as BLOCKER 3.
...
- `docs/plans/v0-1-0-foldback-scope-expansion-fbe4.md` (this file).
- `docs/plans/v0-1-0-foldback-scope-expansion-fbe4.manifest.yaml`.

### Parent plan-doc backfill (post-seal, separate commit):

- `docs/plans/v0-1-0-foldback-scope-expansion.md` — §8 method-decision register: replace `### FBE.4` placeholder with apply commit SHA + seal commit SHA + verification summary.
...
3. **Manifest commit** — author `docs/plans/v0-1-0-foldback-scope-expansion-fbe4.manifest.yaml`.
...
7. **Parent plan-doc backfill** — `docs/plans/v0-1-0-foldback-scope-expansion.md` §8 backfill `### FBE.4` subsection with apply + seal SHAs (separate NEW commit; admitted via `docs/plans/` universal prefix).
...
9. **Pop the pre-build stash** — `git stash pop` to restore the dirty `docs/FUTURE_IDEAS_DRAFT.md` per FBE.{2,3,7} pattern.
...
- **Parent plan:** `docs/plans/v0-1-0-foldback-scope-expansion.md` (§4 FBE.4 row tightened by this sub-plan).
...
- **FBE.7 sub-plan / manifest YAML (shape precedent for fence-three sidecar-bump-only amendment style):** `docs/plans/v0-1-0-foldback-scope-expansion-fbe7.{md,manifest.yaml}` — though FBE.7 has source edits, its narrative-target convention + universal_paths.files admission shape is the model.
- **FBE.3 sub-plan / manifest YAML (shape precedent for partition-manifest edit):** `docs/plans/v0-1-0-foldback-scope-expansion-fbe3.{md,manifest.yaml}` — FBE.3 admits the partition-manifest edit via the partition component's sidecar.
