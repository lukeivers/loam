...
**Amends:** `cost-governance/tests/test_throttle_warning.py` — the file named in `docs/archive/component-research/cost-governance/proposal.md` §6 as the home for C14's tests.
...
**Secondary surfaces:**
- `cost-governance/tests/test_no_sealed_amendments.py` — advance `BASELINE` (open the amendment window to just this amendment) and extend `allowed_prefixes` to admit `docs/plans/` (plan-before-code paper trail lives there, same pattern as memory-system and orchestrator seal tests).
...
- `docs/plans/cost-governance-c14-timing-test.md` — this plan.

**Not touched:**
- `cost-governance/src/` — zero implementation changes. This is a test-coverage re-extension.
- `docs/archive/component-research/cost-governance/proposal.md` — the C14 criterion text already declares the pre-write ordering and fire-once semantics; the test gap is at the test surface, not in the proposal. No proposal edit required.
...
2. `cost-governance/tests/test_no_sealed_amendments.py`
   ...
   - Extend `allowed_prefixes` with `"docs/plans/"` (plan-before-code paper trail).
...
7. `docs/plans/cost-governance-c14-timing-test.md` — this plan.
