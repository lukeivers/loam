...
**Parent plan:** `docs/rebuild/plans/v0-1-0-foldback-scope-expansion.md` (FBE.10 row to be backfilled in §8 register at completion).
...
**Binds to:**
- **AC.PO.1 + AC.PO.2** (prime objective per `docs/rebuild/VALUE_PROPOSITION.md`) — the documented install flow must produce a working stranger experience: clone, install, init, claude. Pre-FBE.10, the README's literal commands (post-FBE.9 `loam init <path>` from inside the cloned tree) fail at the very first step.
...
### 6.3 Plan-doc + manifest (universal_paths.prefixes: `docs/rebuild/plans/`)

- `docs/rebuild/plans/v0-1-0-foldback-scope-expansion-fbe10.md` (this file, NEW commit).
- `docs/rebuild/plans/v0-1-0-foldback-scope-expansion-fbe10.manifest.yaml` (NEW commit).

### 6.4 Parent plan-doc backfill (post-seal, separate commit)

- `docs/rebuild/plans/v0-1-0-foldback-scope-expansion.md` §8 — ADD a new `### FBE.10 — close BLOCKER-FBE9.1 (workspace-bootstrap local-path clones must materialise framework-only)` subsection with apply commit SHA + seal commit SHA + AC surface + verification summary; update the closing sequence narrative.
...
- 1 sealed-component fence (`framework/workspace-bootstrap/`) + universal-paths admission for `docs/rebuild/plans/` + `framework/hands-off-lifecycle/seals/` (HOL narrative anchor).
...
4. **Manifest commit** — author `docs/rebuild/plans/v0-1-0-foldback-scope-expansion-fbe10.manifest.yaml` (1 fence component: workspace-bootstrap; HOL narrative-only via universal admission).
...
9. **Parent plan-doc backfill** — `docs/rebuild/plans/v0-1-0-foldback-scope-expansion.md` §8 add `### FBE.10` subsection with apply + seal SHAs (separate NEW commit; admitted via `docs/rebuild/plans/` universal prefix).
...
- **FBE.9 sub-plan:** `docs/rebuild/plans/v0-1-0-foldback-scope-expansion-fbe9.md`.
...
- **Parent plan:** `docs/rebuild/plans/v0-1-0-foldback-scope-expansion.md` §8 register FBE.9 row + closing sequence.
