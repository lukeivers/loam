...
`git diff --name-only BASELINE..SEAL_COMMIT` shows changes only under `memory-system/`, `workspace-bootstrap/`, `hands-off-lifecycle/`, and universal-paths (`docs/plans/`, `CLAUDE.md` if methodology shifted, `docs/odd-*.md` if methodology shifted). No orchestrator, primary-persona, scope-of-work, objective-tracker, observability-aggregator, self-upgrade, safety-layer, reversibility-primitive, cost-governance, self-correction, or telegram-interface source or test changes.
...
1. Build-agent reads session-start corpus: `docs/odd-methodology.md`, `docs/odd-in-pos.md`, `docs/VALUE_PROPOSITION.md`, `docs/STATE.md`, `docs/FUTURE_IDEAS.md`, this plan, and the research doc.
2. Build-agent writes its own **build plan** to `docs/plans/amendment-29-per-workspace-memory-sidecar-port.builder-plan.md` naming the specific files + symbols it expects to touch, before any code edit. (per plan-before-code CDC)
...
Universal paths: `docs/plans/` (prefix), `CLAUDE.md`, `docs/odd-in-pos.md`, `docs/odd-methodology.md`, `docs/FUTURE_IDEAS.md` (files).
