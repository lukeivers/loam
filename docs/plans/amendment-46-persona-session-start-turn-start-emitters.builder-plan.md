...
**Authoritative scope:** `docs/plans/memory-into-context-integration.md` §4a — AC46.1 through AC46.S.
...
| AC46.S | Seal-diff confined to `primary-persona/`, `hands-off-lifecycle/`, `docs/plans/` | existing `primary-persona/tests/test_no_sealed_amendments.py` + `hands-off-lifecycle/tests/test_cross_cutting.py` H19 |
...
| 6. Sealed-component outside fence appears in diff | DOES NOT FIRE | Diff is confined to `primary-persona/` + `hands-off-lifecycle/` + `docs/plans/`. Verified via `pos-amend apply --dry-run`. |
