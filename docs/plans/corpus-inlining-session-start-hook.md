...
**Research artefact:** `docs/plans/research/corpus-inlining-session-start-hook-research.md` (locked 2026-04-28; governs the inline-strategy + token-budget findings).
...
- **AC.CI.S — Seal-diff confined to fence.** The seal-diff window contains only edits under `hands-off-lifecycle/{hooks,tests,seals}/` plus the universal-paths admissions (`docs/plans/`, `CLAUDE.md` if a README pointer is added, `docs/odd-methodology.md` is read-only, `docs/FUTURE_IDEAS.md` for the FIDRAFT graduation). `hands-off-lifecycle` is H19-frozen-baseline-aware per amendment #23 — `frozen_baseline: true`. No edits outside the fence.
...
If owner rules **D-CI.1.(a) lean** → always-load tier = {`CLAUDE.md`, `docs/VALUE_PROPOSITION.md`, `docs/STATE.md`}.
...
- **Plan-doc:** this file at `docs/plans/corpus-inlining-session-start-hook.md`.
- **Research artefact:** `docs/plans/research/corpus-inlining-session-start-hook-research.md` (locked 2026-04-28; governs).
- **Builder plan:** to be authored by the build agent post-owner-approval at `docs/plans/corpus-inlining-session-start-hook.builder-plan.md`. Contains files-touched, symbol-level details, AC-to-test mapping, D-build choices applied to owner D-CI rulings, §2.5 reverse-direction audit, halt-trigger checks, pos-amend bookkeeping flow.
- **Manifest:** to be authored alongside the builder plan at `docs/plans/corpus-inlining-session-start-hook.manifest.yaml`. Single-component manifest (`hands-off-lifecycle`), `frozen_baseline: true` (H19 frozen per amendment #23). Universal-paths block as standard (`docs/plans/`, `CLAUDE.md`, `docs/odd-methodology.md`, `docs/odd-in-pos.md`, `docs/FUTURE_IDEAS.md`).
- **Pos-amend bookkeeping flow** (per `feedback_dispatch_explicit_pos_amend_apply`):
  1. Author manifest at `docs/plans/corpus-inlining-session-start-hook.manifest.yaml` with the correct BASELINE (HEAD~1 of the upcoming amendment commit per the established amendment pattern).
  ...
  5. `pos-amend seal --plan-doc /Users/lukeivers/ivers-corp-pos-v2/docs/plans/corpus-inlining-session-start-hook.builder-plan.md <manifest>` — runs the scoped test sweep, creates the seal commit, advances SEAL_COMMIT to the seal commit, appends builder-plan §SHA backfill follow-up commit.
...
- **FIDRAFT graduation:** the FIDRAFT entry "SessionStart corpus_gate should inline corpus content" graduates to a sealed amendment via this work. The graduation entry update lands in `docs/FUTURE_IDEAS.md` (graduated-from-DRAFT entry) at seal time.
