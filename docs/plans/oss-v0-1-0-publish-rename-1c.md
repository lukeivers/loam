...
**Authority documents:**
- `docs/plans/loam-rename-decisions.md` Tier-1 item 4 (launchd label rebrand; version suffix dropped concurrently).
...
- `docs/plans/oss-v0-1-0-publish.md` §5 (programme master plan; M1c row already present per M1b precursor commit `7be713b`).
...
- `com.pos-v2.*` references inside historical plan-docs at `docs/plans/*` — historical method-record; preserved (consistent with M1a + M1b's same exclusion).
...
- `docs/plans/*.md` historical method-record (preserved consistent with M1a + M1b).
...
- **Historical plan-docs stay.** `docs/plans/*.md` files not authored by this amendment (including M1a + M1b sub-plans + their manifest YAMLs + the orphan-plist-cleanup-tool plan-doc) are preserved.
...
3. **M1c sub-plan + manifest commit.** This plan-doc + a manifest YAML at `docs/plans/oss-v0-1-0-publish-rename-1c.manifest.yaml` per the established M1a/M1b-precedent shape.
...
- **M1d** (OTel `pos.*` span/event roots → `loam.*`) cleared to dispatch. Dispatcher should author `docs/plans/oss-v0-1-0-publish-rename-1d.md`.
...
- **Authority documents (inherited from series master):**
  - `docs/plans/loam-rename-decisions.md` Tier-1 item 4.
  - `.scratch/claude-output/loam-rename-migration-plan.md` §3.4.
- **Programme master plan:** `docs/plans/oss-v0-1-0-publish.md` (M1c row already in §5 per M1b precursor commit `7be713b`).
...
- **Precedent multi-component sealed-amendment manifests:**
  - `docs/plans/oss-v0-1-0-publish-rename-1b.manifest.yaml` (M1b sibling — establishes the manifest shape under the rename series; eleven-component fence).
