...
- Plan-doc: `docs/plans/v0-1-8-cycle-5-dev-sdlc-skills.md` (this file).
- Manifest: `docs/plans/v0-1-8-cycle-5-dev-sdlc-skills.manifest.yaml` (schema v3 — `plan_doc_ref` + `ac_count` + `smoke_outcome`).
...
**Decision F:** No edits to `odd-methodology.md` required — Cycle 5 ships SKILLs, not extractor / methodology changes. Universal admissions reduced to `docs/plans/` only.
...
- Universal admissions: `docs/plans/` only.
...
| Universal admissions | `docs/plans/` only | No `odd-methodology.md` edit needed (Cycle 5 ships SKILLs, not methodology); no `STATE.md` edit at cycle-time (release-level rollup happens post-smoke-gate). Minimum fence per ODD discipline. |
