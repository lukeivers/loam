...
**Parent plan:** `docs/plans/v0-1-0-foldback-scope-expansion.md` (FBE.2c row to be backfilled in §8 register; closing "Remaining sequence:" line currently lists FBE.5b only — re-shape post-seal).
...
3. **Manifest commit** — author `docs/plans/v0-1-0-foldback-scope-expansion-fbe2c.manifest.yaml` (single component in `components:` block: workspace-bootstrap).
...
7. **Parent plan-doc backfill** — `docs/plans/v0-1-0-foldback-scope-expansion.md` §8 add `### FBE.2c` subsection with apply + seal SHAs; update closing "Remaining sequence:" line (separate NEW commit; admitted via `docs/plans/` universal prefix).
...
- **Parent plan:** `docs/plans/v0-1-0-foldback-scope-expansion.md` §8 (FBE.2c row to be added at backfill time).
...
- **FBE.2b plan + status (the synth-shape change that produced the doubling):** `docs/plans/v0-1-0-foldback-scope-expansion-fbe2b.md` + `<workspace>/.scratch/claude-output/fbe2b-status-2026-05-03.md`.
