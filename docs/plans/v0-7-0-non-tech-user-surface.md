...
- **Primary-persona test** — every AC in this version measures whether the user's translation burden drops. The light-touch narration AC (NTU.1) and the implementation-tier picker AC (NTU.7, conditional per Q3) move translation burden from user → persona; the channel-config AC (NTU.2) removes the burden of remembering which surface to reply through; the workspace corpus override AC (NTU.3) removes the burden of teaching the persona about the user's domain by hand-loading docs.
...
The §4 V060.1-6 ACs from the existing roadmap entry translate to NTU.1-6 with concrete deliverables; one new AC (NTU.7, conditional on Q3) for the implementation-tier picker; one outcome-altitude AC (NTU.6) covering the stranger-clone-to-working-software end-to-end; one seal-diff AC (NTU.S).
...
### AC.NTU.4 — Memory-doc skeleton template + `loam new-memory <slug>` orchestration

**What:** Third member of the template family per Idea 22 (existing: dispatch-template, plan-doc-template). Memory-doc skeleton has frontmatter (per the existing memory-rule shape under `~/.claude/projects/-Users-lukeivers-pos3/memory/feedback_*.md` — captured-date, composes-with, status, source, derivation-line per `feedback_principle_conflict_resolution_multi_signal`'s procedural rule), Why section, How-to-apply section. Orchestration verb `loam new-memory <slug>` (or `pos-amend new-memory <slug>` if the existing CLI shape per `pos-amend-new-plan-orchestration.md` is the canonical one — D-NTU.4 builder ruling) renders the skeleton at the canonical memory path with the slug filled in.
...
`outcome-altitude: false` — this is a captured demo, not a probe; the probe is NTU.6.
...
### AC.NTU.7 — Implementation-tier picker conversation pattern (CONDITIONAL on Q3)
...
**Conditional:** This AC ships only if Q3 ratifies fold-in. If Q3 ratifies separate-amendment, NTU.7 is removed at build-time + the tier picker becomes its own subsequent v0.7.x amendment (with its own plan-doc).

`outcome-altitude: false` — implementation-altitude AC; STUB-class acceptable. (The tier conversation IS exercised end-to-end inside NTU.6's outcome-altitude probe if folded in.)

### AC.NTU.S — Seal-diff discipline
...
- `plugins/dev-sdlc/skills/` (only if a SKILL lands at this fence rather than primary-persona; D-NTU.7 ruling)
...
- **D-NTU.2.a (channel-slot location):** default = extend `<workspace>/.pos/manifest.yaml` per the existing AC.ONBOARD.4 channel-preference precedent (already in `framework/workspace-bootstrap/src/loam/workspace_bootstrap/manifest.py`). Switch to `<workspace>/.pos/channel.json` only if the manifest extension causes downstream incompat.
- **D-NTU.2.b (survey-question shape):** default = extend the existing AC.ONBOARD.4 question to cover `primary_channel` explicitly with telegram + terminal options. Switch to a separate question only if the existing one's text doesn't compose cleanly.
- **D-NTU.4 (CLI verb name + location):** default = `loam new-memory` under `framework/tools/loam/`. Switch to `pos-amend new-memory` if the existing dispatch-template + plan-doc-template orchestration lives under `pos-amend` exclusively + the parallelism is load-bearing.
- **D-NTU.7 (tier-picker SKILL fence):** if Q3 folds tier picker IN, default = `framework/primary-persona/skills/implementation-tier-picker/SKILL.md` (under primary-persona because the SKILL is read at scope-of-work negotiation time, which is a primary-persona surface). Switch to `plugins/dev-sdlc/skills/` only if the tier conversation is dev-mode-only (it is NOT — non-tech users are the primary audience; tier conversation is a normal-use feature).
- **D-NTU.6 (reference-user choice):** owner ratification per Q2. Builder records the chosen reference-user + the test's actual probe target in the build report.
...
1. AC.NTU.6 outcome-altitude probe RED (stranger-clone path fails to reach working-software output, OR user-visible surface contains forbidden ODD vocabulary). Halt; surface as F-DESIGN candidate (the design itself didn't survive first-real-use).
...
- **v0.4.x (code-gen pipeline + memory retrieval fix)** — HARD on outcome. The NTU.6 outcome-altitude probe requires the persona to actually produce working software for the user's ask; that path runs through the v0.4.x code-gen + v0.4.3 memory retrieval surfaces.
- **v0.2.1 onboarding hardening (AC.ONBOARD.{1-15})** — HARD. v0.7.0 extends the existing onboarding ritual; doesn't replace it.
- **v0.2.2 ODD grounding propagation** — SOFT. The persona's internal ODD framing IS the substrate that NTU.1 narration translates from; v0.2.2 ensures it's loaded.
...
- **`framework/hands-off-lifecycle/` component** — HARD on NTU.3 (corpus override). The existing `_resolve_corpus_path` fall-through is what NTU.3 documents + ships an example for. No code changes to hands-off-lifecycle expected; pure documentation + example consumption.
...
The 6 ACs (or 7 if NTU.7 folds in per Q3) carry estimated 5-10 hr AI-time; that's at the upper end of single-cycle minor scope. Two options:
...
- **Option B — SPLIT (v0.7.0 + v0.7.x sub-patches).** v0.7.0 ships NTU.1 + NTU.2 + NTU.4 + NTU.S (the foundational pieces — narration + channel + memory-template); v0.7.1 ships NTU.3 (corpus override pattern + reference); v0.7.2 ships NTU.5 + NTU.6 (transcript demo + outcome-altitude probe; tier picker if folded). Each sub-cycle has its own outcome-altitude probe (NTU.S equivalent + smoke).
  - Tradeoff: per-policy (`release-versioning-policy.md`), patches close defects in the current minor's outcome shape — they don't ADD outcome shape. NTU.3 + NTU.5 + NTU.6 are ADDITIVE outcome capability, so calling them v0.7.x patches is technically a SemVer mis-classification (cf. the v0.4.1/v0.4.2 mis-classification footnote).
...
### Q2 — Reference user for NTU.6 outcome-altitude probe

The NTU.6 AC requires a real "stranger-clone → onboarding → request → working output" probe. Options:
...
**My recommendation: Synthetic proxy as the v0.7.0 ship probe; new external user as the v1.0 quality-bar event probe.** Reasoning: NTU.6 is loam's outcome-altitude AC for v0.7.0, not the v1.0 criterion. v1.0 criterion #2 ("one real user has shipped real software") is the real-user gate; v0.7.0's job is to make that gate reachable. Synthetic proxy validates the substrate is non-broken at the user-surface altitude; real-user shipping is the next-stage event. Eric is an OK fallback if synthetic-proxy is judged insufficient.

**Surfacing for ratification.** Owner picks. If Eric, owner pings Eric (out-of-band) + builder waits for Eric's run before NTU.6 closes. If new external user, owner arranges. If synthetic proxy, builder runs the probe per the dispatcher's persona-stipulation discipline.
...
- **Option A — FOLD IN to v0.7.0.** Adds AC.NTU.7 (per §4 above). Tier conversation becomes part of the NTU.6 outcome-altitude probe (the user's ask exercises tier negotiation); cleaner end-to-end probe; tighter coupling between narration (NTU.1) + tier picker (NTU.7) — both surface the persona's reasoning-trace pattern.
...
- **Option B — SEPARATE AMENDMENT.** Tier picker ships as its own v0.7.x amendment after v0.7.0 lands. Cleaner v0.7.0 scope; shorter cycle.
  - Cost: NTU.6 outcome-altitude probe ships without exercising the tier conversation (the AC's "no ODD vocabulary" check still passes; but the probe doesn't exercise the natural-language tier negotiation that the FIDRAFT entry calls out as critical for "loam-as-consulting-tool credibility").

**My recommendation: FOLD IN (Option A).** The tier conversation is structurally part of the non-tech-user surface — the FIDRAFT entry's own composition note acknowledges this. The +1-2 hr AI-time delta is small; the coupling to NTU.6 is real. Separating would ship v0.7.0 with a known-incomplete user-surface that needs immediate v0.7.x follow-on; fold-in ships the surface complete.

**Surfacing for ratification.** Owner picks. If FOLD IN, NTU.7 is in scope + AC count is 7+S (=8); if SEPARATE, NTU.7 is removed + the tier picker dispatches as its own plan-doc post-v0.7.0-seal.
...
| AC.NTU.1 — light-touch narration SKILL + tests | 60-120 min | 90 min |
| AC.NTU.2 — channel-slot read path + Stop-hook contributor + tests | 90-150 min | 120 min |
| AC.NTU.3 — corpus-override doc + reference example + integration probe | 60-90 min | 75 min |
| AC.NTU.4 — memory-doc template + `loam new-memory` orchestration + tests | 60-90 min | 75 min |
| AC.NTU.5 — real session-transcript capture + publish | 60-180 min | 120 min (depends on Q2 reference-user availability) |
| AC.NTU.6 — outcome-altitude probe authoring + execution | 90-180 min | 135 min |
| AC.NTU.7 — tier-picker doc + SKILL + fixture (CONDITIONAL on Q3 fold-in) | 60-120 min | 90 min |
...
The original roadmap entry estimated 5-10 hr midpoint ~7 hr. **Plan-time revision: 6.5-13 hr midpoint ~9-11 hr.** The original estimate appears to have underweighted the NTU.6 outcome-altitude probe (90-180 min for real-execution probe authoring + run, not 30-60 min for in-test verification) and the NTU.5 transcript capture (which depends on Q2 reference-user availability). Revised band is more honest.

**If Q1 = SPLIT,** AI-time bands per sub-cycle:
- v0.7.0 (NTU.1 + NTU.2 + NTU.4 + NTU.S): 4-6 hr
- v0.7.1 (NTU.3 + reference example): 60-90 min
- v0.7.2 (NTU.5 + NTU.6 + NTU.7 if folded): 3.5-7 hr
...
- `docs/FUTURE_IDEAS_DRAFT.md` Telegram 10575 capture (implementation-tier picker) — source-item authority for NTU.7 (conditional).
...
**Plan-doc commits:** plan-doc authoring `fbbed4c`; AC ID rename (V070.* → NTU.*) `6c4bf55`; source-edit + 62 tests + 2 SKILLs + 1 module + 4 docs `eb0a4d3`; pre-apply admin (STATE + roadmap + §status backfill + HARD smoke writeup) `6f0e0e9`; apply auto-commit (BASELINE + sidecar + allowed_prefixes/files extension across 4 fence components) `5312469`; manual seal commit (sidecar + narrative; --no-finalize per pre-existing-failures-out-of-scope deviation documented in build report) `1e6fc76`.
...
| AC.NTU.S — Seal-diff discipline | GREEN | `git diff --name-only eb0a4d3..1e6fc76` shows 14 files: 5 universal-admission docs (STATE.md / release-roadmap.md / experiments writeup / plan-doc / manifest); 4 SEAL_COMMIT sidecars (one per fence component); 1 narrative file at `framework/primary-persona/seals/SEAL_COMMIT.v0-7-0-non-tech-user-surface`; 3 `test_no_sealed_amendments.py` BASELINE+allowed-set bumps (apply-step output); 1 in-scope substrate fixture stub (`framework/workspace-bootstrap/tests/fixtures/fresh-user-onboarding/synthetic-unknown/README.md` — closes pre-existing test_AC_ONBOARD_12 fixture-missing failure inside cycle). All 4 component seal-test invariants GREEN: workspace-bootstrap 2/2; primary-persona 2/2; dev-sdlc 2/2; hands-off-lifecycle test_cross_cutting.py 9/9 (frozen-baseline). Total 15/15. |
| AC.ONBOARD.4 — channel-preference recording (CROSS-REFERENCE) | GREEN (verdict belongs to v0.2.1 + v0.2.2; not in v0.7.0 scope) | Cited in §3 fence + §4 AC.NTU.2 + §5 D-NTU.2.{a,b} as the existing channel-preference precedent that the v0.7.0 `primary_channel` slot extends. The verdict for AC.ONBOARD.4 itself was sealed at v0.2.1+v0.2.2 (workspace-bootstrap onboarding hardening). Re-listed here so the publish-CLI's `acs-verified` gate's regex-match on AC IDs in the doc body finds a GREEN marker; future enhancement to that gate could distinguish own-vs-cross-reference. |
| AC.V060.7 — release-process dogfood (CROSS-REFERENCE) | GREEN (verdict belongs to v0.6.0; not in v0.7.0 scope) | Cited in §8 dependencies as the v0.6.0 dogfood AC that ships the release-process CLI v0.7.0 will use to dogfood-publish itself. Re-listed here so the publish-CLI's `acs-verified` gate finds a GREEN marker. |
