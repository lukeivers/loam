...
**Status:** research-plan for the D4 amendment cycle. Authored 2026-04-24 in the pos3 session that surfaced the underlying defect. Expected next artefact: a research doc at `docs/plans/research/memory-system-env-scrubber-research.md` answering the questions below, from which a plan-to-amend is authored.

**Session-start corpus:** research agent reads the five mandatory paths in `CLAUDE.md`'s session-start-discipline section before answering any question below. Component-scoped reads: `docs/archive/component-research/memory-system/`.
