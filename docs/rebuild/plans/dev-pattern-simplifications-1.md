...
**Source authority.** `docs/rebuild/plans/cost-audit-loam-dev-pattern-2026-05-04.md` recommendations A, D, E (§3.2, §3.1-partial, §3.4) + §5 priorities 1, 4, 5.
...
- **AC.DPS1.13** — All 106 existing manifest YAMLs (schema 1 + schema 2) under `docs/rebuild/plans/` continue to validate clean post-amendment (`loam amend validate <path>` returns 0 for all of them). This is the hard backward-compat guarantee.
...
11. Author this amendment's manifest YAML at `docs/rebuild/plans/dev-pattern-simplifications-1.manifest.yaml` (using OLD shape — schema_version 1 — because this IS the amendment introducing v3; the manifest itself is the LAST one to use the legacy shape).
...
- `docs/rebuild/plans/cost-audit-loam-dev-pattern-2026-05-04.md` — source authority for Recommendations A/D/E.
