...
**Outcome:** `git diff <baseline>..<feature-commit-tip> --stat` shows changes only in: the named docs/prose surfaces (eleven files), the plan-doc + manifest under `docs/rebuild/plans/`, the H/L cross-cutting test file (admission + marker updates only), and the workspace-bootstrap AC39_6 test file (sentinel update only — no test-shape changes; no test-removal).
