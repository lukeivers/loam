...
## ADDENDUM 2026-06-09 (later) — The RECALL tier delivered (memory recall cycle sealed)

Backfilled at the seal of `docs/plans/memory-decision-ledger-surfacing-dispatch-packs.md` — the merged A/B north-star build, one amendment, five slices atop the GUARD tier the correctness cycle sealed earlier today:

1. **The June-7 eval verdict is EXECUTED.** Co-citation spread DELETED from production (the measured KILL — worse on every metric, 2× latency, 0.0→0.0 on its own target subset); power-law activation neutralized to 1.0 behind the default-off `LOAM_FBM_ACTIVATION` switch (the measured FIX-not-kill). Live-store harness re-run at the deletion: exact ranking identity with the floor arm, 330/330 queries, 0 mismatches. **Re-enabling activation is gated on a live-access-log re-run beating the BM25 floor — the flag exists; the measurement is the gate.**
2. **Surfacing rebuilt on BOTH render paths** (`keep_pace/retrieval.py` + `memory_consumer.py`): source paths on every model-facing pointer, salient pointer text (the shared `salient_snippet` — never envelope/notification junk), ~5KB-class named budgets (1200/1600 → 5000), whole-record injection (drop-whole, never half-emit).
3. **The decision ledger is live** (the structural piece neither prior cycle had): owner rulings as first-class records written at ruling time (D2=(a), owner-ratified), Stop-seam ruling-detector steer + session-start catch-up sweep, unified retrieval with whole-record loading + open-ruling ride-along, dispatch memory packs through the sealed frame-kernel bundle, and the decision-claim guard widening the sealed Layer CG's ground truth to the ledger. The $750k Tilth failure and the memory-blind-dispatch failure are now STANDING outcome-altitude regressions (AC.DLG.OA / AC.DMP.OA / AC.DCG.OA) riding every future memory-touching seal.
4. **Cycle 3 (consolidation) remains the open roadmap item**; the Cycle-4+ backlog (MM2/MM3, premise-flip auto-detection) stands as the correctness cycle left it, MINUS the spread/activation tending (retired by measurement).
