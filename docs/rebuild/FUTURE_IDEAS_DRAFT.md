...
- **Plan-doc skeleton template should mirror dispatch-template's session-accumulated lessons.** The dispatch-template update at commit `a17f1f7` added "Cycle mechanics" lessons (pre-author §14, `pos-amend apply` BEFORE commit, abs-path on `--plan-doc`, one-test-file-per-AC, H19 admission debt). The plan-doc skeleton template at `tools/pos-amend/templates/plan/dev-discipline.md` should ALSO carry these — particularly the pre-authored §14 heading expectation. Symmetric update worth making in a future small dev-discipline pass.
