...
follows the aggregator's existing `pos.aggregator.*` convention
(ingest.py:59).
"""
...
def test_s4_aggregator_store_close_surfaces_exception(tmp_path, caplog):
    ...
    with caplog.at_level(logging.DEBUG, logger="pos.aggregator.store"):
        store.close()

    matching = [
        r for r in caplog.records
        if r.name == "pos.aggregator.store"
