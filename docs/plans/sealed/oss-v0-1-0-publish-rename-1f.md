...
**Authority documents:**
- `docs/plans/loam-rename-decisions.md` Tier-2 (the M1f target).
- `.scratch/claude-output/loam-rename-migration-plan.md` §4.1 (research mechanics).
- `docs/plans/oss-v0-1-0-publish-rename.md` §2 ladder (M1f row), §5 series-wide hard constraints, §7 series-wide halt triggers.
- `docs/plans/oss-v0-1-0-publish.md` §5 (programme master plan; M1f row per series-master ladder).
...
8. **Item 8 — Component docs subdir rename per series-master M1f row.** `docs/archive/component-research/graceful-degradation/` → `docs/archive/component-research/dormancy/` via `git mv`. **This rename DEVIATES from the series-wide convention that `docs/archive/component-research/<comp>/{research.md,research-plan.md,brief.md,component.md}` are preserved historical records** — the series-master M1f row explicitly names this rename as in-scope for M1f, overriding the general preservation default for THIS component (and only this component). See §10 D-build.M1f.5 for the method-decision.
...
- **`docs/spec/pos-v2-rebuild-proposal.md`** and other `docs/spec/*.md` historical narrative — preserved per Idea 10 / dispatcher M1e ruling 3.
...
- **Historical plan-docs** at `docs/plans/*.md` — preserved per series convention. Plan-docs that mention `graceful-degradation` retain that wording (they describe pre-M1f events).
- **`docs/plans/two-modes-and-multi-workspace/*.md`** historical plan-docs — preserved.
- **`docs/archive/component-research/{hands-off-lifecycle,foundation-audit,self-upgrade-framework}/research.md` etc. historical research-records** — preserved (per series convention; their references to `graceful-degradation` describe pre-M1f events).
...
**Sealed-component fence (preliminary — see §4 ACs + §11 surface inventory):** **1 packaged component (dormancy, formerly graceful-degradation) plus 2 cross-component consumers (workspace-bootstrap, self-upgrade).** Plus universal admissions for `framework/tools/loam-migrate-dormancy-config/` (the new migration helper), `framework/first-run-inventory.yaml` (1 entry rename), `docs/archive/component-research/graceful-degradation/` → `docs/archive/component-research/dormancy/` (the docs subdir rename per series-master M1f row).
...
`docs/archive/component-research/graceful-degradation/` → `docs/archive/component-research/dormancy/` via `git mv`. Files moved as part of the directory rename: `component.md`, `proposal.md`, `research-plan.md`, `research.md`. The 13-allowlist test `framework/graceful-degradation/tests/test_no_sealed_amendments.py` (which becomes `framework/dormancy/tests/test_no_sealed_amendments.py` under the directory rename) carries the literal allowed-prefix `docs/archive/component-research/graceful-degradation/` — that allowlist entry rebases to `docs/archive/component-research/dormancy/` as part of the test-fixture content edit.

**Method-decision (D-build.M1f.5 in §10):** content INSIDE the renamed component-record docs is preserved verbatim under the rename — the prose describes pre-M1f research / proposal events using `graceful-degradation` vocabulary, which IS the historical record. The `git mv` operates on the directory shell, not the inner prose. This matches the series convention (`docs/archive/component-research/<comp>/{research.md,research-plan.md,brief.md,component.md}` historical preservation) for the inner content while honouring the series-master M1f-row directive that the directory shell (the path, not the prose) follow the rename.

**Outcome:**
- `ls docs/archive/component-research/dormancy/component.md` exists.
- `ls docs/archive/component-research/graceful-degradation/` returns "No such file or directory".
- `git log --follow docs/archive/component-research/dormancy/component.md` returns the full pre-M1f history.
- The four files' contents are byte-identical pre-vs-post (`git diff <baseline>..HEAD -- docs/archive/component-research/dormancy/` shows ONLY the directory move, no content edits).
...
- `docs/archive/component-research/graceful-degradation/...` → `docs/archive/component-research/dormancy/...` (the directory rename per AC.RNM-1f.8).
...
- No `docs/archive/component-research/{hands-off-lifecycle,foundation-audit,self-upgrade-framework}/research.md` etc. historical-research edits (they reference pre-M1f events using the `graceful-degradation` vocabulary, which is the historical record).
- No `docs/spec/*.md` content or filename edits.
- No `docs/plans/two-modes-and-multi-workspace/*.md` historical plan-doc edits.
...
- **Historical preservations.** `docs/plans/*.md` (other than this plan-doc + manifest YAML), `framework/dormancy/seals/SEAL_COMMIT.*` (content preserved; file moves with directory), `docs/archive/component-research/{hands-off-lifecycle, foundation-audit, self-upgrade-framework}/research.md` etc., `docs/archive/component-research/dormancy/{research.md,research-plan.md,component.md,proposal.md}` content (path moves; content preserved), `docs/plans/two-modes-and-multi-workspace/*.md`, `docs/spec/*.md`, and `framework/self-upgrade/manifests/pos-v2-v0.2.0.yaml` files preserved verbatim per series convention.
...
- **Historical component-record docs** at `docs/archive/component-research/{hands-off-lifecycle,foundation-audit,self-upgrade-framework,...}/{research.md,research-plan.md,brief.md,component.md}` — preserved (their references to `graceful-degradation` describe pre-M1f events).
- **Historical component-record docs INSIDE the renamed `docs/archive/component-research/dormancy/` directory** — content preserved verbatim under the rename per AC.RNM-1f.8 (the `git mv` operates on the directory shell, not the inner prose).
- **`docs/spec/*.md` (including `pos-v2-rebuild-proposal.md`)** — preserved.
- **`docs/plans/two-modes-and-multi-workspace/*.md`** historical plan-docs — preserved.
...
3. **M1f sub-plan + manifest commit.** This plan-doc + a manifest YAML at `docs/plans/oss-v0-1-0-publish-rename-1f.manifest.yaml` per the established M1a..M1e precedent shape.
...
5. **Phase B — Component docs subdir rename.** `git mv docs/archive/component-research/graceful-degradation docs/archive/component-research/dormancy`. Content preserved verbatim under the rename per AC.RNM-1f.8.
...
11. **A `dormancy` identifier already in use.** Pre-build verification at plan-authoring time: `grep -rE "dormancy" framework/ docs/` returns 0 hits in framework/ live surface and only hits in `docs/plans/loam-rename-decisions.md` + master plan + sub-plans (which name the rename target). NO collision. Halt-trigger fires only if a NEW collision emerges during the rename.
...
6. **Component docs subdir rename DEVIATES from series convention.** Per series convention, `docs/archive/component-research/<comp>/` historical records are preserved. Per series-master M1f row, `docs/archive/component-research/graceful-degradation/` → `docs/archive/component-research/dormancy/` IS in scope. Per §10 D-build.M1f.5 ruling: directory-shell move only (content preserved verbatim). Risk: builder accidentally edits inner prose during the rename, breaking the historical record. Mitigation: the `git mv` operates on the directory shell; content edits are explicitly out of scope for the docs subdir rename per AC.RNM-1f.8.
...
- **D-build.M1f.5 — Component docs subdir rename method.** Builder's call within AC.RNM-1f.8. Series convention preserves `docs/archive/component-research/<comp>/{research,research-plan,brief,component}.md` as historical records. Series-master M1f row OVERRIDES the convention for THIS component only. Recommendation: directory-shell `git mv` only; inner content preserved verbatim under the rename. The `git mv` operates on the directory shell; content edits are explicitly out of scope for the docs subdir rename. The `git log --follow` per file returns full pre-M1f history.
...
3. **(`docs/archive/component-research/graceful-degradation/` rename DEVIATES from series convention.)** Per series convention (M1a..M1e): `docs/archive/component-research/<comp>/{research.md,research-plan.md,brief.md,component.md}` are preserved historical records. Per series-master M1f row: this directory IS renamed in M1f. Per §10 D-build.M1f.5: directory-shell `git mv` only; inner content preserved verbatim. The deviation is an explicit override of the convention for this single component (the only Tier-2 component-rename in the catalogue).
...
7. **(The `framework/dormancy/tests/test_no_sealed_amendments.py` allowlist needs the path-self-reference rebrand.)** The allowlist literal `"framework/graceful-degradation/"` and `"docs/archive/component-research/graceful-degradation/"` inside the test file rebases to `"framework/dormancy/"` and `"docs/archive/component-research/dormancy/"` as part of the test's own content edit. Plus the module docstring at the top references `"graceful-degradation"` extensively (it's the seal-enforcement-retrofit narrative for that component) — recommendation: preserve the historical narrative inside the docstring (it describes pre-M1f events using the historical vocabulary) BUT update the module-top BASELINE constant to point at `820fd84` (M1f's BASELINE) rather than `74ae5d3` (M1e's). The allowlist rebases; the historical narrative stays. Builder's call at build time per §10 D-build.M1f.5's general principle (path edits, not prose edits, for historical-record files).
...
- AC.RNM-1f.8 — component docs subdir rename: verified by `ls docs/archive/component-research/dormancy/` + `git log --follow` history preservation.
...
- **Prior sub-amendments:**
  ...
  - `docs/plans/oss-v0-1-0-publish-rename-1e.md` (sealed `c806f57`; §14 backfill `820fd84`).
- **Authority documents (inherited from series master):**
  - `docs/plans/loam-rename-decisions.md` Tier-2 (the M1f target).
  - `.scratch/claude-output/loam-rename-migration-plan.md` §4.1 (mechanics).
- **Programme master plan:** `docs/plans/oss-v0-1-0-publish.md` (M1f row in §5 per series-master ladder).
...
- **Precedent multi-component sealed-amendment manifests:**
  - `docs/plans/oss-v0-1-0-publish-rename-1e.manifest.yaml` (M1e sibling — 14-component namespace pivot; precedent for `seal_diff` allowed_prefixes shape).
  - `docs/plans/oss-v0-1-0-publish-rename-1d.manifest.yaml` (M1d sibling — 13-component OTel fence).
  - `docs/plans/oss-v0-1-0-publish-rename-1b.manifest.yaml` (M1b sibling — 11-component fence; precedent for migration-helper authoring inside an amendment).
