...
6. **Seal**. `loam amend seal` commit. Advances SEAL_COMMIT sidecars; writes the per-amendment seal narrative; runs the touched components' suites AND the GUARD-SWEEP FLOOR — the cross-component protection sweep set (every tracked fence test, any tree location, plus the sweep-class guards in `docs/plans/guard-floor.yaml`) — at every seal regardless of the amendment's fence, with no bypass. A floor breach blocks the seal at the introducing cycle; a registry pattern resolving to zero tracked files halts loudly. Adding a new cross-component guard class = one pattern entry in the registry (universally-admitted path; never a fence breach). Per `docs/plans/seal-guard-sweep-floor.md` (AC.GFLOOR.*).
