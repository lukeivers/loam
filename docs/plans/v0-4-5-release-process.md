# v0.4.5 patch — concrete release process
...
**Slug:** `v0-4-5-release-process`
**Date authored:** 2026-05-09.
**Class:** META-FRAMEWORK (process discipline + tooling).
**Predecessor:** v0.4.4 patch (subagent-personas-routing-and-priming) — in flight at authoring time.
...
VALUE_PROPOSITION.md prime objective (loam helps people use LLMs to **build software**) → reliable release process is part of the harness toolkit the primary persona draws from → without it, every publish has dispatcher-recall risk + investigation tax → v0.4.5 ACs (V045.{1-6} + S) implement the CLI + runbook + dogfood.
...
### AC.V045.1 — `loam release` CLI verb
...
**Acceptance:** CLI registers in `framework/tools/loam/src/loam/cli.py` (or wherever existing subcommand routing lives); `loam release --help` produces usage; integration test verifies subcommand dispatch.

### AC.V045.2 — Structural pre-publish gates
...
5. Current branch is `pos-v2` (`git branch --show-current` returns `pos-v2`).
...
**Builder note:** Path A — single CLI module with per-gate functions. Path B — pluggable gate registry with each gate as a separate file. Default A unless complexity warrants B. _Builder rules at build time per D-V045.2._

### AC.V045.3 — Tag + push action

**What:** CLI tags the seal commit with annotated tag (message = version + objective sentence from roadmap), pushes branch + tag to `loam` remote.

**Idempotency:** Re-running on already-published version produces no-op + clear message ("v0.4.X already on loam remote at <SHA>; nothing to do").
...
### AC.V045.4 — Optional GitHub Release with auto-generated notes
...
### AC.V045.5 — Release-process runbook
...
4. Manual fallback (if CLI is unavailable: the explicit `git tag` + `git push loam pos-v2:main` + `git push loam <tag>` ritual; matches today's manual ritual for back-compat).
...
### AC.V045.S — Seal-diff discipline

`git diff --name-only BASELINE..SEAL_COMMIT` shows changes only under:

- `framework/tools/loam/` (CLI subcommand registration + new `release.py` module + tests)
- `docs/release-process.md` (new file)
- `docs/release-roadmap.md` (§2 row added for v0.4.5; §3 active-version updated)
- `docs/STATE.md` (v0.4.5 SHIPPED rollup)
- Plan-doc §status backfill on this file post-seal
...
2. **Composes with `feedback_hard_smoke_per_minor_before_publish`** — HARD smoke gate is a structural pre-publish prereq enforced by AC.V045.2.
...
9. **Outcome-altitude AC requirement** per `feedback_test_outcome_altitude_required.md` — AC.V045.6 is the outcome-altitude probe (dogfood v0.4.5 publish using the new CLI).
...
- **Multi-remote publish** — only `loam` remote per the current architecture. Multi-remote (e.g., publishing to a mirror) is FIDRAFT-able if it ever becomes needed.
...
- **Notification (email / Slack / Telegram on publish)** — not in v0.4.5 scope. Captured for FIDRAFT.

## §7 — HARD HALTs (build-time)

Halt-and-surface to dispatcher (return owner-call) — do NOT proceed past — on any of:

1. AC.V045.6 dogfood publish RED (CLI fails against any pre-publish gate during the v0.4.5 publish itself). Halt; surface as F-DESIGN candidate (the design itself didn't survive first-real-use).
2. ODD §2.5 violation in your work OR surrounding code (per `feedback_subagent_odd_violation_halt`).
3. Wrong-tree-write (any edit lands at a path outside `/Users/lukeivers/ivers-corp-pos-v2/`).
...
- **D-V045.2** (CLI module shape): default Path A (single module with per-gate functions). Switch to Path B (pluggable gate registry) only if complexity demonstrably warrants. Document chosen path + reasoning in build report.
- **D-V045.4** (auto-generated release notes shape): default = plan-doc §1 + §status + commit log. If commit log is too noisy (e.g., includes §14 backfill + manifest-bump churn), filter to feat/fix/docs commits only.
- **D-V045.5** (runbook depth): default = 1-page reference (4 sections). Switch to longer onboarding-tutorial shape only if first reader signals confusion.

## §9 — Dependencies

- **v0.4.4 (subagent-personas-routing-and-priming)** — **SOFT** per `docs/release-roadmap-dependency-map.md` taxonomy. Different fence (`plugins/dev-sdlc/` vs `framework/tools/loam/`); sequencing preference only. v0.4.5 plan-doc + build dispatch are parallel-safe with v0.4.4 build.
- v0.4.3 (memory retrieval BM25 fix) — HARD on commit-graph (v0.4.5 builds on v0.4.3's HEAD).
...
| **Total v0.4.5 build** | **90-160 min** | **~125 min** |

Per `feedback_duration_estimation_rubric` — single-component amendment + dogfood probe; midpoint ~125 min.
...
1. **Path A (CLI verb) + Path B (runbook doc) bundle vs split.** Plan-doc bundles both into v0.4.5. Alternative: ship runbook as v0.4.5 (immediate; covers today's gap with a reference doc), CLI as v0.4.6 (structural; follows when runbook validates the design). My call: **bundle**, because runbook-only is the rule-shape Luke explicitly rejected as "discipline that won't survive."
2. **AC.V045.4 GitHub Release inclusion** — opt-in via `--release` flag. Alternative: Release creation is mandatory part of every publish. Recommend opt-in (matches current state where v0.1.0 → v0.4.3 don't have Release pages on GitHub; backward-compat).
3. **AC.V045.6 dogfood timing** — v0.4.5 publish IS the test (per AC). Alternative: ship without dogfood, dogfood lands at v0.4.6's publish (one-version delay reduces risk). Recommend dogfood, because the outcome-altitude rule (`feedback_test_outcome_altitude_required`) requires the AC to test the user-visible outcome; non-dogfood would be STUB-class.
4. **Sequence vs parallel with v0.4.4.** v0.4.4 builder is in flight; v0.4.5 build agent could dispatch in parallel (different fence per dep-map). Recommend **wait for v0.4.4 to seal** before v0.4.5 build dispatches (avoids two concurrent build agents in same canonical worktree per `feedback_serialize_amendment_builds`); plan-doc author/ratify is parallel-safe.
5. **CLI subcommand naming** — `loam release` vs `loam publish`. Both are reasonable. Recommend `loam release` because "release" is a noun + matches GitHub Release vocabulary; "publish" reads as a verb-only action. Either is OK; surface for explicit ratification.

## §12 — Authority chain

- Telegram 10547 (owner directive for the work itself).
- Telegram 10541 (owner authorization to proceed with v0.4.4 + sequencing).
- Calibration analysis from today's v0.4.3 publish: `<workspace>/.scratch/claude-output/soft-halt-version-deps-orchestration-analysis-2026-05-09.md` (origin of the dep-map artefact that this v0.4.5 references).
- Memory rule `feedback_soft_halt_vs_hard_halt.md` (codified today; publish is a HARD HALT class action; CLI gates respect that).
