...
`docs/plans/amendment-29-per-workspace-memory-sidecar-port.md`
...
- Plan path stays as `docs/plans/amendment-29-per-workspace-memory-sidecar-port.md`
  (no rename — `#29` is the resolved number).
- New manifest:
  `docs/plans/amendment-29-per-workspace-memory-sidecar-port.manifest.yaml`.
- Narrative target:
