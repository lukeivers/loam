...
the slug only.
**Research plan:** `docs/rebuild/plans/research/memory-system-env-scrubber-research-plan.md`.
**Research:** `docs/rebuild/plans/research/memory-system-env-scrubber-research.md`.
...
1. Build agent authors its own plan at a path under
   `docs/rebuild/plans/` (plan-before-code CDC — separate from this
...
`docs/rebuild/plans/amendment-<N>-memory-system-env-scrubber-user.manifest.yaml`):

- `schema_version: 1`
- `amendment.number: <assigned>`, `slug: memory-system-env-scrubber-user`
- `baseline: <pre-amendment-tip>` (captured at dispatch time)
- `plan: docs/rebuild/plans/amendment-30-memory-system-env-scrubber-user.md`
- `components:` — one entry:
  ...
  - No `extra_allowed_prefixes` unless the builder discovers a new
    ...
    `docs/rebuild/components/memory-system/` are admitted already.
