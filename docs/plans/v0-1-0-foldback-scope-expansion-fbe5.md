...
**Parent plan:** `docs/plans/v0-1-0-foldback-scope-expansion.md` (FBE.5 row in §1 + AC ladder in §4 + §8 register).
...
### Surface #7 (no halt — recorded; CLI claims `.claude/` scaffolded but no code creates it)

Pre-seal smoke verification (run as part of §12 step 2) revealed: `loam init` prints "  .claude/    ← scaffolded (Claude Code expects this here)" (`framework/loam-init/src/loam/loam_init/cli.py:128`) but no code path actually creates `<workspace>/.claude/` on disk. Direct verification: `ls /tmp/test-fbe5-ws/` after `loam init /tmp/test-fbe5-ws --from /Users/lukeivers/ivers-corp-pos-v2/` shows `framework/`, `workspace/`, `.gitignore` only — no `.claude/` directory. Source trace: `framework/workspace-bootstrap/src/loam/workspace_bootstrap/new_workspace.py:508` declares `claude_dir = new_ws_path / ".claude"` and line 580 returns it in `BootstrapResult` but no `mkdir` call exists for it; FBE.7 retired the `mcp_json_writer` invocation (`first_run_scaffold.py:719-725` writes `MCPJsonWriteResult(wrote=False, reason="skipped_v0_1_0_no_graphiti")`) which may have been the historical creator of the `.claude/` directory tree.

**Out of FBE.5 scope (per ODD §2.5):**
- AC.FBE.5.6 names `~/.loam/`, NOT `<workspace>/.claude/`. The smoke result is AC.FBE.5.6-passing (loam init exits 0; workspace bootstraps; `~/.loam/` contract intact for local-path form).
- The CLI message at `cli.py:128` is misleading prose, not a behaviour bug for FBE.5's scope.

**FUTURE_IDEAS_DRAFT candidate (or FBE.6 reviewer-flag candidate):** "loam init prints `.claude/  ← scaffolded` but no code actually creates `<workspace>/.claude/` (verified at `framework/loam-init/src/loam/loam_init/cli.py:128` + `framework/workspace-bootstrap/src/loam/workspace_bootstrap/new_workspace.py:508` — `claude_dir` declared but never `mkdir`-ed). FBE.7 retired the `mcp_json_writer` invocation which may have been the historical creator. Either (a) `mkdir(claude_dir, exist_ok=True)` belongs in `bootstrap_new_workspace` after the FBE.7 retirement, OR (b) the CLI summary message should be updated to drop the `.claude/` line. Surface for FBE.6 reviewer to re-flag if it matters for the stranger UX (Claude Code may auto-create `.claude/` on first session-start regardless)."

This is recorded but not halted on because:
1. AC.FBE.5.6 verification scope is `~/.loam/`, not `<workspace>/.claude/`.
2. The smoke exits 0 — no functional regression of FBE.1's contract.
3. Surfaced as a candidate item for FBE.6 reviewer + FUTURE_IDEAS_DRAFT.
...
**Binds to:**
- **AC.PO.1 + AC.PO.2** (prime objective per `docs/VALUE_PROPOSITION.md`) — closing the "stranger reads the docs and is confused by `pOS v2` / `M1g` / `Phase 4` etc. vocabulary" failure mode that the M11a-3 reviewer flagged as HIGH 4 + LOW 1 + LOW 3.
...
| **AC.FBE.5.6** (rewritten — contract-aligned) | `loam init /tmp/test-fbe5-ws --from /Users/lukeivers/ivers-corp-pos-v2/` (invoked from a fresh shell session against the post-seal canonical tree) succeeds with exit 0 AND scaffolds `/tmp/test-fbe5-ws/framework/` (clone of canonical) + `/tmp/test-fbe5-ws/workspace/` (workspace state with `.pos/` + `personas/` populated). The `~/.loam/canonical-cache/` write is contract-conditional on URL-form clone (per `new_workspace.py:638-642`); local-path form does not write to `~/.loam/`. **The smoke is "loam init succeeds end-to-end"**; HIGH 2's "`~/.loam/` exists" concern auto-resolves via the URL-form contract being intact (verifiable by code-read of `new_workspace.py:638-642`, NOT by the local-path smoke). Per `feedback_loose_AC_text_fix_AC_not_implementation` — the parent plan AC.FBE.5.6 + the §2.5 HIGH 2 framing are both loose on the URL/local-path distinction; this AC tightens to the actual `bootstrap_new_workspace` contract. | Direct shell invocation against the canonical-installed `loam` (the venv at `/Users/lukeivers/ivers-corp-pos-v2/.venv/bin/loam`); verify `loam init` exits 0; verify `/tmp/test-fbe5-ws/framework/` exists with `<canonical>/.git` cloned; verify `/tmp/test-fbe5-ws/workspace/.pos/` populated with `sync-config.yaml` + `personas/primary/` files. URL-form `~/.loam/canonical-cache/` contract verified by code-read of `new_workspace.py:638-642`. |
...
- `docs/plans/v0-1-0-foldback-scope-expansion-fbe5.md` (this file).
- `docs/plans/v0-1-0-foldback-scope-expansion-fbe5.manifest.yaml`.

### Parent plan-doc backfill (post-seal, separate commit):

- `docs/plans/v0-1-0-foldback-scope-expansion.md` — §8 method-decision register: replace `### FBE.5` placeholder with apply commit SHA + seal commit SHA + verification summary.
...
4. **Manifest commit** — author `docs/plans/v0-1-0-foldback-scope-expansion-fbe5.manifest.yaml` (15 components in `components:` block).
...
7. **Parent plan-doc backfill** — `docs/plans/v0-1-0-foldback-scope-expansion.md` §8 backfill `### FBE.5` subsection with apply + seal SHAs (separate NEW commit; admitted via `docs/plans/` universal prefix).
...
- **Parent plan:** `docs/plans/v0-1-0-foldback-scope-expansion.md` (§4 FBE.5 row).
- **FBE.4 status (partner-prefix gap precedent):** `<workspace>/.scratch/claude-output/fbe4-status-2026-05-03.md` Surface #5.
- **FBE.4 sub-plan / manifest YAML (multi-component fence shape precedent):** `docs/plans/v0-1-0-foldback-scope-expansion-fbe4.{md,manifest.yaml}` — fence-three-no-edit shape; this FBE.5 sub-plan extends to fence-fifteen-with-edit.
