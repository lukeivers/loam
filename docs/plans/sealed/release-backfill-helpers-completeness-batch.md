...
- `plugins/dev-sdlc/tests/test_no_sealed_amendments.py` (BASELINE pointer auto-bump at seal-time — bookkeeping, not new test logic)
- `framework/per-project-pm/state/SEAL_COMMIT.dev-sdlc` (per-project-pm sidecar, if applicable in current repo state)
