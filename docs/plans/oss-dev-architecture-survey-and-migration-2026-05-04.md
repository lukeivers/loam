...
- A future workspace-bootstrap could use a simpler convention: paths matching `**/dev/**` or `docs/plans/**` are dev-internal; paths under `framework/<component>/src/` are runtime; paths under `framework/<component>/tests/` are test material. No manifest file required.
...
- **34 plan files** with FBE in the filename (`docs/plans/*FBE*`).
...
| Where do dev internals live? | **In-repo, visible.** `docs/plans/` stays where it is. `CLAUDE.md` + `CLAUDE.dev.md` stay at root (Astral/swarms precedent). `CONTRIBUTING.md` describes the contribution flow. The amendment-narrative commit messages stay (they are valuable and not embarrassing). |
| What about the synthesis tool? | **Deprecate.** Remove `framework/tools/pos-publish-framework-only/` from active code; archive its source under `docs/archive/synthesis-tool-2026-05-04/` for historical context. Replace its version-substitution role with `setuptools_scm`. |
...
| **4** | Delete `framework/tools/pos-publish-framework-only/`. Replace any version-substitution callsites with `setuptools_scm`. Remove `publish-mode-manifest.yaml` (or move to `docs/archive/`). Update `framework-only` branch tracking — drop the branch from all remotes. | 30–90 min | 5 min (decide whether to archive vs. delete the synthesis tool source) | Reversible until phase 6 |
...
| 4 | Removing synthesis tool breaks an in-flight dispatch that depends on it | Pre-flight: `grep -r 'pos-publish-framework-only\|publish_framework_only\|framework-only' framework/ docs/plans/` and audit. The amendment workflow does not depend on this tool — only the (deprecated) M11/M12 publish path does. |
...
|---|---|---|
| `framework/tools/pos-publish-framework-only/` (source + tests) | **Delete from `main`**, archive copy to `docs/archive/synthesis-tool-2026-05-04/` | Solving an imaginary problem (§3.2) |
...
| `docs/plans/`, `docs/archive/component-research/`, `docs/spec/`, `VALUE_PROPOSITION.md` | **Kept verbatim** in new repo |
...
- **If a contributor objects to the visibility of half-finished plan files** (e.g. "I PRed something that contradicts a plan in `docs/plans/` that I didn't know existed"), there's a small UX cost. **Mitigation:** `CONTRIBUTING.md` explicitly says "plans in `docs/plans/` reflect the maintainer's current thinking; if you disagree, file an issue."
...
- **Option B (archive):** move to `docs/archive/synthesis-tool-2026-05-04/` for future-Luke discoverability.
...
- 34 plan files with `FBE` in filename under `docs/plans/`.
...
`docs/archive/synthesis-tool-2026-05-04/`. Archive `README.md`
...
`docs/plans/` are immutable history — they reflect the
operational reality at the time those plans were authored and are not
updated. Two refs in `docs/archive/component-research/*/research.md` are
...
- The test conftest (`framework/workspace-bootstrap/tests/conftest.py`)
  ...
  to `docs/archive/`, those fixtures break unless rewritten.
...
6. Sweep CLAUDE.dev.md / docs/spec/ for any
...
- Synthesis tool source preserved at
  `docs/archive/synthesis-tool-2026-05-04/` with deprecation
  README.
