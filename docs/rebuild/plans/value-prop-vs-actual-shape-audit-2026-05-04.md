...
- `docs/rebuild/plans/v0-1-x-roadmap.md` (current in-flight roadmap).
- Recent FIDRAFT entries (81 entries; full content available at `docs/rebuild/FUTURE_IDEAS_DRAFT.md`).
...
**Pattern A — heavy plan-doc + manifest authoring before any code.** Every recent amendment ships a sub-plan-doc (e.g. `docs/rebuild/plans/v0-1-3-skill-packages.md`, `docs/rebuild/plans/v0-1-2-V11-A-orchestrator-fix.md`, the in-flight m-fbm-operational-health sub-plan) plus a manifest (`*.manifest.yaml`) plus a §8 SHA-record commit on the parent roadmap. Citation: commits `f50647f`, `c7c429c`, `93a2a3a`, `2c95507`, `6749f44`, `d63ba9c`, etc. — every amendment cycle adds three or more doc-only commits before and after any sealed-component build. This is consistent with `feedback_plan_before_code` (hard rule) but the *volume* of plan-doc material is large enough that the persona's tokens-per-turn skew toward plan authorship rather than translation.
...
*End of audit. Companion artefacts: `VALUE_PROPOSITION.md` (anchor); `framework/primary-persona/templates/persona-template/prompt.md` (contract surface); `docs/design/primary-persona-shape.md` (shape rationale); `docs/rebuild/plans/v0-1-x-roadmap.md` (sequencing). Reshape lands at v0.1.4 if Luke rules in favor of §5 Option 5.*
