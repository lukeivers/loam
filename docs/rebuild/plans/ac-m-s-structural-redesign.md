...
**Research doc:** `docs/rebuild/plans/research/ac-m-s-structural-redesign-research.md`.
...
- `docs/rebuild/plans/` + `docs/rebuild/plans/research/` (universal
  prefix)
- `framework/primary-persona/seals/` (seal narrative target)
- `framework/primary-persona/tests/SEAL_COMMIT` (sidecar advance,
  via pos-amend)
- `CLAUDE.md`, `docs/rebuild/FUTURE_IDEAS.md` only if a relevant
...
- `docs/rebuild/plans/` — universal prefix (this plan + manifest).
...
- Restore any of the post-#48 widening admissions
  ...
  `docs/rebuild/capability-corpus/`, etc.) to the AC.M.S
...
plan: docs/rebuild/plans/ac-m-s-structural-redesign.md
...
4. **§10.3 doc tightening conflicts with another amendment in
   ...
   methodology docs per `docs/rebuild/plans/`'s in-flight scan),
...
- `docs/rebuild/FUTURE_IDEAS_DRAFT.md` — originating capture
