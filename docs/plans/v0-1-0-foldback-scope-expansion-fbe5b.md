...
**Parent plan:** `docs/plans/v0-1-0-foldback-scope-expansion.md` (FBE.5b row to be backfilled in §8 register; see end-of-doc note in §1).
...
**Binds to:**
- **AC.PO.1 + AC.PO.2** (prime objective per `docs/VALUE_PROPOSITION.md`) — closing the "stranger-clone install + first-run looks right but Claude Code session-start doesn't find a workspace `.claude/`" failure mode that Lens 2's primary-persona test exposes.
...
- `docs/plans/v0-1-0-foldback-scope-expansion-fbe5b.md` (this file).
- `docs/plans/v0-1-0-foldback-scope-expansion-fbe5b.manifest.yaml`.

### Parent plan-doc backfill (post-seal, separate commit):

- `docs/plans/v0-1-0-foldback-scope-expansion.md` — §8 method-decision register: ADD a new `### FBE.5b — <workspace>/.claude/ scaffold gap closed` subsection with apply commit SHA + seal commit SHA + verification summary; also update the closing "Remaining sequence:" line to drop FBE.5b (now done) and lead with FBE.6.
...
3. **Manifest commit** — author `docs/plans/v0-1-0-foldback-scope-expansion-fbe5b.manifest.yaml` (single component in `components:` block: workspace-bootstrap).
...
7. **Parent plan-doc backfill** — `docs/plans/v0-1-0-foldback-scope-expansion.md` §8 add `### FBE.5b` subsection with apply + seal SHAs + drop FBE.5b from the closing "Remaining sequence:" line (separate NEW commit; admitted via `docs/plans/` universal prefix).
...
- **Parent plan:** `docs/plans/v0-1-0-foldback-scope-expansion.md` (FBE.5b named in closing line; §8 register backfilled at completion).
- **FBE.5 status (Surface #7 origin):** `/Users/lukeivers/pos3/workspace/.scratch/claude-output/fbe5-status-2026-05-03.md`.
- **FBE.5 sub-plan (Surface #7 framing):** `docs/plans/v0-1-0-foldback-scope-expansion-fbe5.md` §2 Surface #7.
