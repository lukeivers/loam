...
``loam amend apply``'s `objectives` manifest block (per pos-amend-
tracker-integration plan AC.D-pa.1 — historical plan filename) registers
its declared ACs in the tracker as part of the apply step. ``loam amend
seal`` then populates ``lifted_from.source_commit``.
...
The module is fully synchronous: loam amend's ``register_objectives``
...
def verify_continuous_registration(
    *,
    loam_amend_repo_root: Path | None = None,
) -> VerifyReport:
    ...
    amendment-AC has a parent), then composes against loam amend's
    ...
    The function does NOT shell out to ``loam amend apply`` — that's a
