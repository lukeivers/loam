"""``pos-amend seal`` — advance sidecars + finalise the amendment cycle.
...
verify ``pos-amend apply --dry-run`` exits 0 against the post-seal
...
from pos_amend.manifest import Manifest, ManifestError, load_manifest
...
def _discover_sealed_components(repo_root: Path) -> list[str]:
    ...
    Discovery: every top-level directory that carries a
    ``tests/SEAL_COMMIT`` sidecar is a sealed component. This matches
    the convention every seal-diff test in the workspace already
    relies on. Returned sorted for determinism.
    """
    components: list[str] = []
    for sidecar in repo_root.glob("*/tests/SEAL_COMMIT"):
...
def _seal_diff_test_path(repo_root: Path, component: str) -> Path | None:
    ...
    Convention: ``<comp>/tests/test_no_sealed_amendments.py`` for most
    sealed components, ``<comp>/tests/test_cross_cutting.py`` for
    hands-off-lifecycle (per amendment #22 ruling on the lifecycle's
...
def _apply_dry_run_post_seal(manifest_path: Path) -> int:
    """Run ``pos-amend apply --dry-run`` against the post-seal HEAD."""
...
def _legacy_seal(manifest: Manifest, repo_root: Path) -> int:
    ...
    are byte-identical to pre-extension ``pos-amend seal``.
    """
...
def _finalize(
    ...
    *,
    ...
    # ------------------------------------------------------------------
    for comp in manifest.components:
        comp_tests = repo_root / comp.name / "tests"
    ...
    if dry_rc != 0:
        _emit_diagnostic(
            _FailureCheckpoint(
                ...
                detail=(
                    f"post-seal `pos-amend apply --dry-run` exited "
    ...
    if plan_doc is not None:
        ...
        backfill_body_lines = [
            ...
            "",
            "Mechanised by `pos-amend seal --plan-doc` per AC.D-sa.7.",
        ]
