...
**Parent plan:** `docs/rebuild/plans/v0-1-0-foldback-scope-expansion.md` §4 FBE.1.
...
**Binds to:**
- **AC.PO.1 + AC.PO.2** (prime objective per `docs/rebuild/VALUE_PROPOSITION.md`) — making the documented `loam init` verb actually exist is a prerequisite for the harness's "primary persona translates intent → execution" promise.
...
- `docs/rebuild/plans/v0-1-0-foldback-scope-expansion-fbe1.md` (this file).
- `docs/rebuild/plans/v0-1-0-foldback-scope-expansion-fbe1.manifest.yaml`.

### Parent plan-doc backfill (post-seal, separate commit):

- `docs/rebuild/plans/v0-1-0-foldback-scope-expansion.md` — §8 method-decision register entries: apply commit SHA + seal commit SHA for FBE.1.
...
- **Risk: parent plan's `extra_allowed_files` list is wider than needed.** Mitigation: my manifest uses `universal_paths.prefixes` only for the partition manifest path + `docs/rebuild/plans/` (already universal); the fence stays tight at `framework/loam-init/`.
...
4. **Manifest commit** — author `docs/rebuild/plans/v0-1-0-foldback-scope-expansion-fbe1.manifest.yaml`.
...
7. **Parent plan-doc backfill** — `docs/rebuild/plans/v0-1-0-foldback-scope-expansion.md` §8 FBE.1 entries get the apply + seal SHAs (separate NEW commit; admitted via `docs/rebuild/plans/` universal prefix).
...
- **Universal-paths-admission precedent:** `docs/rebuild/plans/oss-v0-1-0-publish-public-docs-partition-fix.manifest.yaml` (M7-partition-fix, amendment #98).
