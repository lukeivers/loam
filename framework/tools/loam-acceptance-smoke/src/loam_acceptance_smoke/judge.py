...
SOFT_DIMENSIONS: dict[str, str] = {
    ...
    "four-step-loop-ran": (
        "Did loam run the four-step loop AND close PROPORTIONALLY? The four legs: "
        "infer the real end-intent, propose a healthy way to enable it, SURFACE "
        "that proposal back to the user as a checkable HYPOTHESIS (never a silently "
        "assumed, already-built thing), and ADAPT from the answer. The owner's "
        "ratified close-bar is RELATIVE: loam LANDS the literal ask as the ONE "
        "thing, and BY DEFAULT it may ASK about moving EXACTLY ONE rung up a "
        "leverage ladder (e.g. a doc -> an optional reusable TEMPLATE), opt-in and "
        "phrased as a question the user can decline. That one-rung opt-in ask is "
        "the EXPECTED default — its absence is only correct when the user gave a "
        "clear one-off signal ('just this once' / 'no thanks' / overwhelmed / "
        "explicit decline). REWARD: landed the literal ask + asked at most one "
        "opt-in rung up BY DEFAULT (suppressed only on a clear one-off signal) + "
        "all four legs visible and adapted. PENALIZE (FAIL): jumped TWO+ rungs "
        "(pitching a workflow/system/framework for a single doc); ASSERTED or "
        "already-built structure instead of an opt-in ASK; or FAILED to offer the "
        "one-rung ask when the user gave NO suppressing signal. Do NOT require "
        "committed concrete structure — an opt-in ask is the right-sized leg, not "
        "a built framework. PASS only if all four legs are visible AND the close "
        "was proportional (at most one opt-in rung up, by default)."
    ),
    "no-over-engineering": (
