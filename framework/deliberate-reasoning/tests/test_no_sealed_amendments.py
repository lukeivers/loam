...
deliberate-reasoning component + its declared universal-admitted partners.
...
BASELINE: the commit immediately preceding this slice's source-edit commit
...
BASELINE = "c0c584f4"  # the RATIFIED plan-doc commit (main tip @ plan-authoring;
# the commit immediately preceding this slice's source-edit commit).
...
def test_only_deliberate_reasoning_and_partners_changed() -> None:
    ...
    # The fence: the new component + the universal-admitted doc/plan
    # surfaces (manifest universal_paths).
    allowed_prefixes = (
        "framework/deliberate-reasoning/",
        "docs/plans/",
    )
    allowed_files = {
        "CLAUDE.md",
