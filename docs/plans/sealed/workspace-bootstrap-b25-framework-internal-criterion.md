...
**Amends:** `workspace-bootstrap/docs/archive/component-research/workspace-bootstrap/proposal.md` (new B25 criterion), `workspace-bootstrap/tests/test_extension_protocol.py` (new B25 test), `docs/odd-in-pos.md` (§6 cross-reference). Docs + test-only. Zero `workspace-bootstrap/src/` edits.
...
- `docs/archive/component-research/workspace-bootstrap/proposal.md` — add §4.8 (new group "Framework-internal phase set") with a single criterion **B25**. Update §4's count header (24 → 25 objectives). Leave B18 text unchanged at the criterion level; it continues to govern the external-extension protocol.
...
- `docs/plans/workspace-bootstrap-b25-framework-internal-criterion.md` — this plan.
...
- Add to `allowed_prefixes` tuple: `"docs/archive/component-research/workspace-bootstrap/"` (this amendment is the first workspace-bootstrap proposal-doc edit since the initial port at `a11f081`; the prefix must be admitted so the B20 diff-scope check tolerates the proposal edit).
...
- `allowed_prefixes` tuple in workspace-bootstrap seal test: verified missing `docs/archive/component-research/workspace-bootstrap/` and `docs/odd-in-pos.md` — both get added, both scoped precisely.
...
1. **Amendment commit** — `fix(workspace-bootstrap, hands-off-lifecycle): workspace-bootstrap B25 framework-internal-phase criterion (amendment #17)` — includes:
   - New B25 text in `docs/archive/component-research/workspace-bootstrap/proposal.md` §4.8; §4 count header 24 → 25; optional one-sentence cross-reference in §4.5 after B19.
