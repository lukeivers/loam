...
| **4** | **prioritization + the relational web** | Derived priority; blocks/waits-on edges; graph self-heal against FBM STATE (deviation → #71). | Increment 2 + #71 | MED | BUILT (sealed-local on `build/wms-increment-4`, seal `1fb84d9c`): `docs/plans/wms-increment-4-prioritization-and-relational-web.md` — two `primary-persona` keep-pace modules. `prioritize.py`: a derived, TRANSPARENT, calibrate-on-use FIVE-signal ordering (the existing `tracker_context` priority-key + blocking-impact off the existing edge graph + goal-alignment off `OBJECTIVES.md` + staleness off `last_transition_at` + explicit owner pin/defer as a HARD override), calibrate-on-use weights (no magic number, WMS-D5 confirmed), a plain-language per-item reason — NEVER a surfaced numeric score (D-PRI.4). `relational.py`: a `TriggerKind.turn` lens surfacing the relational web from the EXISTING `unblocked_next`/`waiting_on_other`/`trace_to_root` queries (prioritized unblocked-next single-top-plus-tail + reason / blocked-on-what / waiting-on-me-vs-others / decomposition tree), ONE capped fail-soft block. The objective-tracker store is CONSUMED via its existing READ API, NOT modified (D-PRI.1) — no edge/priority STORAGE added. **The §7-row "graph self-heal against FBM STATE" is SPLIT: inc-4 took the READ-slice — FBM `derive_project_state` as a priority signal where a project is FBM-bound — and explicitly DEFERS the edge-MUTATION self-heal to #71 (D-PRI.5 / §10 RF#2).** Outcome-altitude AC.WMS4.LIVE.1 exercised against a real dependency chain through the live production entry points. |
...
| **7** | **analytics** | Throughput / aging / bottleneck / plate-load views over the graph + event history. | Increment 2 (the graph + events) | LOW (which analytics a non-tech user wants is unverified) | BUILT (sealed-local on `build/wms-increment-7`, seal `fde2b17`): `docs/plans/sealed/wms-increment-7-analytics.md` — the LAST increment, CLOSING the 7-increment WMS roadmap. ONE `primary-persona` keep-pace module `analytics.py` (three read-only derivations + an on-demand `render_analytics_block`) composing the inc-2 event log + projection READ-ONLY (Lens-1). **The lowest-confidence increment's honest realisation (WMS-D6 / D-ANL.1-2): the conservative THREE actionable insights ship — (1) where work is piling up/stalling, (2) chronically blocked/waiting items, (3) completion-vs-intake balance — each one CHANGES what the user does next (the Lens-2 test); the VANITY metrics are CUT** (raw throughput-as-a-number, velocity/burndown, cycle-time-headline, per-item bottleneck-edge counts, charts/time-series — number-theatre for a single person tracking their own work). Cycle-time survives only as a SUPPORTING phrase inside the pile-up insight (D-ANL.5), never a headline. **ON-DEMAND only (D-ANL.3): mirrors inc-5's `render_plate_block`/`render_goals_block` — registers NO `TriggerKind.turn` contributor, zero new always-on per-turn surface** (the FBM-don't-bloat mandate inc-6 slimmed). Derives over the event log + projection read-only — NO new store/field/event-kind/query/lifecycle/lens (D-ANL.4); thresholds are calibrate-on-use method-defaults (D-ANL.6). Outcome-altitude AC.WMS7.LIVE.1 exercised against a REAL store with a REAL transition history through the live production entry point, no mocks at the store/event-log boundary. **The 7-increment WMS roadmap is COMPLETE (all increments sealed-local); named post-roadmap follow-ons remain open (NOT roadmap increments): the #71 edge-mutating self-heal; widening the #34 seed taxonomy so `work-tracking` is a first-class `AIM_AREAS` area; registering Money/Personal-Home as FBM projects; the behavioural auto-learn of the lens choice.** |
...
**WMS-D5 — Priority is derived, never hand-stored; the weighting is calibrate-on-use, not imported.** RECOMMEND: compose on the existing `tracker_context` priority-key; add goal-alignment + dependency-position + staleness + explicit-pin signals; tune on real use (Lens 4 — no magic number). User pins always override. **CONFIRMED + BUILT (increment 4, seal `1fb84d9c`):** the FIVE-signal set landed exactly as named (priority-key · blocking-impact · goal-alignment · staleness · explicit pin/defer-as-hard-override), with calibrate-on-use weights (`prioritize.DEFAULT_SIGNAL_WEIGHTS`, a tunable mapping — no imported magic number, AC.PRI.5) and a plain-language transparent reason in place of any surfaced score (D-PRI.4 / AC.PRI.4). The signal-SET-vs-WEIGHTS line (the §10 RF#1 scope-confidence call) was honoured: the set was the owner-shape call, the numeric weights an autonomous calibratable method-default.

**WMS-D6 — Analytics scope + timing. RECOMMEND: defer to the last increment; ship only after the lenses prove the model and there's event-history to analyze. Which analytics a non-tech user actually wants is unverified — don't build speculatively.** — **BUILT (Increment 7, seal `fde2b17`): confirmed the conservative-three-not-dashboard realisation.** The recommend held: analytics shipped LAST, over the proven lens model + the inc-2 event history, NOT speculatively. The owner ruling (D-ANL.1/.2) shipped exactly the three insights that change what the user does next and CUT every vanity metric — the honest answer to "which analytics a non-tech user actually wants" is the few that end in an action, not a dashboard.
