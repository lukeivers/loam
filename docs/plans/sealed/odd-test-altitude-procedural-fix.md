...
- `docs/STATE.md` — methodology-fix entry inline (post-seal).
...
- Manifest: `docs/plans/odd-test-altitude-procedural-fix.manifest.yaml` (schema v3).
- Component: `dev-sdlc` (single-component sweep at seal time; SKILLs live under this component).
- Universal admissions: `docs/plans/` for plan-doc paper trail + `docs/odd-llm-grounding.lean.md` for the methodology surface.
- Commit ladder:
...
- `feedback_plan_before_code` — every build writes a plan-doc to `docs/plans/<slug>.md` BEFORE code.
...
1. Plan-doc authored at `docs/plans/odd-test-altitude-procedural-fix.md`.
2. Manifest authored at `docs/plans/odd-test-altitude-procedural-fix.manifest.yaml` (schema v3).
