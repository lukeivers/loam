...
3. Universal admitted paths: `docs/rebuild/plans/`,
   `docs/odd-methodology.md`, `docs/odd-in-pos.md`, `CLAUDE.md`,
   `docs/rebuild/FUTURE_IDEAS.md`. `data/` stays per-component
   (runtime artefact path, scope-bounded). `docs/rebuild/components/<comp>/`
   stays per-component.
...
`docs/rebuild/plans/amendment-N-<slug>.manifest.yaml`:
...
4. `plan` required path to an existing file under `docs/rebuild/plans/`.
...
`docs/rebuild/plans/amendment-22-pos-amend-cli.manifest.yaml` with
...
10. hands-off-lifecycle (via `test_cross_cutting.py`; handled specially
    ...
    allowed set, but `docs/rebuild/plans/` was already admitted through
...
- `docs/rebuild/FUTURE_IDEAS.md` amendment-dispatch-speedups CDC note
