...
The "13-section shape" is induced from the following recent plans (all under `docs/plans/`):
...
- `RESEARCH_PATH` — derived from slug: `docs/plans/research/<slug>-research.md`; orchestration pre-fills.
...
- `--vars-out <path>` — where the scaffolded vars-file is written. Default: `<repo-root>/docs/plans/<slug>.vars.yaml` — see §5.2 for the path-resolution decision.
- `--plan-out <path>` — where the rendered plan markdown is written if `--render` is passed. Default: `<repo-root>/docs/plans/<slug>.md`.
...
- D-1a (recommended): `<repo-root>/docs/plans/<slug>.vars.yaml` — sibling of where the rendered plan lands. Predictable; co-located with the plan-doc; matches the convention that all plan-related artefacts live under `docs/plans/`.
...
# RESEARCH_PATH: "docs/plans/research/<slug>-research.md"   # auto-filled
...
- **D-1 — Vars-file path:** `<repo>/docs/plans/<slug>.vars.yaml` (recommended) vs `.scratch/` ephemeral vs `tools/pos-amend/scratch/`.
...
- Sibling plan (engine ancestor): `docs/plans/dispatch-prompt-template-extension.md`.
- Sibling plan (seal-automation ancestor for `--plan-doc`): `docs/plans/pos-amend-seal-automation-extension.md`.
...
- Plan corpus surveyed: `docs/plans/memory-system-live-client-and-stop-hook-write.md`, `docs/plans/primary-persona-conversational-onboarding-and-default-archetype.md`, `docs/plans/bootstrap-progress-statusline.md`, `docs/plans/dispatch-prompt-template-extension.md`, `docs/plans/pos-amend-seal-automation-extension.md`, `docs/plans/pos-amend-tracker-integration.md`, `docs/plans/amendment-33-memory-consumer-wiring-primary-persona.md`.
