...
- `memory-system/tests/test_no_sealed_amendments.py` — advance `BASELINE` from `fd7c6cf` to `1b144f6`; extend the BASELINE-history comment block with this amendment's narrative. No `allowed_prefixes` change (the existing tuple already admits `memory-system/` + `docs/plans/` + `data/`).
...
- `docs/plans/d12-chaos-durability-split-pytest.md` — this plan.
