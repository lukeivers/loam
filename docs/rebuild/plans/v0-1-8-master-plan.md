...
| Cycle 4b — canonical Ruby-Rails fixture + DRY refactor | (planned) | — | — | Surfaced from Cycle 4a 4a/4b split per master plan §7.9. Ships canonical ruby-rails-payment full fixture (5–10 routes + ActiveRecord + callbacks + concerns + polymorphic + Sidekiq + ≥10 RSpec tests + LICENSE), AC.FIXTURES.4 ratification e2e on Ruby fixture, AC.FIXTURES.5 LICENSE check, + DRY refactor of repo_sha + slugify + heuristic-inference patterns into `lang/_common/` (RF surfaced in 4a §10 #6). Wall-clock band: 4–8 h (fixture-only cycle; no new adapter code; no new recognizers). |
