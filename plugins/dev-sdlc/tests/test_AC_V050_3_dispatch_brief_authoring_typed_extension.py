...
"""AC.V050.3 — ``dispatch-brief-authoring`` SKILL.md extension for
typed personas.

Per ``docs/plans/v0-5-0-subagent-personas-routing-and-priming.md``
§4 AC.V050.3: the existing ``dispatch-brief-authoring`` SKILL is
...
- Names that briefs dispatched via ``subagent_type: <persona>``
  ...
  per the v0.5.0 audit cross-walk).
...
def test_AC_V050_3_typed_extension_section_present() -> None:
    """The brief-authoring SKILL body contains a section naming
    'When subagent_type is not general-purpose' (AC.V050.3)."""
    body = _load_skill_body()
    assert "When `subagent_type` is not `general-purpose`" in body, (
        "AC.V050.3: SKILL.md must contain a 'When subagent_type is "
        "not general-purpose' section."
    )


def test_AC_V050_3_extension_references_v044_3_ac() -> None:
    """The extension is anchored to AC.V050.3 (so the link between
    SKILL section and locking AC is structural)."""
    body = _load_skill_body()
    assert "AC.V050.3" in body, (
        "AC.V050.3: extension section must cite AC.V050.3 explicitly "
        "for traceability."
    )


def test_AC_V050_3_extension_names_partial_omission() -> None:
    """The extension explicitly notes the omission is PARTIAL (not
    blanket) per the v0.5.0 audit cross-walk — protects against
    ...
    assert has_partial_word or has_per_persona_table, (
        "AC.V050.3: extension must name PARTIAL omission or carry "
        "a per-persona OMIT-OK/propagate table."
    )


def test_AC_V050_3_extension_names_required_brief_slots() -> None:
    ...
    for slot in (
        ...
        assert slot in extension_block_flat, (
            f"AC.V050.3: extension block must name the {slot!r} slot "
            "as still-required when typed."
        )


def test_AC_V050_3_extension_names_skippable_when_typed() -> None:
    ...
    assert skip_signals, (
        "AC.V050.3: extension must acknowledge the per-cycle "
        "re-derivations the brief MAY skip when typed."
    )


def test_AC_V050_3_backward_compat_clause_present() -> None:
    ...
    assert "backward-compat" in body_lower or "backward compat" in body_lower, (
        "AC.V050.3: extension must carry a backward-compat clause "
    ...
    assert "general-purpose" in body, (
        "AC.V050.3: backward-compat clause must reference the "
        "general-purpose default."
    )


def test_AC_V050_3_no_regression_on_AC_DBT_block() -> None:
