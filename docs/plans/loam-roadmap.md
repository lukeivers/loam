...
| **N2 — STATE-OF-LOAM operative-reality record + substrate-audit gate (R-1 + R-3) — sealed** | The standing mechanism that catches built-≠-live drift automatically: a record DERIVED FRESH from ground truth (git refs + seal sidecars + live config + a REAL backend probe — config alone mis-classifies a shim-backed backend as live), the `loam audit` verb, the 8th `ALL_GATES` release-gate (HARD-BLOCK substrate-audit), and the FBM stored-claim-vs-truth reconcile second entry point (ONE comparator, two callers — folds the "memory↔reality reconciliation" candidate). | `framework/tools/loam/src/loam_cli/audit/` (probe/record/comparator/reconcile/loam_state/cli). Gate 8 `check_substrate_audit` in `ALL_GATES` (`framework/tools/loam/src/loam_cli/release/gates.py`). Seal `59a44ca`; amendment `cedc9c4`. Outcome test `test_AC_SOL_PLANTED_outcome_altitude.py` (the REAL `loam audit` verb catches a planted dark-for-live divergence). Live verb run: probe reports FBM `merged`+`wired` — the §6 R-1 finding derived automatically. Amendment #157. |
...
|---|---|---|---|---|---|
| **N1** | **`.loam/` workspace layout + boundary lock (v-next F0.2 + P1.2)** — ✅ **BOUNDARY LOCK SHIPPED (2026-05-31)**: ADR-0001 (`docs/design/adr/boundary-framework-vs-user-state.md`) + the declared allowlist + the `boundary-respected` release gate (gate 9). The `.loam/` **scaffold is already DONE** (P1.2 `01f3b40`: `establish_loam_layout()` + declared dirs + self-describing README) — N1's remaining scope was the LOCK only, NOT a re-scaffold. | Scaffold the per-workspace `.loam/` dir (user-state store, `migrations/` + `.cursor`, profile/rules/objectives homes) AND write the framework↔user-state boundary as an architectural decision record. This is the seam the whole cutover rests on; everything else in the kernel reads/writes through it. | FBM-LIVE (DONE) | **G2 RATIFIED: repo shape = EVOLVE-IN-PLACE** on the existing canonical tree (NOT a clean new tree — supersedes the v-next §3 recommendation; see ADR-0001 §4) + user-state physical home (`~/.claude/` global + `<ws>/.loam/` scoped). | **M** (ADR + 1 gate + test pair; the scaffold was already shipped, so smaller than the original M–L estimate) |
| **N2** | **STATE-OF-LOAM operative-reality record + substrate-audit gate (R-1 + R-3)** — ✅ **SHIPPED (slice 1), seal `59a44ca` — see §2 DONE.** The `loam audit` verb + the generate-fresh ground-truth record + the 8th HARD-BLOCK release-gate + the FBM reconcile entry point landed. **Fast-follows OUT of slice 1** (deferred by ruling): the always-on plan-author PreToolUse hook arm (D2 — owner-class to turn ON; composes on the verb with no rework), NL "rides existing X" prose-scanning (D4 — a clean second slice with its own tighter AC), and auto-HEAL of a stale doc (D3 — deferred until the comparator's false-positive rate is measured). | FBM-LIVE / live state to probe (DONE) | The gate is a process change → owner-gated to turn ON (the always-on authoring hook is the owner-gated fast-follow; the release-gate arm + verb shipped). | M |
| **N3** | **onboarding / init flow (v-next P1.4)** | The "translate-in" intake for a brand-new instance: run the operating loop on a new user, seed initial user-state. This is what Phase 3 dogfoods — building it before the user-model that adapts the state is the right order (the adapter needs something to adapt). **Baseline SEALED (amendment #158, `96aae8a`).** **D-5 deep-role-research fast-follow SEALED LOCAL 2026-05-31** (`docs/plans/deep-role-research-provider.md`): the real `RoleResearchProvider` fills the N3 seam — a bounded `claude -p` 3-axis research-subagent + synthesis to ≤3 person-specific leverage ideas (`is_stub=False`), graceful-degrading, behind `default_research_provider()`; the featherlight AC.ONDEEP.1 gate held; AC.ONDEEP.2 backfilled to PASS. Does NOT re-base the N3→N4 critical path (N4 was never blocked on it — the baseline already seeds). | N1 (`.loam/` to write seeded state into) | Owner-gated flow review. | M–L |
| **N4 — BUILT (SEALED LOCAL 2026-05-31, amendment #159)** | **MVP user-model + config (v-next P1.5) — FLAGSHIP pillar 1, first slice** | The smallest adaptive layer riding the now-live keep-pace hooks: seed `INTERACTION-MODEL.md`, map work-anchor→area, inject the exposure/autonomy cell on the live hook, explicit-override + plain-language inspect. **This is the per-user-tuned-translation engine's first real brick** — the home for FBM rule auto-weighting (infer + surface). **DONE:** new `keep_pace/interaction_model.py` (read/classify/override/inspect/auto-weight) + a fail-soft `n4-interaction-model` contributor on the EXISTING keep-pace hook (no new hook) + the enforce-path `exposure=` threshold on the KP9 draft-gate (D-N4.2 INCLUDED). Forks D-N4.1..4 ruled (deterministic classifier / enforce-path IN / override hard-set+locked / auto-weight surface-for-confirm). G5 openness-default honoured. MVP fence held — cells move ONLY by explicit statement; zero behavioural instrumentation. 44 tests GREEN incl. the outcome-altitude AC.UM.READ.4 at the real hook. | N3 (state to adapt) + live hooks (DONE) | **Yes — G5: ratify the openness-default revision of `abstraction_first_default.md`** before seeding (reverses a Luke-tuned `minimal` default). **PRE-RULED in the build dispatch.** | M–L |
...
| **Full adaptive user-model (FLAGSHIP pillar 1, remainder)** — **UNBLOCKED 2026-05-31 (N4 predecessor satisfied)** | Behavioural signal counters + hysteresis (dark-launch first), fast-down-on-distress, tone + learning-appetite axes, weekly re-eval consolidation + drift judge. The full per-user engine. | N4 (MVP user-model) — **BUILT** | partial (behaviour changes owner-gated) | L |
...
| **Non-tech-user self-recovery** — **FIRST SLICE BUILT + SEALED LOCAL 2026-05-31** (seal `73090c97`, `self-correction` fence; in-fence parts shipped — distress-detection hook + self-diagnosis + watchdog + plain-language recovery + safe backup-first/fail-closed FBM reset + `loam recover` verb; matrix-row backfill F-4 a dispatcher-owned follow-on; live `settings.json` wiring owner-gated) | distress-as-fire-alarm → forced self-diagnosis → watchdog hooks → plain-language recovery → safe hard-reset on the user-state store. Shares the distress detector with the user-model's fast-down trigger (that shared detector is the one now built). | N1 (state to reset) + full user-model (reads recovery-pitch level) | partial | M–L |
...
| Item | What + why | Dependency | Owner-gated? | Size |
|---|---|---|---|---|
| **memory↔reality reconciliation mechanism** | ✅ **SUBSUMED-by-N2 — the fold landed (slice 1, seal `59a44ca`).** The detect half is built: the SAME comparator that catches a doc's stale status (`compare_claim`) also reconciles a checkable stored FBM claim against ground truth (`reconcile.reconcile_stored_claim` — ONE comparator, two callers, AC.SOL-RECONCILE). The HEAL half (auto-correct a stale doc/episode) is the deferred D3 fast-follow (gated on measuring the comparator's false-positive rate first). No longer tracked separately. | live state | n | M (detect: DONE; heal: deferred) |
| **PyPI publish / conventional install** (flatten target stale — see note) | Publish to PyPI so `pip install loam; loam init` (or `pipx install loam`) works (~9 h, the v0.2 documented target, blocked solely by no-publish-yet — the dependencies-only meta-package is already built and proven against a local wheelhouse; only the public flip remains). **Flatten note (2026-06-01 health check):** the `framework/framework` cosmetic doubling this row used to pair with **no longer exists in the canonical tree** (`find` returns nothing) — the flatten is either already done or never present in this layout; do NOT schedule a flatten cycle for a directory that isn't there. PyPI publish is the only live half of this row. | none (hygiene) | PyPI account/credential steps owner-gated | L (PyPI) |
...
| **skill triage + loam↔Claude-primitives integration review** | ✅ **skill-triage REMAINING scope DONE (foundation-polish cluster SUB-ITEM 4, amendment #162).** Verified every installed `plugins/loam-skills/` skill's trigger fires on its intended NL shape (deterministic trigger-match, AC.SKTRI.1) + enforced the live-set invariant / no-live-consumer retirement gate (AC.SKTRI.2). TRIAGE OUTCOME: all 22 installed skills verified-KEPT; zero retirements (each carries a firing trigger + ≥3 live consumers; none superseded/non-functional — the surface is already the live set, not a graveyard). **STILL OPEN:** the loam↔Claude-primitives integration review (which primitives loam should compose on vs re-implement) + the automated capability-adoption loop (separate roadmap lane; this triage was the manual first pass). | none | n | S–M |
