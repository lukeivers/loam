...
**Authority documents:**
- `docs/plans/loam-rename-decisions.md` Tier-1 items 2 + 3 (per-host config dir + env-var prefix).
...
- `docs/plans/oss-v0-1-0-publish.md` §5 (M1 row needs replacement with M1a..M1g ladder; **M1b includes that precursor doc-only commit per series-master §11 / M1a's recommendation**).
...
- `POS_V2_*` references inside historical plan-docs at `docs/plans/*` — historical method-record; preserved (consistent with M1a's same exclusion).
...
`docs/plans/oss-v0-1-0-publish.md` §5's `M1.rename` row is replaced with seven rows (M1a..M1g) that mirror the series-master §2 ladder. Each row carries its own description, AC reference, AI-time range, and midpoint. The programme total at the end of §5 updates to reflect the post-split sum.
...
- The programme master plan §5 update at `docs/plans/oss-v0-1-0-publish.md`.
...
- No `docs/plans/*.md` historical method-record edits.
...
- **Historical plan-docs stay.** `docs/plans/*.md` files not authored by this amendment are preserved (consistent with M1a).
...
- **Historical plan-docs** at `docs/plans/*.md` (other than this plan-doc + this manifest + the programme master plan §5 update) — preserved.
...
- **Spec docs** at `docs/spec/pos-v2-*.md` (filename + content) — M1e.
...
3. **Programme master plan §5 update commit (precursor doc-only).** Replace the M1.rename single row in `docs/plans/oss-v0-1-0-publish.md` §5 with the seven-row M1a..M1g ladder. Include each sub-amendment's wall-clock estimate (taken from series-master §2 commentary; M1a actuals from `oss-v0-1-0-publish-rename-1a.md`). Update §5's programme-total prose. **Doc-only commit. Lands first.**
4. **M1b sub-plan + manifest commit.** This plan-doc + a manifest YAML at `docs/plans/oss-v0-1-0-publish-rename-1b.manifest.yaml` per the established M1a-precedent shape.
...
- **Prior sub-amendment:** `docs/plans/oss-v0-1-0-publish-rename-1a.md` (sealed `143d465`).
- **Authority documents (inherited from series master):**
  - `docs/plans/loam-rename-decisions.md` Tier-1 items 2 + 3.
  - `.scratch/claude-output/loam-rename-migration-plan.md` §2.5 + §3.3.
- **Programme master plan:** `docs/plans/oss-v0-1-0-publish.md` (M1b includes §5 ladder-replacement precursor commit).
...
- **Precedent multi-component sealed-amendment manifests:**
  - `docs/plans/oss-v0-1-0-publish-rename-1a.manifest.yaml` (M1a sibling — establishes the sub-amendment manifest shape under the rename series).
  - `docs/plans/single-framework-restructure.manifest.yaml` (multi-component fence; universal-paths pattern).
