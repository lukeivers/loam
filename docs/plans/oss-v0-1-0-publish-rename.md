...
- `docs/plans/oss-v0-1-0-publish.md` §5 (M1 row needs re-pricing post-split — flagged for the dispatcher in §13).
...
Each row is an independently-sealed amendment authored against its own plan-doc. **Each carries its own AC family** (`AC.RNM-1a.*`, `AC.RNM-1b.*`, …) so AC-prefix collisions across sub-amendments are structurally impossible. Sub-plans live alongside this master at `docs/plans/oss-v0-1-0-publish-rename-1<letter>.md`.
...
- **`docs/spec/pos-v2-objectives-spec.md` / `pos-v2-rebuild-proposal.md` filename changes.** Filename changes fold into M1e for the same reason. M1a updates content prose only; filenames stay until M1e.
...
- **Authority documents:**
  - `docs/plans/loam-rename-decisions.md` (locked Tier-1 + Tier-2 + Kept-Technical catalogue).
  - `.scratch/claude-output/loam-rename-migration-plan.md` (research; mechanics + dependency ordering).
  - `docs/plans/oss-v0-1-0-publish.md` §5 (M1 row needs re-pricing post-split).
...
- **Precedent multi-component sealed-amendment manifests:**
  - `docs/plans/single-framework-restructure.manifest.yaml` (three-component fence; universal-paths pattern).
