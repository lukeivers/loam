"""Tests for AC.D-np.6 — pre-existing pos-amend behaviour byte-identical.
...
Per `docs/rebuild/plans/pos-amend-new-plan-orchestration.md`:
    ...
    the post-extension tree without modification. ``pos-amend validate``,
    ``pos-amend apply [--dry-run]``, ``pos-amend seal``, ``pos-amend
    ...
    new third-party dependency on ``tools/pos-amend/pyproject.toml``.
"""
...
def test_AC_D_np_6_existing_template_engine_module_unchanged_at_api_surface() -> None:
    ...
    deletion or rename.
    """
    from pos_amend.template_engine import (  # noqa: F401
