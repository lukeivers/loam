...
def test_P6_settings_json_timeout_is_seconds_range() -> None:
    """The ship-time canonical-dev settings template must carry a
    timeout in the same seconds range — otherwise the stanza this
    file ships as contradicts the stanza-builder the post-self-retire
    flow writes.

    Post-D.1 (amendment #61): the tracked dev template moved from
    ``<root>/.claude/settings.json`` to
    ``framework/hands-off-lifecycle/canonical-dev/settings.dev-template.json``
    per the directory restructure (plan §13 finding 1).
    """
    template_path = (
        REPO_ROOT
        / "framework"
    ...
    assert isinstance(timeout, int)
    assert 1 <= timeout <= 600, (
        f"ship-time settings template timeout {timeout} is outside the seconds range"
    )
