...
2. **ODD covers adjacent ground at a different altitude — honestly: a weaker runtime guarantee.** Outcomes' rubric-driven grader iteration ≈ ODD's outcome-altitude AC verification, but ODD operates at **plan-doc authoring time** (the AC family is locked before code commits) while Outcomes operates at **runtime per-iteration**. ODD's mechanical enforcement (lint, sealed-component discipline, `feedback_test_outcome_altitude_required`) gives strong authoring-time + seal-time guarantees, but it does not replicate Outcomes' per-iteration runtime grading — the divergence trades that runtime guarantee away, deliberately, for the no-API-key constraint. (Restated 2026-06-10 per the KEEL Phase-1 honesty pass; the original "equivalent or stronger guarantees" wording overclaimed.) For users without API access, ODD is the only path; for users with API access, ODD is upstream of Outcomes.
...
The framing is honest about the trade: ODD operates at a different altitude with a **weaker runtime guarantee** than Outcomes' per-iteration grader, chosen deliberately for the no-API-key constraint. The framing is "loam chose subscription-only to reduce translation burden, and that choice is consistent with the build-software-with-LLMs prime objective per `docs/VALUE_PROPOSITION.md`."
