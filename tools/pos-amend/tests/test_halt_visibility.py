...
Halt diagnostics emitted by ``pos-amend seal`` and ``pos-amend
...
Note: ``pos-amend template render`` is exempt by AC.D-tpl.5 — the
...
def _make_minimal_repo_with_dirty_tree(tmp_path: Path) -> Path:
    ...
    _git(repo, "config", "user.name", "pos-amend halt test")
...
def test_AC_PA_hv_seal_dirty_tree_halt_visible_on_stdout(
    ...
    """``pos-amend seal`` with a dirty tree must emit a
...
def test_AC_PA_hv_new_plan_invalid_slug_halt_visible_on_stdout(
    ...
    """``pos-amend new-plan`` with an invalid slug must emit a
...
def test_AC_PA_hv_template_halt_remains_stderr_only(capsys) -> None:
    """``pos-amend template render`` halt path is exempt from the
