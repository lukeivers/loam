...
**Companion:** `docs/plans/pos-amend-tracker-integration.md` (the OTHER queued pos-amend extension; sequencing decision in §11 D-6).
**Prior dev-discipline plan precedents:** `docs/plans/pos-amend-install-instructions-fix.md`, `docs/plans/orphan-plist-cleanup-install-instructions-fix.md`, `docs/plans/pos-amend-tracker-integration.md`.
...
**Test shape:** run the full `tools/pos-amend/tests/` suite at the post-extension tree; assert green. Verify against all `docs/plans/amendment-*.manifest.yaml` files: `pos-amend apply --dry-run` exits 0 on each (sample-tested at fixture-injection time on representative manifests; full enumeration at integration-test time).
...
4. Write builder-plan to `docs/plans/pos-amend-seal-automation-extension.builder-plan.md` naming specific files + symbols expected to be touched.
...
- **D-7a. `docs/FUTURE_IDEAS.md` amendment-dispatch-test-scope CDC** — add a sentence noting the post-amendment-commit cycle is now mechanised inside `pos-amend seal`. Single-paragraph edit.
...
- `pos-amend apply --dry-run docs/plans/amendment-40-*.manifest.yaml` exits 0 at a clean post-extension tree (i.e. with no uncommitted edits to `tools/pos-amend/` itself).
...
- D-7 follow-on doc edits (`docs/FUTURE_IDEAS.md` CDC text + dispatch-template update) are queued as separate small commits, not bundled here.
