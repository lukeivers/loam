...
**Parent plan:** `docs/plans/eric-final-delivery-plan-2026-05-04.md` (§2 v0.1.8 row).
**Companion research (load-bearing):**
- `docs/plans/eric-saas-app-use-case-version-sequence-2026-05-04.md` (Eric path)
- `docs/plans/layered-skill-story-research-2026-05-04.md` (skills path; §5 = candidate dev-sdlc SKILLs)
...
- Master plan: `docs/plans/v0-1-8-master-plan.md` — §3 Cycle 1 scope.
- Eric synthesis: `docs/plans/eric-final-delivery-plan-2026-05-04.md` §2 v0.1.8 row.
...
Author at: `docs/plans/v0-1-8-cycle-1-odd-extractor-scaffolding.md`
Manifest at: `docs/plans/v0-1-8-cycle-1-odd-extractor-scaffolding.manifest.yaml` (schema v3; plan_doc_ref pointing at the plan-doc)
...
- Backfill `docs/plans/v0-1-x-roadmap.md` §8 method-decision register row for v0.1.8 Cycle 1.
- Backfill `docs/plans/eric-final-delivery-plan-2026-05-04.md` §2 v0.1.8 progress notes after seal.
...
- Master plan: `docs/plans/v0-1-8-master-plan.md` — §3 Cycle 2 scope.
...
Author at: `docs/plans/v0-1-8-cycle-2-bands-and-ratification.md`
Manifest at: `docs/plans/v0-1-8-cycle-2-bands-and-ratification.manifest.yaml`
...
- Master plan: `docs/plans/v0-1-8-master-plan.md` — §3 Cycle 3 scope.
...
Author at: `docs/plans/v0-1-8-cycle-3-ruby-rails-adapter.md`
Manifest at: `docs/plans/v0-1-8-cycle-3-ruby-rails-adapter.manifest.yaml`
...
- Master plan: `docs/plans/v0-1-8-master-plan.md` — §3 Cycle 4 scope.
...
Author at: `docs/plans/v0-1-8-cycle-4-jsts-playwright-adapter-and-fixtures.md`
Manifest at: `docs/plans/v0-1-8-cycle-4-jsts-playwright-adapter-and-fixtures.manifest.yaml`
...
- Master plan: `docs/plans/v0-1-8-master-plan.md` — §3 Cycle 5 scope.
...
Author at: `docs/plans/v0-1-8-cycle-5-dev-sdlc-skills-pass-1.md`
Manifest at: `docs/plans/v0-1-8-cycle-5-dev-sdlc-skills-pass-1.manifest.yaml`
...
- **Master plan source authority:** `docs/plans/eric-final-delivery-plan-2026-05-04.md` §2 v0.1.8 row + Decision O + Decision F.
- **Layered-skills first-pass list:** `docs/plans/layered-skill-story-research-2026-05-04.md` §5 + parent §2 v0.1.8 row.
...
- **FIDRAFT V11.C entry:** `docs/FUTURE_IDEAS_DRAFT.md` (search "ODD-reverse-engineering"; defers heavy V11.C to v0.1.4+ — now realised at v0.1.8).
...
| Cycle 4a — JS/TS/Playwright adapter + JsTs fixture | sealed | `07033d3` | `67dd302` | Single-component fence (dev-sdlc/odd-extractor). Plan-doc `5ed881d`; source-edit BASELINE `c2356a6`; §14 SHA backfill `79f6960`. AC.JSTS.{1..5} + AC.FIXTURES.{1, 3-jsts} all green; 99 new tests on dev-sdlc side (304 total) all pass. D1+D2-idempotency+D5+D6 smoke exercised against jsts-playwright-app fixture (≥3 VERIFIED + ≥5 PLAUSIBLE + ≥2 HYPOTHESISED satisfied: actual was 12V+38P+10H = 60 ACs); D3/D4 n/a (one-shot CLI). Multi-grammar tree-sitter dispatch (`tree-sitter-javascript` for .js/.mjs/.cjs/.jsx; `tree-sitter-typescript` `language_typescript()` for .ts; `language_tsx()` for .tsx). Both ESM and CommonJS module shapes parsed. 8 idiom recognizers: Express routes / Playwright tests / Playwright page objects / TS types / Zod schemas / class-validator decorators / Jest-Mocha-Vitest test runners / plain HTML/JS file-level. Heuristic-shaped HYPOTHESISED inference (5 patterns; no LLM). Slicer + heuristic_inferences + repo_sha local copies; SliceDriftError reused from Ruby. Per-file routing extension to Cycle 3's analyze.py _LANGUAGE_HINTS table (additive). Synthetic ~17-file jsts-playwright-app fixture matches Eric-first-project shape (TS Playwright + JS Express + HTML/JS + Zod + class-validator + ≥10 tests across runners + ESM/CJS mix). odd-methodology.md §13 added (per-language adapter conventions — JS/TS/Playwright second). Cycle 4 was split per master plan §7.9 + dispatch halt-trigger: 4a (this) ships JS/TS adapter + JsTs fixture + e2e smoke; 4b ships canonical Ruby-Rails-payment fixture + Ruby e2e + DRY refactor across language adapters. Wall-clock under the 10h halt-trigger; no in-flight halt-and-surface needed. Python adapter deferred to v0.2.2+. |
| Cycle 4b — canonical Ruby-Rails fixture + DRY refactor | sealed | `042c3e1` | `c648cf9` | Single-component fence (dev-sdlc/odd-extractor). Plan-doc `3f8a8d1`; source-edit BASELINE `c3c5afc`. AC.FIXTURES.{2, 3-ruby, 4-ruby, 5} + AC.DRY.{1..4} all green; 88 new tests on dev-sdlc/odd-extractor side (392 total) all pass; 71 dev-sdlc parent tests pass (463 total). D1+D2-idempotency+D5+D6 smoke exercised against canonical ruby-rails-payment fixture; D3/D4 n/a (one-shot CLI). Canonical ruby-rails-payment fixture (23 files; 5 RESTful resources; 3 ActiveRecord models with callbacks/concerns/polymorphic; 2 Sidekiq jobs; 3 controllers; 3 migrations; ≥10 RSpec specs; Apache-2.0 LICENSE; SYNTHETIC banner) co-exists with Cycle 3's intentionally-thin synthetic-rails fixture. Band distribution against canonical fixture: 15 VERIFIED + 48 PLAUSIBLE + 4 HYPOTHESISED = 67 ACs (well above the ≥3+5+2 floor). DRY refactor closes Cycle 4a §10 RF #6: `lang/_common/` NEW subpackage exposes `repo_sha.resolve_repo_sha` (per-adapter repo_sha.py DELETED), `slugs.{slugify, file_slug}` (compat shim re-export at per-adapter `_ast_utils.py`), and `heuristic_helpers.make_inferred_banded_ac()` constructor (both adapters' `heuristic_inferences.py` refactored to call it). 17 recognizer modules (8 jsts + 9 ruby) migrated to import slugify/file_slug from `..._common.slugs` directly (canonical path; AC.DRY.4). `SliceDriftError` + `aggregate_slice_results` deferred to Cycle 5+ per Surface #5 (already DRY: one home + re-exporter). odd-methodology.md §13.11 updated to reflect closure. JsTs LICENSE addition (Apache-2.0; AC.FIXTURES.5 jsts portion) verified missing on Cycle 4a HEAD; added in 4b. Behaviour-preservation halt-trigger: 304 odd-extractor + 71 dev-sdlc parent tests at HEAD `67dd302` all pass post-refactor (375 baseline preserved). Wall-clock under 8h halt-trigger; no in-flight halt-and-surface needed. |
| Cycle 5 — 6 SKILLs first pass | sealed | `5ceffe4` | `e4512b9` | Single-component fence (dev-sdlc). Plan-doc `68f5da1`; source-edit BASELINE `1d5c75f`; §14 SHA backfill `cbc600d`. AC.SKILLS-DSDLC1.{1..8} all green; 38 new tests on dev-sdlc side (109 total) + 392 odd-extractor tests preserved (501 total) all pass. D1+D2 smoke exercised live against tmp-workspace+canonical-plugins layout (14 SKILLs auto-symlinked: 8 from loam-skills + 6 new from dev-sdlc; idempotent on re-run); D3/D4 n/a (filesystem state); D5 inherited from Anthropic native discovery; D6 inherited from v0.1.7 Cycle 3 audit-trail floor. Six SKILL.md packages at `plugins/dev-sdlc/skills/<name>/SKILL.md`: `loam-amend-cycle` (sealed-component amendment workflow) / `dispatch-brief-authoring` (dispatch-brief structural shape) / `plan-before-code-author` (ODD-shaped plan-doc skeleton) / `fidraft-capture` (FUTURE_IDEAS_DRAFT entry shape) / `front-load-principle-walk` (turn-start principle re-citation ritual) / `audit-finding-triage` (four-bucket triage of agent halt-and-surface findings). Each SKILL: frontmatter `description` ≤1536 chars (Anthropic combined-cap); body covers What captures / When to use / How persona applies it / Graceful degradation / Composition / Out of scope. Re-evaluation per master plan §7.4 confirmed first-six list intact (every SKILL exercised ≥5× in Cycles 1–4b ritual usage; no swap warranted). Wall-clock well under 5h halt-trigger; no in-flight halt-and-surface needed. v0.1.8 release-level smoke gate runs next per master plan §5. |
