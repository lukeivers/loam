...
**Status:** v0.6.1 — landed at canonical 2026-05-08 from pos3 forward-staging draft (Stage 2 deliverable of foundation revision, 2026-05-02). Path resolved to canonical's existing `docs/design/` directory.
...
1. **Show, for each principle in the corpus, whether F4 (scope ↔ confidence) informs it (compose-with), is unrelated to it (independent), or partially overlaps (partial).** This is *not* a strict derivation hierarchy — the owner ruling during the foundation discussion was that F4 is most-broadly-applicable but NOT a first axiom from which all other principles derive.
...
## Existing principles (26 + the 3 newly-codified)

Verified count via `ls feedback_*.md /Users/lukeivers/.claude/projects/-Users-lukeivers-pos3/memory/`: 26 files pre-foundation-revision, 2 added by Stage 2 (F4 + M5), 1 added by Stage 3a (F3), 1 added by Stage 4 (F2). F2 and F3 are now in corpus; no forward-references remain.
...
## F3 + F2 — now in corpus (both forward-refs upgraded)

F3 was a forward-reference in Stage 2. Stage 3a landed `feedback_swarming_recursive_decomposition.md`. Row promoted from forward-ref section to corpus below.
...
**Top three compose-with-F4 compositions in F3 (for cross-reference):**
...
- Total principles in corpus (post-Stage 4): 30 (26 existing + F4 + M5 + F3 + F2).
...
Two of the 28 classifications are flagged AMBIGUOUS — surfaced for owner ruling rather than auto-classified:
...
The remaining 26 classifications are stated with reasonable confidence; the justifications above are the basis. If the owner reads the table and disagrees on any specific label, the disagreement itself is M5 input — name the conflict, the signals weighted differently, and re-rule.
...
*Authored by Stage 2 Opus dispatch, foundation revision, 2026-05-02. Landed on canonical pos-v2 at v0.6.1 (2026-05-08) per `docs/plans/v0-6-1-foundation-docs-plan.md`.*
