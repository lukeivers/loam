...
|---|---|---|---|---|
| Cycle 1 — PR-safety gate engine + override workflow | (planned) | — | — | NEW component `plugins/dev-sdlc/pr-safety/`. Per-band gating engine (3×3 decision matrix); override-commit recognition (`contract-update:` prefix); CLI `loam pr-safety gate`; SOC-2 audit-log; production-stake integration. |
| Cycle 2 — Hook installers + CI templates + PR description template | (planned) | — | — | Single-component fence on `plugins/dev-sdlc/pr-safety/`. Pre-commit + pre-push installers; 3 CI templates (GitHub Actions / GitLab CI / CircleCI); provenance-traceable PR description template; `loam pr-safety install --all`. |
