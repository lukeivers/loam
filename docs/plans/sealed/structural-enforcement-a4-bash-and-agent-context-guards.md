...
**A4 research:** `docs/plans/research/structural-enforcement-a4-bash-and-agent-context-guards-research.md` (sibling artefact authored alongside this plan).
...
**Pre-flight verification (mandatory; per `feedback_verify_dispatch_before_sending`):** pre-A4-build dispatch verifies `git log --grep="A4|bash.guard|agent.guard|bash-and-agent"` returns no A4 amendment commit AND `ls docs/plans/ | grep -iE "a4|bash.guard|agent.guard"` returns nothing matching `structural-enforcement-a4-*` other than this plan-doc + its sibling research. Halt-and-surface if either does.
...
- **AC.A4.S — Seal-diff confined to fence.** A4's seal-diff window contains only edits under `framework/hands-off-lifecycle/{hooks,tests,seals}/` and the universal-paths admissions (`docs/plans/`, `CLAUDE.md`, `docs/odd-methodology.md`, `docs/odd-in-pos.md`, `docs/FUTURE_IDEAS.md`, `docs/FUTURE_IDEAS_DRAFT.md`). Per-invariant frozen-both-endpoints BASELINE pattern (per `docs/odd-in-pos.md` §10.3) for the A4 invariant test.
...
- **Plan-doc:** this file at `docs/plans/structural-enforcement-a4-bash-and-agent-context-guards.md`.
- **Research artefact:** `docs/plans/research/structural-enforcement-a4-bash-and-agent-context-guards-research.md` (this dispatch).
...
- **A1 plan (sealed substrate):** `docs/plans/structural-enforcement-a1-substrate.md`.
- **A2 plan (sealed sibling gate):** `docs/plans/structural-enforcement-a2-objective-binding-gate.md`.
- **A3 plan (sealed sibling gate + helper-library precedent):** `docs/plans/structural-enforcement-a3-tdd-guard.md`.
- **A3 builder plan (D-build precedent A4 mirrors):** `docs/plans/structural-enforcement-a3-tdd-guard.builder-plan.md`.
...
- **Manifest:** authored alongside the builder plan at `docs/plans/structural-enforcement-a4-bash-and-agent-context-guards.manifest.yaml`. Single-component manifest (`hands-off-lifecycle`); `frozen_baseline: true` (H19 frozen since project-start). Universal-paths block as standard.
- **Pos-amend bookkeeping flow** (per `feedback_dispatch_explicit_pos_amend_apply`):
  1. Author manifest at `docs/plans/structural-enforcement-a4-bash-and-agent-context-guards.manifest.yaml` with the correct BASELINE (HEAD~1 of the upcoming amendment commit per the established #29/#34/.../#71 pattern).
  ...
  7. `pos-amend seal --plan-doc /Users/lukeivers/ivers-corp-pos-v2/docs/plans/structural-enforcement-a4-bash-and-agent-context-guards.builder-plan.md <manifest>` — runs the scoped test sweep, creates the seal commit, advances SEAL_COMMIT to the seal commit, appends builder-plan §SHA backfill follow-up commit.
...
- A4 research (sibling): `docs/plans/research/structural-enforcement-a4-bash-and-agent-context-guards-research.md`
- A1 plan (sealed; substrate this builds on): `docs/plans/structural-enforcement-a1-substrate.md`
- A2 plan (sealed; sibling gate, single-matcher precedent): `docs/plans/structural-enforcement-a2-objective-binding-gate.md`
- A3 plan (sealed; sibling gate, helper-library precedent): `docs/plans/structural-enforcement-a3-tdd-guard.md`
- A3 builder plan (D-build precedent): `docs/plans/structural-enforcement-a3-tdd-guard.builder-plan.md`
...
- VALUE_PROPOSITION: `docs/VALUE_PROPOSITION.md` (Lens 2 anchor)
