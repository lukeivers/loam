...
- `docs/plans/loam-rename-decisions.md` Tier-1 item 4 (com.pos-v2.* → com.loam.*; version-suffix-drop ruling).
...
- `docs/plans/**` — all 16 plan-doc refs (most pre-this-amendment; this plan-doc itself adds 1+ during authoring).
- `docs/archive/component-research/**` — 10 component-narrative refs (historical proposals).
...
`docs/plans/loam-rename-decisions.md` Tier-1 item 4: "`com.pos-v2.<slug>.*` launchd labels → `com.loam.<slug>.*` — the version suffix is dropped concurrently (no `v1` to differentiate)."
...
AC family **AC.RNM-1c-fix.\*** (per dispatch — doesn't collide with existing AC families). Each AC ladders to AC.OSS-M1.S (M1.rename programme seal) → AC.OSS.6 (final scrub) → AC.PO.1 + AC.PO.2 (prime objective per `docs/VALUE_PROPOSITION.md`).
...
- Pre-rename plan-doc references in `docs/plans/` describing past work.
...
Three FIDRAFT entries appended to `docs/FUTURE_IDEAS_DRAFT.md`:
