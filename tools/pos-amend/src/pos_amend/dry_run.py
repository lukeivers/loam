...
def _cross_component_prefixes(manifest: Manifest, self_name: str) -> set[str]:
    """Return the set of partner-component top-level prefixes for *self_name*.
    ...
    manifest's component set. Names map 1:1 to top-level dir prefix
    (e.g. ``cost-governance`` → ``cost-governance/``).
    """
    return {f"{c.name}/" for c in manifest.components if c.name != self_name}
