...
`docs/FUTURE_IDEAS.md` in `/Users/lukeivers/ivers-corp-pos-v2/`
...
1. `docs/FUTURE_IDEAS.md` gains a new `## Idea 6 — ...`
...
- `docs/FUTURE_IDEAS.md` (additive only — new Idea 6
  appended; Ideas 1–5 untouched; CDC block untouched; Catalogue
  discipline untouched).
- `docs/plans/odd-as-default-framing-idea.md` (this plan).

## Validation

1. `grep -n "Idea 6" docs/FUTURE_IDEAS.md` returns at least
   one hit at an `## Idea 6` header.
2. `grep -n "VALUE_PROPOSITION.md" docs/FUTURE_IDEAS.md` and
   `grep -n "odd-methodology.md" docs/FUTURE_IDEAS.md` each
   return a hit within the new Idea 6 section.
3. `grep -n "Idea 2" docs/FUTURE_IDEAS.md` returns the
   existing Idea 2 header plus at least one cross-reference from
   within Idea 6.
4. `git diff --name-only HEAD` after the edit lists exactly two
   paths: `docs/FUTURE_IDEAS.md` and
   `docs/plans/odd-as-default-framing-idea.md`.
