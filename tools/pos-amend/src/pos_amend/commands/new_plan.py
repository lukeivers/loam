"""``pos-amend new-plan <slug>`` — scaffold a vars-file for a new plan-doc.

Surface (per ``docs/rebuild/plans/pos-amend-new-plan-orchestration.md``):

- ``pos-amend new-plan <slug>`` — write a YAML vars-file at
  ...
  ``pos-amend template render plan/dev-discipline --vars-file <…>``
...
def _vars_file_content(slug: str, title: str, ac_prefix: str) -> str:
    ...
    return f"""# pos-amend new-plan scaffold for {slug}
# Edit this file, then render with:
#   pos-amend template render plan/dev-discipline --vars-file <this-file> --out docs/rebuild/plans/{slug}.md --force
...
SECTION_9_BODY: |
  ...
  "Bookkeeping surface" and include a `pos-amend` manifest YAML stub)
HALT_TRIGGERS: |
...
def run(
    ...
    *,
    ...
    render
        ...
        ``pos_amend.commands.template.run("render", ...)``).
    force
