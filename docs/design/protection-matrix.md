...
| **FM.SECRET-LEAK** — Commits secrets or .env files | the secret-pattern guard hook + the dangerous-flag guard hook — now EXTENDED (AC.SBB.1, secure-build baseline) with a staged-diff scan at the commit/push boundary: a `git commit` / `git push` triggers a scan of the content the boundary would publish (the staged diff for commit, the unpushed range for push) for the same credential shapes, catching a secret embedded in a FILE — not just on the command line — and firing for the artifact loam BUILDS (any repo under cwd), not only loam's own. The staged-diff path is strictly additive (the inbound paste path's fail-OPEN convention is preserved) and the secret floor is non-tunable (AC.SBB.4 — its strictness cannot be downgraded). | hook | floor | YES | — |
...
| **FM.VULN-DEPENDENCY-SHIPPED** — Ships a build with a known-vulnerable dependency | the secure-build baseline dependency-hygiene gate — on a build / publish command in a supported ecosystem (Node/Next, Python) the floor runs the ecosystem's OWN audit tool (`npm audit` / `pip-audit`; Lens 1 compose, never a re-implemented vuln DB) and, on a vulnerability at or above a configured severity floor, blocks-or-surfaces per the configured strictness; a clean audit passes silently; an audit tool that is unavailable is SURFACED honestly (never reported as clean for an audit that did not run). Strictness (block vs surface) is the only tunable. | hook | floor | YES | — |
| **FM.ARTIFACT-LEAKS-RUNTIME-STATE** — Sweeps harness working state into the shipped artifact | the secure-build baseline artifact-cleanliness guard — a generated project carries the floor `.gitignore` (harness runtime state + secrets), and a pre-commit sweep inspects a broad-stage command's target repo for runtime-state / secret paths that are present and NOT git-ignored (would enter the artifact under `git add -A`), then blocks-or-surfaces per strictness. Sample/example secret files are exempt. The secret-never-committed half is the non-tunable floor (AC.SBB.4); artifact-cleanliness strictness (block vs surface) is the tunable. | hook | floor | YES | — |
