...
schema-v2 manifest YAML in-line and invoke ``loam amend apply`` /
``loam amend seal`` against it.
    ...
    *,
    ...
    data: dict = {
        ...
        "components": [
            {
                "name": c,
                "seal_test": f"framework/{c}/tests/test_no_sealed_amendments.py",
                "sidecar": f"framework/{c}/tests/SEAL_COMMIT",
            }
            for c in components
        ],
...
def test_AC_D_pa_3_seal_source_commit_idempotent(tracker_repo) -> None:
    ...
    from loam_cli.amend.tracker_registration import update_source_commits
...
def test_AC_D_pa_5_corrupt_tracker_db_halts_with_diagnostic(
    ...
    seal_test = (repo / "framework" / "alpha" / "tests" / "test_no_sealed_amendments.py").read_text()
...
def test_AC_D_pa_5_missing_tracker_parent_dir_halts(
    ...
    """
    import loam_cli.amend.tracker_registration as tr_mod
