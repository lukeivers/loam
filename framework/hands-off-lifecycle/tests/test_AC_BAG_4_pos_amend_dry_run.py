...
``pos-amend apply --dry-run`` would fail (DEV-MODE-only).
...
the hook invokes ``pos-amend apply --dry-run <manifest>``; on exit
...
def test_AC_BAG_4_amendment_shape_dry_run_failure_denies(
    ...
    assert decision.failure_class == "pos-amend-dry-run-failure"
    assert "AC.BAG.4" in decision.reason
    assert "BASELINE pin failed" in decision.reason
    assert "pos-amend" in decision.reason
...
def test_AC_BAG_4_non_amendment_commit_admitted(
    ...
    monkeypatch.setattr(bash_guard, "_pos_amend_dry_run", fake_dry_run)
    monkeypatch.setattr(
        bash_guard,
        "_candidate_manifest_paths",
        lambda _ws, _s: [tmp_path / "manifest.yaml"],
