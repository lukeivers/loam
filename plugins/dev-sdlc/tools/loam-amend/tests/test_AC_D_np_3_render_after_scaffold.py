...
Per `docs/plans/pos-amend-new-plan-orchestration.md`:
    ...
    ``<repo>/docs/plans/<slug>.vars.yaml`` AND a rendered plan-doc
    at ``<repo>/docs/plans/<slug>.md``. The rendered plan-doc
...
def test_AC_D_np_3_render_writes_both_files(tmp_path: Path) -> None:
    ...
    assert rc == 0
    vars_path = tmp_path / "docs" / "plans" / "example-slug.vars.yaml"
