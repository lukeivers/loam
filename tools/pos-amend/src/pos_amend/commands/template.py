"""``pos-amend template`` — markdown template engine for high-repetition
...
- ``pos-amend template list`` — enumerate registered templates by family.
- ``pos-amend template render <family>/<id> [--var KEY=VALUE]... [--vars-file PATH] [--out PATH] [--force]``
- ``pos-amend template validate <family>/<id>``

Templates live under ``tools/pos-amend/templates/<family>/<id>.md`` by
default; tests inject an alternate root via ``--templates-root``.

Exit code mapping (existing pos-amend taxonomy, no new codes — AC.D-tpl.5):
...
from pos_amend.template_engine import (
    ParsedTemplate,
    TemplateError,
    TemplateNotFound,
    TemplateMalformed,
...
# path resolves to ``<repo>/tools/pos-amend/templates/`` when pos-amend
