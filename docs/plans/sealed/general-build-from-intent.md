...
7. **OA run 2's 415.1s "heartbeat breach" was system sleep, not silence — audit clock fixed at the implementation.** Post-fix run 2 still showed a 415.1s planning-stage gap; Tier-0 diagnosis (monotonic 298.7s vs wall 651.0s; pmset: Maintenance Sleep 23:32:51 → DarkWake 23:38:46, 355s) proved the machine was OS-suspended for 352s of it — active silence ≈63s, under the bound, with the same run's building-leg heartbeat firing at exactly 120s. AC.PRG.1's text already scopes the bound to "while work is active," so this was the audit measuring on the wrong clock, not a loose AC and not a wrapper hole: run-record events now carry `ts_mono` and `audit_progress` checks the bound on the monotonic clock (the heartbeat scheduler's own clock, which a suspended process cannot violate) while still reporting the wall gap (`max_gap_wall_s`) so suspension stays visible. AC text NOT loosened; fix commit `cc394548`; evidence preserved at `smoke/oa-live-run2-sleep-artifact-evidence/`. Operational corollary: live proof runs execute under `caffeinate` so the demo-shaped environment (a watched, awake machine) is what's actually measured. Post-fix OA live run 3 (caffeinated, 2026-06-10 00:14): all three OA tests passed in one shared run — terminal done, wall 574.5s, max gap 120.0s monotonic (= wall; no sleep), 0 unverifiable claims, 17 user-visible updates, 6 live norms, 5/6 gate criteria traced, converged first pass.
8. **Frame-kernel amend-fixup folded into this amendment's seal window (dispatcher ruling: fold-in, normal corrective commit, no new manifest admissions).** A post-seal frame-kernel corrective (`17529112` — route the SubagentStart bundle's active-workstream probe through `workspace_paths.pos_subdir`, AC.D.2.5) landed inside this amendment's 775d32af..SEAL window; workspace-bootstrap's seal-scope test (`test_B20`) fails the seal on the corrective without admissions. Resolution per the dispatcher's fence ruling: admit the fixup's two files file-precisely into the seal window (`cf793459`, widened mechanically by `loam amend apply`) rather than opening a second amendment — the fence question the prior build agent halted on is answered FOLD-IN. Not a fence widening for new work: frame-kernel edits remain a plan §8 #4 halt.
9. **Seal-blocking ref-rewind: the LIVI suite rewound the REAL checkout's `main` on every run — diagnosed Tier-0, fixed test-side (`21422aa4`).** `bootstrap_new_workspace`'s local-path branch calls `_materialise_canonical_branch(<source>)`, which runs `git -C <source> update-ref refs/heads/main refs/remotes/origin/main` against the SOURCE; four AC.LIVI call sites passed `canonical_source=str(LOAM_ROOT)` (the real checkout), so every suite run — including the seal sweep itself — rewound the live repo's `main` to `origin/main` whenever local was ahead (always true mid-amendment; this is what broke the first seal attempts). Fix (minimal diff, test-side only): a session-scoped `isolated_canonical_clone` conftest fixture (a local `git clone` of the checkout — cheap, hardlinked objects, and MORE faithful to the documented-Quickstart topology where `main == origin/main`) replaces the real checkout at all four call sites; its teardown asserts the real checkout's `refs/heads/main` is byte-identical before/after the session (the regression baked in as a standing guard). Verified: 11 LIVI tests green (162s), real `main` unmoved across the run AND across the seal sweep. The PRODUCTION-side guard (refuse/no-op the rewind when the source's `main` is ahead of `origin/main`) is explicitly OUT OF SCOPE per the dispatcher — captured in the pos3 queue as `ws-bootstrap-materialise-rewinds-ahead-source-main`. LIPW_4/LIPW_5's real-bootstrap sites are gated behind `PB_SUBLOAM_REAL_CLAUDE=1` or use a fake `bootstrap_fn` — outside the sweep's hazard surface (noted for the queued guard).

---

### Commit SHAs

- Amendment commit (window end): `21422aa46499aa2895abbe406fb0e4fb12b95d1a` —
  `fix(workspace-bootstrap): LIVI tests bootstrap from an isolated clone — stop rewinding the real checkout's main`
- Apply commit: `7dcec73a7267e32b51dadff0926bf0ecf4557ffe` —
  `chore(amend): general-build-from-intent manifest+apply — workspace-bootstrap BASELINE+sidecar bump to 775d32a`
- Seal commit: `f4fd93b0d5e9851420f9b6f71ab2ce69f5901a9f` —
  `chore(seals): general-build-from-intent — workspace-bootstrap at 21422aa`
- Cross-component sweep: 21 components green (1 skipped — no seal-diff test recognised: scope-of-work); post-seal `apply --dry-run` clean per the seal tool's own verification.
