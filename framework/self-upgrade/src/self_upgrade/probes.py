...
def _probe_scope_of_work(paths: Paths) -> ProbeRecord:
    ...
    try:
        from scope_of_work.store import EventStore  # type: ignore
        from scope_of_work.upgrade import capture_pre_upgrade  # type: ignore
...
def _probe_objective_tracker(paths: Paths) -> ProbeRecord:
    ...
    try:
        from objective_tracker.store import EventStore  # type: ignore
        from objective_tracker.upgrade import capture_pre_upgrade  # type: ignore
    except Exception as exc:
        return ProbeRecord(
            component="objective_tracker",
            status="error",
...
def _probe_orchestrator(paths: Paths) -> ProbeRecord:
    ...
    try:
        from pos_orchestrator.local_state import LocalStateStore  # type: ignore
    except Exception as exc:
        return ProbeRecord(
            component="orchestrator",
            status="error",
...
def _probe_degradation(paths: Paths) -> ProbeRecord:
    ...
    try:
        from graceful_degradation.state import DegradationStore  # type: ignore
    except Exception as exc:
        return ProbeRecord(
            component="degradation",
            status="error",
...
def _probe_primary_persona() -> ProbeRecord:
    ...
    """
    try:
        from primary_persona.compaction import SURVIVAL_LIST  # type: ignore
