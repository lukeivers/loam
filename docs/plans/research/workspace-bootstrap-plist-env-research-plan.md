...
**Session-start corpus:** research agent reads the five mandatory paths in `CLAUDE.md`'s session-start-discipline section first. Component-scoped reads: `docs/archive/component-research/workspace-bootstrap/`, `hands-off-lifecycle/hooks/first_run_scaffold*.py` (where the plist template lives).
