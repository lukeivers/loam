...
**Authority documents:**
- `docs/plans/loam-rename-decisions.md` Tier-1 #6 (the M1g target — `pos-amend` → `loam amend` subcommand under unified `loam` CLI).
- `.scratch/claude-output/loam-rename-migration-plan.md` §3.6 (research mechanics for CLI rename).
- `docs/plans/oss-v0-1-0-publish-rename.md` §2 ladder (M1g row), §5 series-wide hard constraints, §7 series-wide halt triggers.
- `docs/plans/oss-v0-1-0-publish-rename-1e.md` §11 finding #4 (tools-tree residuals carried into M1g scope).
- `docs/plans/oss-v0-1-0-publish.md` §5 (programme master plan; M1g row per series-master ladder).
...
10. **Item 10 — Doc/code prose pos-amend → loam amend rebrand.** All non-historical references to the `pos-amend` CLI binary in:
    ...
    - **Out of scope — preserved historical record** per Idea 10 / dispatcher M1e ruling 3:
      ...
      - All `docs/plans/*.md` historical plan-docs (M1a..M1f sub-plans, amendment-NN-*.md plans, etc.).
...
- **Historical component-record docs** at `docs/archive/component-research/*/{research,research-plan,brief,component}.md` — preserved.
- **`docs/spec/*.md` (including pos-v2-rebuild-proposal.md)** — preserved per Idea 10.
- **`docs/plans/two-modes-and-multi-workspace/*.md`** — preserved.
...
- `docs/plans/*.md` historical plan-docs (M1a..M1f sub-plans, amendment-NN-*.md plans, etc.) — preserved per Idea 10 / dispatcher M1e ruling 3.
- `framework/<comp>/seals/SEAL_COMMIT.*` — preserved verbatim.
- Past commit messages — preserved.
- `docs/spec/pos-v2-rebuild-proposal.md` — preserved per Idea 10.
...
- No `docs/plans/*.md` historical plan-doc edits beyond this plan-doc + manifest YAML.
...
- **Historical preservations** per §6: past commit messages, past seal narratives, all `docs/plans/*.md` historical plan-docs, `docs/spec/pos-v2-rebuild-proposal.md`, and inner content of historical component-record docs preserved verbatim.
...
- **Historical seal narratives** at `framework/<comp>/seals/SEAL_COMMIT.*` — preserved.
- **Historical plan-docs** at `docs/plans/*.md` (other than this plan-doc + manifest YAML) — preserved.
- **Historical component-record docs** at `docs/archive/component-research/<comp>/{research,research-plan,brief,component}.md` — preserved.
- **`docs/spec/*.md` (including `pos-v2-rebuild-proposal.md`)** — preserved per Idea 10.
- **`docs/plans/two-modes-and-multi-workspace/*.md`** — preserved.
- **Frozen self-upgrade release manifest `framework/self-upgrade/manifests/pos-v2-v0.2.0.yaml`** — preserved.
- **Repo directory rename** `ivers-corp-pos-v2` → `loam` — M9-deferred.
...
3. **M1g sub-plan + manifest commit.** This plan-doc + a manifest YAML at `docs/plans/oss-v0-1-0-publish-rename-1g.manifest.yaml` per the established M1a..M1f precedent shape.
...
- **Prior sub-amendments:**
  ...
  - `docs/plans/oss-v0-1-0-publish-rename-1f.md` (sealed `390e1ca`; §14 backfill `af2e740`).
- **Authority documents (inherited from series master):**
  - `docs/plans/loam-rename-decisions.md` Tier-1 #6 (the M1g target).
  - `.scratch/claude-output/loam-rename-migration-plan.md` §3.6 (mechanics).
- **Programme master plan:** `docs/plans/oss-v0-1-0-publish.md` (M1g row in §5 per series-master ladder).
...
- **Precedent multi-component sealed-amendment manifests:**
  - `docs/plans/oss-v0-1-0-publish-rename-1f.manifest.yaml` (M1f sibling — 3-component dormancy rename).
  - `docs/plans/oss-v0-1-0-publish-rename-1e.manifest.yaml` (M1e sibling — 14-component namespace pivot).
