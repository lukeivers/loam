...
def test_AC_D_np_6_pyproject_no_new_third_party_dependency_introduced() -> None:
    """The plan-extension build does not introduce any new third-party
    dependency. Post-M6b.1 (master plan AC.OSS-M6.15 + §10
    D-build.M6.15) the loam-amend package's own pyproject.toml lives at
    plugins/dev-sdlc/tools/loam-amend/pyproject.toml; its dependency
    list is ``["PyYAML>=6", "loam-cli"]`` — ``PyYAML`` is the only
    third-party dep, and ``loam-cli`` is an INTRA-FRAMEWORK dep
    (loam_cli ships ``__version__`` consumed by the amend CLI's
    ``--version`` flag) which is owner-permitted per the M6b.1 plan.
    """
    pyproject = Path(__file__).resolve().parents[1] / "pyproject.toml"
    contents = pyproject.read_text(encoding="utf-8")
    # AC.D-np.6 invariant: ``PyYAML>=6`` is the only third-party dep
    # declared. Intra-framework deps (``loam-cli``) are admitted post-
    # M6b.1 — the rule is no NEW THIRD-PARTY surface.
    assert "PyYAML>=6" in contents, (
        "pyproject.toml dropped PyYAML; AC.D-np.6 requires PyYAML>=6 to "
        "remain the listed parser dependency."
    )
    # Defensive: assert no surprise third-party deps slipped in.
    forbidden_third_party = ("requests", "click", "rich", "pydantic", "httpx")
    for name in forbidden_third_party:
        assert name not in contents, (
            f"pyproject.toml introduced unexpected third-party dependency "
            f"'{name}'; AC.D-np.6 disallows new third-party deps. "
            "Update only if the change is owner-approved."
        )
...
def test_AC_D_np_6_existing_template_engine_module_unchanged_at_api_surface() -> None:
    ...
    deletion or rename.
    """
    from loam_amend.template_engine import (  # noqa: F401
