...
**Programme master:** `docs/plans/oss-v0-1-0-publish.md` (master plan §5 M4 row + §6 sequencing rule #3).
...
- Programme AC: AC.OSS.2 (D-1) — `docs/plans/oss-v0-1-0-publish.md` §3.
...
- `docs/plans/oss-v0-1-0-publish-dispatch-with-scope.md` (this plan-doc).
- `docs/plans/oss-v0-1-0-publish-dispatch-with-scope.manifest.yaml` (the manifest).
...
1. **Author the manifest.** `docs/plans/oss-v0-1-0-publish-dispatch-with-scope.manifest.yaml`.
...
- Amendment commit: `92b1130a3e59d6171d11506245523345cc22e569` —
  `chore(wire-dispatch-apply): loam amend apply for amendment #85 (M4 wire dispatch_with_scope)`
- Seal commit: `1719e1408838c3e6011101273fc77559ecbb2195` —
  `chore(seals): M4 wire dispatch_with_scope as the persona's actual Agent-dispatch path — author NEW PreToolUse hook framework/hands-off-lifecycle/hooks/dispatch_setup_hook.py (matcher Task, ~250-350 LOC) that, when dispatch prompt carries <AC-MANIFEST> CSV block (component,ac_id,source_path_glob per row), performs disk-side four-gate setup phase (1. write_active_scope_sentinel, 2. tracker.register_source_binding per AC, 3. write_dispatcher_stub per AC, 4. plan-doc reference recorded via sentinel's plan_path field) BEFORE allowing Task tool to fire — wires AC.OSS.2 (D-1 from feature-usage audit; dispatch_with_scope had zero non-test callers) into the persona's actual Agent-dispatch path via Claude Code native PreToolUse event per master plan §12 Lens 1 + dispatch authority. Add NEW thin public function write_dispatcher_stub(workspace_root, spec, *, scope_id, plan_path) in framework/primary-persona/src/loam/primary_persona/dispatch_wrapper.py delegating to existing private _write_stub_idempotent (~10 LOC additive; existing _write_stub_idempotent + _run_setup_phase + dispatch_with_scope + every other surface preserved byte-identical per dispatch out-of-scope clause + plan §10 D-build.M4.5). Re-export from loam.primary_persona via __init__.py. Compose alongside A2/A3/A4 in multi-contributor PreToolUse stanza per merge_pre_tool_use — 5th entry, runs sequentially after agent_guard (both share matcher Task; agent_guard refuses + dispatch_setup_hook always allows; AC.DSA.4 idempotency covers re-fire after agent_guard deny per plan §9 risk #1). Mode-bit short-circuit on NORMAL USE workspaces per AC.DSA.6 mirror. Fail-soft on substrate failures (tracker unavailable / sentinel write fails / stub write fails) with structured NDJSON to <workspace>/workspace/.pos/dispatch-setup-hook.log per AC.DSA.5/AC.DSA.9 mirror. Passthrough with NDJSON deprecation log on no-AC dispatches (option 3 per dispatch — preserves AC.DSA.10 backwards-compat for research/plan dispatches that legitimately don't author new ACs; future amendments can audit log to characterise no-AC traffic + rule on tightening to refusal per plan §10 D-build.M4.3). Recursion handling: LOAM_DISPATCH_BYPASS_HOOK=1 env-var bypass + sentinel-already-written idempotency (theoretical today since dispatch_with_scope has zero production callers; design-for-future per plan §10 D-build.M4.4). AC declaration format: <AC-MANIFEST>...</AC-MANIFEST> CSV block (stdlib csv.reader parseable; namespace-segregated marker — verified at plan-authoring no collision per plan §11 finding #4; recommended per plan §10 D-build.M4.2). 6 new test files (5 in hands-off-lifecycle: test_AC_OSS_M4_1_setup_phase_runs.py + test_AC_OSS_M4_2_ac_declaration_parsing.py + test_AC_OSS_M4_3_pre_tool_use_composition.py + test_AC_OSS_M4_5_normal_use_short_circuit.py + test_AC_OSS_M4_6_fail_soft_diagnostic.py; 1 in primary-persona: test_AC_OSS_M4_4_public_stub_writer.py — covers AC.OSS-M4.4 public surface). Sealed-component fence: 2 components — primary-persona + hands-off-lifecycle (per plan §10 D-build.M4.6). HC#4 byte-content sample status: NO RETIRE-AND-REBASELINE (no HC#4 sample paths under either touched component impacted by M4 diff; verified at plan-authoring per plan §11 finding #6). Critical architectural finding surfaced + accommodated at plan-authoring: Claude Code's PreToolUse hook protocol allows only allow/deny/ask outputs — cannot wrap tool execution; dispatch's enumeration of four gates is satisfied by the disk-side setup phase (per amendments #52/#74 setup-phase semantics) per plan §11 finding #1 + §10 D-build.M4.1. IPC-bound gate chain (cost/safety/reversibility/orchestrator wraps inside activate_scope_with_spec) stays as explicit-call surface for in-process dispatch_with_scope callers — out of M4 scope per dispatch + plan §6. — primary-persona+hands-off-lifecycle at 92b1130`
## 15. References
...
- **Precedent multi-component sealed-amendment manifests:**
  - `docs/plans/oss-v0-1-0-publish-wire-clis.manifest.yaml` (M3 — closest predecessor; multi-component fence shape).
  - `docs/plans/agent-dispatch-as-scope-wrapper.manifest.yaml` (amendment #52 — original `dispatch_with_scope` authoring).
