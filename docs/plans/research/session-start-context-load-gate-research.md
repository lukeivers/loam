...
`docs/plans/research/session-start-context-load-gate-research-plan.md`.
...
`docs/odd-in-pos.md`, `docs/VALUE_PROPOSITION.md`,
`docs/STATE.md`, `docs/FUTURE_IDEAS.md` (Idea 8 verbatim),
...
- **B — Dynamic lookup against the component's proposal/seal sidecar.**
  `docs/archive/component-research/<name>/` is the canonical directory; every
...
`~/.pos/orchestrator.sock`; (d) glob `docs/plans/amendment-*.md`
...
4. **New inference: component overlay resolution uses directory
   ...
   don't follow the `docs/archive/component-research/<name>/` convention.
...
- **Lens 3 (ODD authoring):** acceptance criteria in the proposal
  ...
  - *Outcome:* "A component in
    `docs/archive/component-research/<name>/` gains a new proposal artefact;
...
- **D-1. Component-set resolution source.** Research recommends
  directory-listing of `docs/archive/component-research/<name>/` (§3.3
  ...
  - *Owner ruling required if:* there's intent to move component
    artefacts out of `docs/archive/component-research/`, or to house
...
- Session-start corpus: `docs/odd-methodology.md`;
  ...
  `docs/STATE.md`;
  `docs/FUTURE_IDEAS.md` (Idea 8 verbatim at §Idea 8).
...
- Persona-layer surface: `primary-persona/src/monitor.py` (the
  ...
  load);
  `docs/archive/component-research/primary-persona-loader/proposal.md` (D3
...
- D7 research plan: `docs/plans/research/memory-consumer-wiring-research-plan.md`
