...
**Research:** `docs/plans/research/bootstrap-progress-statusline-research.md` (governs this plan).
...
| AC.SL.S | The amendment's seal-diff is confined to `hands-off-lifecycle/` and `docs/plans/`. No edits to `workspace-bootstrap/`, `primary-persona/`, or any other sealed component. | Existing seal-diff harness (`test_no_sealed_amendments.py` per touched component, plus `hands-off-lifecycle/tests/test_cross_cutting.py`'s H19). |
...
- Research doc: `docs/plans/research/bootstrap-progress-statusline-research.md`.
- Spec binding: `docs/spec/pos-v2-objectives-spec.md` "Non-tech users" v1.0 acceptance bullet — re-extension of low-friction-onboarding.
- VALUE_PROPOSITION binding: `docs/VALUE_PROPOSITION.md` Lens 2 (translation burden + harness toolkit).
- Sibling pattern (settings-merge functions, hook-stanza authoring shape, ODD §2.5 trace): `docs/plans/amendment-46-persona-session-start-turn-start-emitters.builder-plan.md`.
- Sibling pattern (workspace-local state-file routing, defence in depth): `docs/plans/amendment-28-workspace-identity-routed-first-run.md`.
...
- FUTURE_IDEAS CDCs: `docs/FUTURE_IDEAS.md` (research-before-plan, plan-before-code, scope-only-dispatch, amendment-dispatch test-scope).
