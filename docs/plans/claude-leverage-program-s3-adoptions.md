...
- **D-ADOPT.1** — native-`/goal`-vs-`autonomy_continuation.py` disposition.
  Recommendation in §10; builder records the final ruling + the SHA of the
  decision-record commit here at build time.
- **D-ADOPT.2** — `goal.md` matcher-row precision tier (deny/warn boundary).
  Builder records the regex shape + tier choice + any false-positive tuning here.
- **D-ADOPT.3** — corpus fence admission (universal-prefix vs component).
  Builder records the manifest choice + confirmation here.
- SHA-register backfilled by `loam amend seal --plan-doc`.
