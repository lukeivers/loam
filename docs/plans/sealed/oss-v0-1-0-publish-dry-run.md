...
**Programme master:** `docs/plans/oss-v0-1-0-publish.md` — §5 row M11 + §6 sequencing rule 8 ("M11 is the integration gate") + §7 Gate G2 (synthesis dry-run review) + §3 prime ACs (AC.OSS.1 / AC.OSS.2 sweep / AC.OSS.3 sweep / AC.OSS.5 sweep).
...
- Master plan §5 row M11: `docs/plans/oss-v0-1-0-publish.md` — synthesis dry-run + private-staging review + automated sweep + owner gate G2 (rename master-plan numbering: G2 = "M8" in audit numbering).
...
- Memory-pivot precedent (recent landed sub-plan; format reference): `docs/plans/oss-v0-1-0-publish-memory-pivot.md`.
- M9 scrub precedent (synth-substitution mechanism this sweep verifies): `docs/plans/oss-v0-1-0-publish-scrub.md` — sealed `2161cb1` 2026-04-29; landed the four-entry SUBSTITUTION_TABLE the M11 AC.OSS.5 sweep checks.
- M7 docs lane precedent (verify-first / rewrite-only-on-fail pattern reference): `docs/plans/oss-v0-1-0-publish-public-docs.md`.
...
- **M11a (mechanical, autonomous; v0.1.0 critical-path; runs now).** Synthesise canonical HEAD via the M2-extended `pos-publish-framework-only` pipeline → produce synthetic `framework-only` branch locally → push BOTH `framework-only:main` AND `framework-only:framework-only` to private `lukeivers/loam-staging` (dual-ref per FBE.11; recommended; gh CLI authenticated to lukeivers) → run four automated sweeps:
...
**Verification.** The CLI exits 0 (no `SynthesisError`); `git rev-parse refs/heads/framework-only` produces a non-empty SHA; the recorded SHA-pair is committed to the M11a report at `docs/plans/oss-v0-1-0-publish-dry-run.md` §14 (post-build).
...
- Private repo `lukeivers/loam-staging` exists on github.com (created via `gh repo create lukeivers/loam-staging --private` if not already present). **Operator note (race window — captured 2026-05-03 from M12 ship-day):** `gh repo create` followed immediately by `git push` to the new repo URL intermittently fails with `remote: Repository not found.` exit 128 — github needs ~1-3 seconds to fully provision the new repo before accepting pushes. Mitigations (use any one): (a) sleep 2-3s between `gh repo create` and the first push; (b) wrap the first push in a retry loop (up to 3 retries with 1s backoff); (c) verify-via-`gh repo view`-loop until the repo is queryable before pushing. Observed during initial M12 publish: an annotated-tag push happened to land first and triggered repo materialization; the dual-ref push then succeeded on retry. Worth pinning the mitigation in any future automated publish-flip script.
- The push is a **dual-ref push** (per FBE.11, closes BLOCKER-FBE6c.1): the single `git push` command publishes BOTH `framework-only:main` AND `framework-only:framework-only` to the staging remote. Both refs point at the same SHA (the local `framework-only` branch tip). Push command shape: `git push https://github.com/lukeivers/loam-staging.git framework-only:main framework-only:framework-only --force` (single invocation; both refspecs in one command). The `main` ref preserves a friendly browse default for any operator who clones outside the loam workflow; the `framework-only` ref is the bootstrap-required ref that `_clone_canonical` materialises into the workspace's `framework/` subdir checkout. Without the `framework-only` ref, `loam init <ws>` (post-FBE.{9,10} smart-default) fails at the `git checkout -B framework-only origin/framework-only` step with `fatal: 'origin/framework-only' is not a commit` — exactly the failure FBE.6c surfaced. With BOTH refs published, stranger clones get `refs/remotes/origin/framework-only` and the bootstrap proceeds.
- `lukeivers/loam-staging:main` AND `lukeivers/loam-staging:framework-only` both carry a squashed initial commit; post-push trees are byte-identical to the local synthetic `framework-only` tree (same SHA on both refs).
- Owner can `git clone https://github.com/lukeivers/loam-staging.git` against the private repo (gh CLI authenticated to lukeivers; verified at dispatch-time per dispatch-side gate). The clone produces both `refs/remotes/origin/main` and `refs/remotes/origin/framework-only`; `loam init <ws>` succeeds against the clone path (the bootstrap finds `origin/framework-only` and materialises it into a local `framework-only` ref before the workspace `framework/` subdir checkout).

**Verification.** `gh api repos/lukeivers/loam-staging` returns 200; `git ls-remote https://github.com/lukeivers/loam-staging.git` includes BOTH `refs/heads/main` AND `refs/heads/framework-only` at the same SHA; `git diff <local-framework-only-tree> <remote-staging-main-tree>` produces zero output (and equivalently for the `framework-only` ref). If owner rules D-Q.M11.1 = defer to M11b or skip-entirely-until-M12, this AC is omitted from M11a's authored report and recorded as deferred.
...
7. **M11a outcome is partly observable via .scratch/** (the sweep report). Per CLAUDE.md §3 output convention: ephemeral artefacts go to `<workspace>/.scratch/claude-output/<subject>.md`; persistent state goes to canonical paths. M11a sweep report at `.scratch/` is correct (gitignored; survives session boundaries; doesn't bloat canonical tree). The M11 plan-doc itself (this file) IS canonical (`docs/plans/`). **NOT a halt** — captures the convention correctly.
...
- D-build.M11a.10 (dispatch 3): AC.M11a.7 staging push mechanism — `gh repo create lukeivers/loam-staging --private` (single call) + `git push https://github.com/lukeivers/loam-staging.git framework-only:main` (single call); verification via `git ls-remote` (remote main SHA matches local `framework-only` SHA) + `gh api repos/...` (200 OK; private:true; default_branch:main). **Superseded post-FBE.11 (2026-05-03):** the push mechanism now publishes a dual-ref shape — `git push <staging-url> framework-only:main framework-only:framework-only --force` (single invocation; both refspecs in one command) — so stranger clones get `refs/remotes/origin/framework-only` (the bootstrap-required ref). Original FBE.6/FBE.6b/FBE.6c dispatches landed before the supersede; their as-dispatched single-ref `framework-only:main` push is the historical record. See AC.M11a.7 §body above for the current dual-ref push contract; FBE.11 sub-plan-doc at `docs/plans/v0-1-0-foldback-scope-expansion-fbe11.md` for the closure rationale.
...
- **Memory-pivot sub-plan precedent:** `docs/plans/oss-v0-1-0-publish-memory-pivot.md`.
- **M9 scrub precedent (substitution mechanism this sweep verifies):** `docs/plans/oss-v0-1-0-publish-scrub.md`.
- **M7 docs-lane precedent (verify-first / rewrite-only-on-fail pattern):** `docs/plans/oss-v0-1-0-publish-public-docs.md`.
- **Synthesis tool:** `framework/tools/pos-publish-framework-only/src/loam/publish_framework_only/{synth.py,partition.py,substitution.py,cli.py}`.
...
- **STATE.md:** `docs/STATE.md` — governing rules + component table.
- **Owner-ruling-history:** `docs/plans/oss-launch-decisions.md` — M0 + (forthcoming) M11b ruling.
