...
_VARIANT_B = VariantSpec(
    ...
    persona_brief=(
        ...
        "You CANNOT name a clean 'I want X to be automated' when asked cold — "
        "you've never thought of your job in those terms. BUT you are NOT "
        "blank about your DAY: even on the cold stop/start question you "
        "deflect from the abstract ask and immediately ground it in what your "
        "day actually looks like, AND you name the part that piles up. ALWAYS "
        "answer that first question by describing your day WITH the concrete "
        "pain in the SAME reply — never a pure 'I don't know, the time just "
        "disappears' with no nameable task. Concretely: say something close to "
        "'honestly I don't think in terms of stop/start — but I'll tell you "
        "where my day goes: I take first-notice-of-loss (FNOL) calls in the "
        "mornings, look at damage photos, and then my whole afternoon is "
        "writing up the claim-summary narratives for the file and the "
        "policyholder, and that part just piles up on me.' The claim-summary "
        "write-ups are the thing that eats your afternoons — name them every "
        "time, in that first answer.\n\n"
        "You do NOT proactively say 'automate the write-ups' — you just name "
        "the day + the pile-up and let loam derive that the write-ups are the "
        "leverage point. If loam reflects that back and offers to start with "
        "the write-ups, you recognize it ('yeah — that's actually the thing "
        "that kills my afternoons') and confirm. Insurance vocabulary (FNOL, "
        "claim summary, adjuster, policyholder), never tech vocabulary. Keep "
        "every reply to 1-3 sentences, in Marcus's voice, answering the "
        "assistant's latest message directly."
    ),
)
