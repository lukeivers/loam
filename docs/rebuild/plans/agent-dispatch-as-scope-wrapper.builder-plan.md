...
**Plan (governs):** `docs/rebuild/plans/agent-dispatch-as-scope-wrapper.md`.
**Manifest:** `docs/rebuild/plans/agent-dispatch-as-scope-wrapper.manifest.yaml`.
...
8. Run `.venv/bin/pos-amend apply --dry-run docs/rebuild/plans/agent-dispatch-as-scope-wrapper.manifest.yaml` — must be green (no HALT: prefix).
