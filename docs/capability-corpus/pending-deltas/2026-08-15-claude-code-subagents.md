...
A background subagent that completes stays listed in /tasks , marked done and sorted below running work, until the session cleans up its task list.
...
When Claude doesn’t request a type, it gets the general-purpose subagent, and subagents spawned from a definition, such as Explore, work as usual.
