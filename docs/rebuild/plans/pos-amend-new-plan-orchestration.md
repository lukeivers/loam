...
**Companions:** `docs/rebuild/plans/dispatch-prompt-template-extension.md` (engine landed at commit `dbabd37` — this plan composes on top), `docs/rebuild/plans/pos-amend-seal-automation-extension.md` (the §14 `### Commit SHAs` subsection scaffold this plan must preserve byte-identical).
**Ancestor record:** `docs/rebuild/FUTURE_IDEAS_DRAFT.md` entries "Plan-doc skeleton template" (line 25), "`pos-amend new-plan <slug>` orchestration" (line 57), and the §14 / one-test-file-per-AC lessons (lines 71–72, 80).
**Research:** `docs/rebuild/plans/research/pos-amend-new-plan-orchestration-research.md`.
...
2. **`pos-amend new-plan <slug>` orchestration** — new CLI subcommand that, given a slug, scaffolds an empty-but-pre-stubbed vars-file at a predictable path (`<repo>/docs/rebuild/plans/<slug>.vars.yaml`), pre-fills `TITLE` / `AC_PREFIX` from optional CLI args, fills `RESEARCH_PATH` and `STATUS_LINE` from the slug + current date, and (with `--render`) immediately renders the plan-doc to `<repo>/docs/rebuild/plans/<slug>.md`. Leverages the existing `pos-amend template render` engine — no engine change.
...
Invoking `pos-amend new-plan <slug> --title "T" --ac-prefix AC.X.x --render` writes BOTH the vars-file at `<repo>/docs/rebuild/plans/<slug>.vars.yaml` AND a rendered plan-doc at `<repo>/docs/rebuild/plans/<slug>.md`. The rendered plan-doc carries every section heading from §1 through §14 verbatim (including §14 method-decision register subsection scaffold), the `TITLE` substitution in the heading-1, the `AC_PREFIX` substitution in §4's heading and body references, and the §14 `### Commit SHAs` subsection scaffold (so `pos-amend seal --plan-doc <abs-path>` finds its target verbatim).
...
13. **Vars-file path is `<repo>/docs/rebuild/plans/<slug>.vars.yaml`.** Per D-1 ruling. Predictable. Co-located with plan-doc. Audit-friendly. Not under `.scratch/`.
...
4. Write builder-plan to `docs/rebuild/plans/pos-amend-new-plan-orchestration.builder-plan.md` naming specific files + symbols expected to be touched, including the exact widening of the skeleton's `required:` and `optional:` lists.
...
[edit docs/rebuild/plans/my-new-feature.vars.yaml]
[re-render: pos-amend template render plan/dev-discipline --vars-file docs/rebuild/plans/my-new-feature.vars.yaml --out docs/rebuild/plans/my-new-feature.md --force]
```
...
- **D-1a. `<repo>/docs/rebuild/plans/<slug>.vars.yaml`** — sibling of the plan-doc, gitted, audit-friendly (RECOMMENDED).
...
|---|---|---|
| D-1 | Vars-file at `<repo>/docs/rebuild/plans/<slug>.vars.yaml` (D-1a) | Predictable, gitted, audit-friendly |
...
- Research doc: `docs/rebuild/plans/research/pos-amend-new-plan-orchestration-research.md`
...
- Sibling plan (engine ancestor; landed `dbabd37`): `docs/rebuild/plans/dispatch-prompt-template-extension.md`
- Sibling plan (seal-automation; the §14 SHA-backfill machinery): `docs/rebuild/plans/pos-amend-seal-automation-extension.md`
...
- STATE / FUTURE_IDEAS: `docs/rebuild/STATE.md`, `docs/rebuild/FUTURE_IDEAS.md`
