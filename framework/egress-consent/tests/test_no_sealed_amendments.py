...
egress-consent component + its declared universal-admitted partners. Zero
deltas to any sealed component outside the fence.

Sidecar pattern (matching usage-window-guard / protection-matrix /
state-migration-engine): SEAL_COMMIT reads the exact seal SHA post-seal;
pre-seal it reads HEAD so a build on an unfinished seal still exercises the
test.

BASELINE: 9fd5593e — loam HEAD @ plan-authoring on branch
``egress-consent-core-and-bug-report``
(``docs(plans): record usage-window-guard-foundation commit SHAs ...``). This
...
def test_only_egress_consent_and_partners_changed() -> None:
