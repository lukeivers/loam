...
- **Spec anchors:** `docs/rebuild/VALUE_PROPOSITION.md`
...
- **FUTURE_IDEAS surface:**
  `docs/rebuild/FUTURE_IDEAS_DRAFT.md` — Bash-tool
...
3. **Plan-before-code.** This plan exists; the builder
   ...
   `docs/rebuild/plans/graphiti-async-write-and-prewarm.builder-plan.md`
...
plan: docs/rebuild/plans/graphiti-async-write-and-prewarm.md
...
| AC.J.S | seal-diff invariant | Dev-discipline amendment — no SEAL_COMMIT bump; new files all under `primary-persona/src/`, `primary-persona/tests/`, `workspace-bootstrap/src/`, `workspace-bootstrap/tests/`, `docs/rebuild/plans/`. Existing seal-fence tests (`test_no_sealed_amendments.py`, `test_AC_M_S_seal_diff_window.py`) pass since SEAL_COMMIT sidecar is unchanged and BASELINE..SEAL_COMMIT diff is unaffected by post-seal commits. |
