...
- `docs/plans/seal-retrofit-graceful-degradation-observability-aggregator.md`
...
- **Previous-BB attempt** — clause-(h) BB-build halted before code,
  leaving untracked
  `docs/plans/self-upgrade-clause-h-llm-merge.{md,builder-plan.md,vars.yaml}`.
...
self-upgrade was sealed 2026-04-19 14:12 (per `docs/STATE.md`)
...
4. **Plan-before-code.** This plan exists; builder writes a
   builder-plan to
   `docs/plans/self-upgrade-seal-bookkeeping-retrofit.builder-plan.md`
...
6. Run `pos-amend apply --dry-run docs/plans/self-upgrade-seal-bookkeeping-retrofit.manifest.yaml`
...
9. `pos-amend seal --plan-doc docs/plans/self-upgrade-seal-bookkeeping-retrofit.md`
...
`docs/plans/self-upgrade-seal-bookkeeping-retrofit.manifest.yaml`.
...
plan: docs/plans/self-upgrade-seal-bookkeeping-retrofit.md
...
- **D-3 (locked):** allowed_prefixes minimal = `("self-upgrade/",
  "data/", "docs/plans/")` plus universal admissions
...
- `docs/STATE.md` — self-upgrade seal time.
- `docs/VALUE_PROPOSITION.md` — AC.PO.1 / AC.PO.2 trace.
- `docs/FUTURE_IDEAS.md` — none added; out of scope.
