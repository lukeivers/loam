...
**Total shipped:** 14 minor + 4 patches. v0.1.0 → v0.4.3 published-pending. v0.3.0 ships META-FRAMEWORK foundation; v0.4.0 ships code-gen-from-objectives optimised for extend-existing-repo; v0.4.1 closes F-DESIGN-1 (multi-commit + from-scratch + tie-breaker); v0.4.2 closes F-DESIGN-2 (Test-interface load-bearing + Py-version-compat); v0.4.3 closes the FBE.7 file-based memory retrieval BM25-bypass + grep-length-bias. v0.4.1 + v0.4.2 + v0.4.3 ship together LOCAL; owner gates the publish.
