...
**Companions:**
- **#56 plan-doc** (parent — keystone workspace-sync component, defines `pos-sync` CLI surface + `~/.pos/sync-config.yaml` reference): `docs/plans/workspace-sync.md`
- **#57 plan-doc** (Bundle α — just sealed; reference §11 for the existing config-file shape pattern): `docs/plans/workspace-sync-resolver-cost-overhaul.md`
- **VALUE_PROPOSITION** (binding spec — AC.PO.1 translation-burden + AC.PO.2 toolkit-primitive growth): `docs/VALUE_PROPOSITION.md`
...
- **FUTURE_IDEAS_DRAFT** "Workspace-sync follow-on family" lines 14-31 (KK / LL / MM captures): `docs/FUTURE_IDEAS_DRAFT.md`
...
6. **Plan-before-code** (binding per `feedback_plan_before_code`). This plan exists. Each AC's builder authors a builder-plan at `docs/plans/<ac-slug>.builder-plan.md` (or, if D-β.4 routes the bundle as a single amendment, at `docs/plans/workspace-sync-ergonomics.builder-plan.md`) before editing source.
...
- **Plan-doc** (this file): `docs/plans/workspace-sync-ergonomics.md`
- **Vars-file** (this plan's variables): `docs/plans/workspace-sync-ergonomics.vars.yaml`
- **Builder-plans** (post-D-β.4 ruling): one OR three, depending on bundle-shape ruling.
  - One-bundle: `docs/plans/workspace-sync-ergonomics.builder-plan.md`
  - Three-amendment: `docs/plans/workspace-sync-ergonomics-beta1.builder-plan.md` + `...-beta2.builder-plan.md` + `...-beta3.builder-plan.md`
- **Manifests** (per sealed-component AC under D-β.4 ruling): `docs/plans/workspace-sync-ergonomics.manifest.yaml` (or per-AC if split). Builder finalises.
...
Builder-plan: `docs/plans/workspace-sync-ergonomics-beta1.builder-plan.md`.
Manifest: `docs/plans/workspace-sync-ergonomics-beta1.manifest.yaml`.
...
- **D-build.7 — Pos-amend manifest shape.** Schema v1; one component (workspace-sync); standard universal-paths admissions (`docs/plans/` + the four CLAUDE.md / odd-* / FUTURE_IDEAS.md files); narrative target `workspace-sync/seals/SEAL_COMMIT.ergonomics-beta1`. Apply + seal followed #56 + #57 precedent exactly.
...
- `docs/VALUE_PROPOSITION.md` (binding spec — AC.PO.1 + AC.PO.2)
- `docs/STATE.md`, `docs/FUTURE_IDEAS.md`, `docs/FUTURE_IDEAS_DRAFT.md`
- `docs/spec/pos-v2-objectives-spec.md`
- `docs/plans/workspace-sync.md` (#56 plan-doc, 1377 lines; the parent plan; defines `pos-sync` CLI surface + `<workspace>/.pos/sync-protected.yaml` envelope)
- `docs/plans/workspace-sync.builder-plan.md` (#56 builder-plan, 731 lines)
- `docs/plans/workspace-sync-resolver-cost-overhaul.md` (#57 plan-doc, 1406 lines; the parent plan; reference §11 for the existing config-file shape pattern)
- `docs/plans/dispatch-prompt-template-extension.md` (recent dev-discipline plan — shape reference for §1-§14 skeleton)
