...
- **AC.D.5.5.S — Seal-diff invariant.** Single-component manifest. Diff confined to `framework/workspace-sync/` + `tools/` deletions + `workspace-sync/` deletions + universal admissions (`docs/rebuild/plans/`, `framework/hands-off-lifecycle/`). All admitted by workspace-sync's existing allowed_prefixes.
...
**Modified (post-seal):**
- `docs/rebuild/plans/d-migration.md` — §14 method-decision register backfill (D.5.5 entry added) + §15 verdict update (record D.5.5 closure).
...
2. **Implementation order:**
   ...
   - (e) `git add framework/workspace-sync/tests/test_AC_D_5_5_bare_paths_absent.py docs/rebuild/plans/d-migration-5-5.{builder-plan.md,manifest.yaml}` and stage the deletions.
3. **Pre-seal commit:** `pos-amend apply --dry-run docs/rebuild/plans/d-migration-5-5.manifest.yaml`. Expect green (all admissions present).
