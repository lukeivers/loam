...
def test_all_six_dev_sdlc_skills_discovered() -> None:
    ...
    directory yields the 6 expected dev-sdlc SKILL packages.
    ...
    assert discovered == sorted(EXPECTED_SKILLS), (
        ...
        f"{sorted(EXPECTED_SKILLS)}; AC.SKILLS-DSDLC1.7 requires "
        "exactly the 6 named packages."
    )


def test_skills_count_six() -> None:
    """v0.1.8 Cycle 5 — the dev-sdlc SKILL bundle is 6 SKILLs
    total (first pass; second pass deferred to v0.1.9)."""
