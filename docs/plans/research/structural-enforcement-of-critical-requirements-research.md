...
Full rule table follows. Sources: `~/.claude/CLAUDE.md`, `/Users/lukeivers/ivers-corp-pos-v2/CLAUDE.md`, `CLAUDE.dev.md`, `docs/odd-methodology.md`, `docs/odd-in-pos.md`, `docs/FUTURE_IDEAS.md` CDCs, 25 user-memory feedback bullets at `~/.claude/projects/-Users-lukeivers-pos3/memory/feedback_*.md`.
...
| 3 | Plan exists at `docs/plans/<name>.md` BEFORE source edits begin | FUTURE_IDEAS CDC; `feedback_plan_before_code` | Code ships without AC alignment; ODD violations land in seal cycle; 3 RED components in 2026-04-22 audit traced here | **PreToolUse Edit/Write on source tree refuses if no plan file matches the touched component** (composition with #1) | 🟡 | ⭐⭐⭐ | **PROMOTE** — A2 (subsumed) |
...
- An active `pos-amend` manifest in `docs/plans/amendment-<N>-<slug>.manifest.yaml` declaring the component+AC pair, OR
- A `.scope-of-work` sentinel in `.scratch/active-scopes/<scope-id>.yaml` declaring the AC-binding for the active scope.

The third path is the run-loop binding — when an agent (background or main) authors a plan at `docs/plans/<name>.md`, the plan's frontmatter writes the sentinel; the gate consults it on subsequent edits.
...
```json
{
  "hookSpecificOutput": {
    ...
    "permissionDecisionReason": "ODD §2.5 — file <path> has no AC binding. Declare the AC in docs/plans/<scope>.md or set .scratch/active-scopes/<scope>.yaml. Or, if this edit is dev-discipline (docs/CLAUDE.md/plans), retry on a docs/ path."
  }
}
```
...
**Hook shape:** the same gate that verifies AC binding in #1 also verifies a plan file exists at `docs/plans/<scope>.md`. If the active scope sentinel says `scope: amendment-N-foo`, the plan path must resolve. No additional hook surface — same hook, additional check.
...
- `docs/spec/pos-v2-objectives-spec.md` line 134–135 (Tiered determinism objective + audit-on-rule-where-hook-would-work acceptance)
- `docs/VALUE_PROPOSITION.md` (AC.PO.1 + AC.PO.2; Lens 2)
- `docs/FUTURE_IDEAS.md` Idea 8 (structural context-load gate — promoted to A1 in this programme)
