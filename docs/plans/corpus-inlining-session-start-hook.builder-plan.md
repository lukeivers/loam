...
**Plan-doc (governs):** `docs/plans/corpus-inlining-session-start-hook.md`.
**Research artefact:** `docs/plans/research/corpus-inlining-session-start-hook-research.md`.
**Manifest:** `docs/plans/corpus-inlining-session-start-hook.manifest.yaml`.
**Amendment number:** 73.
...
```
{"CLAUDE.md", "docs/VALUE_PROPOSITION.md", "docs/STATE.md"}
```

**On-demand tier:** a static set of three workspace-relative paths embedded as path-pointer references:

```
{"docs/odd-methodology.md", "docs/odd-in-pos.md", "docs/FUTURE_IDEAS.md"}
```
...
- `docs/plans/corpus-inlining-session-start-hook.builder-plan.md` (this file)
- `docs/plans/corpus-inlining-session-start-hook.manifest.yaml`
...
1. Author manifest at `docs/plans/corpus-inlining-session-start-hook.manifest.yaml` with BASELINE = `b8caf7f` (pre-amendment HEAD).
