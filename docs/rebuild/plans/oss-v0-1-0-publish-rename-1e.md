...
**Authority documents:**
- `docs/rebuild/plans/loam-rename-decisions.md` Tier-1 items 2–7 (namespace pivot scope).
- `.scratch/claude-output/loam-rename-migration-plan.md` (research; mechanics + dependency ordering).
- `docs/rebuild/plans/oss-v0-1-0-publish-rename.md` §1 D-RNM.2 ruling (per-component namespace-package shape `framework/<comp>/src/loam/<comp>/`), §2 (sub-amendment ladder), §5 (series-wide hard constraints), §7 (series-wide halt triggers).
- `docs/rebuild/plans/oss-v0-1-0-publish.md` §5 (programme master plan; M1e row in §5 per M1b precursor commit `7be713b`).
...
5. **Item 6 — Filename rebrands per Idea 10's no-retroactive-rewrites convention.** Two specific renames per dispatcher ruling 3:
   - `docs/odd-in-pos.md` → `docs/odd-in-loam.md` (current contract; renames).
   - `docs/rebuild/spec/pos-v2-objectives-spec.md` → `docs/rebuild/spec/loam-objectives-spec.md` (current contract; renames).
   - `docs/rebuild/spec/pos-v2-rebuild-proposal.md` → **preserve** (historical record per Idea 10's no-retroactive-rewrites clause; `**Date:** 2026-04-17` plus the prose framing makes this a frozen-in-time gap analysis / approval record).
...
**`pos.bootstrap.contributions` entry-point group callsites:** 1 in `pyproject.toml` `[project.entry-points.…]` header, 1 in `discovery.py`, 1 in `__init__.py`, 1 in `manifest.py`, 1 in `extension_protocol.md`, 1 in `README.md` for workspace-bootstrap; plus 1 in `docs/rebuild/plans/telegram-interface-framework-integration-build-plan.md` (historical plan-doc — preserved per series convention). Plus the `entry_points.txt` in the egg-info (auto-regenerated on `pip install -e`; not committed-source).

**Spec filename rename callsite distribution:**
- `docs/odd-in-pos.md` referenced by 19 live files (CLAUDE.md global, framework/<comp>/tests' `test_no_sealed_amendments.py` cross-cutting checks, multiple dispatch templates, and so on — see §11 finding #2 for the enumeration).
- `docs/rebuild/spec/pos-v2-objectives-spec.md` and `docs/rebuild/spec/pos-v2-rebuild-proposal.md` referenced by ~5 live files (mostly STATE.md / VALUE_PROPOSITION.md cross-references).
...
- **Historical component-record docs** at `docs/rebuild/components/<comp>/{research.md, research-plan.md, brief.md, component.md}` — preserved per M1a/b/c/d convention.
- **`docs/rebuild/spec/pos-v2-rebuild-proposal.md`** — preserved per dispatcher ruling 3 + Idea 10's no-retroactive-rewrites clause.
...
1. `git mv docs/odd-in-pos.md docs/odd-in-loam.md`. Internal text updates self-referential mentions (the title heading, any "this document" / "see odd-in-pos.md" cross-references). Past-tense narrative inside historical sections is preserved verbatim.
2. `git mv docs/rebuild/spec/pos-v2-objectives-spec.md docs/rebuild/spec/loam-objectives-spec.md`. Internal text — see §11 finding #2 for the per-section live-vs-frozen split (the file carries an explicit "v1.0 LOCKED" notice with addenda; locked v1.0 text is preserved verbatim; addenda + structural prose update where they are current-contract).
3. `docs/rebuild/spec/pos-v2-rebuild-proposal.md` — **preserve filename**. The file is a 2026-04-17 gap analysis with explicit "APPROVED" date and ruling-recorded sections (it IS a frozen-in-time approval record per Idea 10's no-retroactive-rewrites clause). Internal text is preserved verbatim including the "pOS v2" and "the new pOS" narrative. NO rename, NO content edit.
4. **Other `pos-v2-*.md` spec files** under `docs/rebuild/spec/`: per dispatcher ruling 3, `ls docs/rebuild/spec/` post-plan-authoring shows ONLY the two `pos-v2-*.md` files above; no third such file exists. The current-contract-vs-historical-record per-file split is therefore a 2-file decision, both ruled here.

All cross-references from live docs / code / fixtures pointing at `docs/odd-in-pos.md` (19 callsites — see §11 finding #2) and at `docs/rebuild/spec/pos-v2-objectives-spec.md` (~5 callsites) update concurrently to point at the renamed paths. Cross-references at `docs/rebuild/spec/pos-v2-rebuild-proposal.md` stay verbatim (preserved filename).

**Outcome:**
- `ls docs/odd-in-loam.md` exists; `ls docs/odd-in-pos.md` does NOT.
- `ls docs/rebuild/spec/loam-objectives-spec.md` exists; `ls docs/rebuild/spec/pos-v2-objectives-spec.md` does NOT.
- `ls docs/rebuild/spec/pos-v2-rebuild-proposal.md` STILL exists (preserved per ruling 3).
...
- `git log --follow docs/odd-in-loam.md` and `git log --follow docs/rebuild/spec/loam-objectives-spec.md` return the full pre-M1e histories.
...
- `docs/odd-in-pos.md` (RENAMED to `docs/odd-in-loam.md`).
- `docs/rebuild/spec/pos-v2-objectives-spec.md` (RENAMED to `docs/rebuild/spec/loam-objectives-spec.md`).
- `docs/rebuild/spec/pos-v2-rebuild-proposal.md` (preserved filename; NO content edits).
- `docs/rebuild/components/<comp>/proposal.md` (universal `docs/rebuild/components/` prefix; updates to `loam.bootstrap.contributions` references and import-path narrative).
...
- No `docs/rebuild/components/<comp>/{research.md, research-plan.md, brief.md, component.md}` edits.
- No `docs/rebuild/spec/pos-v2-rebuild-proposal.md` content or filename edits.
...
- **Historical preservations.** `docs/rebuild/plans/*.md`, `framework/<comp>/seals/SEAL_COMMIT.*`, `docs/rebuild/components/<comp>/{research.md,research-plan.md,brief.md,component.md}`, and `docs/rebuild/spec/pos-v2-rebuild-proposal.md` files preserved verbatim per series convention.
...
- **`docs/rebuild/spec/pos-v2-rebuild-proposal.md`** — preserved per dispatcher ruling 3.
...
3. **M1e sub-plan + manifest commit.** This plan-doc + a manifest YAML at `docs/rebuild/plans/oss-v0-1-0-publish-rename-1e.manifest.yaml` per the established M1a..M1d precedent shape.
...
9. **Phase F — Filename rebrand (item 6).** Two `git mv` operations + cross-reference updates per AC.RNM-1e.5. `docs/rebuild/spec/pos-v2-rebuild-proposal.md` is NOT touched.
...
12. **Phase I — Cross-mode-debt closure.** `framework/<comp>/docs/*.md` files that reference `from pos_<comp> import` / `from <comp> import` in worked examples / architecture diagrams / contract descriptions update concurrently per AC.RNM-1e.2 (live-doc surface). Per-component proposal docs at `docs/rebuild/components/<comp>/proposal.md` likewise. The 5 `docs/rebuild/spec/pos-v2-objectives-spec.md` cross-references update to point at `docs/rebuild/spec/loam-objectives-spec.md`. The 19 `docs/odd-in-pos.md` cross-references update to point at `docs/odd-in-loam.md`.
...
5. **Spec filename rename has load-bearing references inside the spec text that would break.** If `pos-v2-objectives-spec.md`'s body contains links to its own filename (rare) or external links from outside refer to `#anchors` in a way that breaks post-rename, surface specific file/line.
...
14. **A frozen-record file rebrand.** Builder accidentally rebrands `docs/rebuild/spec/pos-v2-rebuild-proposal.md` content or filename. Halt; revert; the file is preserved per dispatcher ruling 3.
...
2. **(Spec filename cross-reference inventory — non-blocking; surfaces the per-rename callsite count.)** Pre-build verification of cross-references:
   - `docs/odd-in-pos.md` referenced by 19 live files: CLAUDE.md (root) cites it as a session-start corpus; framework/<comp>/tests' `test_no_sealed_amendments.py` cross-cutting checks (graceful-degradation, memory-system) reference the file path; framework/hands-off-lifecycle/tests/test_AC_A4_S_seal_diff_window.py + test_AC_CI_*, test_AC_OBG_*, test_AC_TDG_*, test_AC45_S, test_AC_SE_S_seal_diff_window.py reference; framework/tools/pos-amend/{README.md, tests/test_integration_universal_paths.py, tests/test_seal_diff.py, templates/plan/dev-discipline.md} reference; HOL hooks reference (corpus_inline_session_start.py); framework/hands-off-lifecycle/seals/* historical seals reference (preserved); framework/graceful-degradation/tests/test_no_sealed_amendments.py:21–22 references with the line-break-split pattern that M1d build-time finding #10 caught (post-M1e the file IS renamed, so the split-line filename should be `odd-in-loam.md`; surgical edit needed).
   - `docs/rebuild/spec/pos-v2-objectives-spec.md` referenced by ~5 live files (STATE.md, VALUE_PROPOSITION.md, cross-cutting plan-docs).
   - `docs/rebuild/spec/pos-v2-rebuild-proposal.md` referenced by ~5 live files (mostly STATE.md / VALUE_PROPOSITION.md historical context — NOT touched per dispatcher ruling 3 + Idea 10's no-retroactive-rewrites clause; cross-references stay verbatim).
...
- **Authority documents (inherited from series master):**
  - `docs/rebuild/plans/loam-rename-decisions.md` Tier-1 items 2–7.
  - `.scratch/claude-output/loam-rename-migration-plan.md` (mechanics + dependency ordering).
- **Programme master plan:** `docs/rebuild/plans/oss-v0-1-0-publish.md` (M1e row in §5 per M1b precursor commit `7be713b`).
- **STATE.md** — governing rules.
- **ODD methodology + ODD-in-pos:** `docs/odd-methodology.md`, `docs/odd-in-pos.md` (this M1e RENAMES the latter to `docs/odd-in-loam.md`).
...
- **Precedent multi-component sealed-amendment manifests:**
  - `docs/rebuild/plans/oss-v0-1-0-publish-rename-1d.manifest.yaml` (M1d sibling — 13-component OTel fence; post-M1d HC#4 in-band rebaseline precedent).
  - `docs/rebuild/plans/oss-v0-1-0-publish-rename-1c.manifest.yaml` (M1c sibling — 5-component fence).
  - `docs/rebuild/plans/oss-v0-1-0-publish-rename-1b.manifest.yaml` (M1b sibling — 11-component fence).
  - `docs/rebuild/plans/oss-v0-1-0-publish-rename-1a.manifest.yaml` (M1a sibling — 4-component docs-only fence).
...
- **Idea 10 (no-retroactive-rewrites clause):** `docs/rebuild/plans/loam-rename-decisions.md` Q1 + Q2.
