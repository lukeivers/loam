...
- Records lifted from amendment plan docs → `"user"` if the
  ...
  in `docs/plans/amendment-*.md`); `"primary-persona"` (or a
...
- `lifted_from` is structurally meaningful for plan-docs-as-
  ...
  produce `docs/plans/amendment-29-*.md`, it groups records
...
`docs/plans/<work-item-name>.md`. Plan writes-to-disk
...
pOS-core content (the document is in `docs/VALUE_PROPOSITION.md`).
**Tension.**
...
- `docs/VALUE_PROPOSITION.md` — primary-persona test, harness
  test (lines 58–69).
- `docs/STATE.md` — rule #4 (zero core personas), rule #5
  (nothing worked unless against), rule #7 (background-work
  awareness).
- `docs/FUTURE_IDEAS.md` — three lenses, dev CDCs (research-
  before-plan, plan-before-code, all-execution-through-background-
  agents, scope-only-dispatch).
- `docs/spec/pos-v2-objectives-spec.md` — v1.0 / v1.1 / v1.2.
- `docs/archive/component-research/objective-tracker/proposal.md` — D1–D9.
...
- `docs/plans/amendment-{29,30,31,32,33,34}-*.md` — recent
